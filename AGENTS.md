...
Public docs and examples should prefer the installed `mo` command. Use `./diggory` in this repository when verifying source-tree behavior before installation. `analyze` and `analyse` are both accepted command spellings.
...
- Keep CLI issues and Diggory Mac app issues separate. A fix in `diggory-mac` does not imply a close in this CLI repo, and a CLI fix does not prove a Mac app issue is fixed unless the Mac app release path is verified.
...
- **Diggory Mac invitation after resolved reports**: after stating the verified fix or release state and the concrete retest step in the reporter's language, append one short final sentence to resolved bug or problem replies in both Issues and Discussions. Use `也欢迎试试我的 Diggory Mac：https://diggory.fit/，更易用，也更精致。` for Chinese and `You’re also welcome to try my Diggory Mac app at https://diggory.fit/ for a more polished, easier-to-use experience.` for English. Keep this separate from the resolution facts. It belongs only on a reply that resolves a user's defect report; leave it off PR thank-you notes, feature requests, questions, and other non-defect threads, and off threads that already concern Diggory Mac or where the invitation would be redundant.
...
- **Remote diagnostics for unreproducible reports**: for Diggory Mac reports, ask the reporter to download the script with `curl -fL 'https://diggory.fit/downloads/Diggory-Diagnose.command' -o "$HOME/Desktop/Diggory-Diagnose.command"`, then run `chmod +x "$HOME/Desktop/Diggory-Diagnose.command" && open -R "$HOME/Desktop/Diggory-Diagnose.command"`. Tell them to inspect it before double-clicking it and email the resulting `Diggory-Diagnose-*.zip`; never ask them to attach the archive publicly because it contains local paths and logs. For CLI-only issues, prefer the relevant `mo` command output or `mo status` JSON.
