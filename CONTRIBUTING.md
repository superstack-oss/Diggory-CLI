...
- Go 1.24+ is required to build the `digg status` or `digg analyze` TUI binaries locally.

## Go Components

`digg status` and `digg analyze` use Go with Bubble Tea for interactive dashboards.
