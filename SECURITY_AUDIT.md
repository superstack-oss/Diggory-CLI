...
This document describes the security-relevant behavior of the current `main` branch, updated for V1.0.1 on 2026-09-02. It is intended as a public description of Diggory's safety boundaries, destructive-operation controls, release integrity signals, and known limitations.

## Executive Summary

Diggory is a local system maintenance tool. Its main risk surface is not remote code execution; it is unintended local damage caused by cleanup, uninstall, optimize, purge, installer cleanup, or other destructive operations.
...
Diggory prioritizes bounded cleanup over aggressive cleanup. When uncertainty exists, the tool should refuse, skip, or require stronger confirmation instead of widening deletion scope.
...
The highest-risk areas in Diggory are:
...
Diggory has explicit protected-path and protected-category logic in addition to root-path blocking.
...
- `diggory_delete` validates symlinks rather than skipping them, so a symlink whose target resolves into a protected root is refused instead of silently moved
...
Diggory uses sudo for a subset of system-maintenance paths, but elevated behavior is still bounded by validation and protected-path rules.
...
Diggory is not intended to aggressively delete high-value user data.
...
- reverse-DNS bundle ID validation is required before LaunchAgent and LaunchDaemon pattern matching; bundle ID matching uses boundary-aware comparisons (`diggory_name_starts_with_bundle_id_boundary`, `diggory_name_has_bundle_id_boundary`) to prevent cross-app false matches (e.g. `com.example` not matching `com.example123`), and `defaults delete` is guarded by `diggory_is_reverse_dns_bundle_id()` to reject malformed or adversarial domain strings
...
- apps managed by an official vendor uninstaller are excluded from Diggory's own removal list, so the vendor's uninstall flow remains authoritative
...
- batch uninstall now displays system-level remnants for review instead of deleting them; the confirmation prompt is retained and any `launchctl unload`/`bootout` runs under dry-run and `DIGGORY_TEST_MODE` guards
...
Diggory exposes multiple safety controls before and during destructive actions:
...
Diggory treats release trust as part of its security posture, not just a packaging detail.
...
- `tests/file_ops_diggory_delete.bats`
- `tests/purge.bats`
