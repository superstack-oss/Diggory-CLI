...
DIGGORY_USER_HOME="$(get_invoking_home)"
[[ -n "$DIGGORY_USER_HOME" ]] || DIGGORY_USER_HOME="$HOME"

CLEAN_PREVIEW_FINAL_FILE="$DIGGORY_USER_HOME/.config/diggory/clean-list.txt"
...
WHITELIST_WARNINGS=()
if [[ -f "$DIGGORY_USER_HOME/.config/diggory/whitelist" ]]; then
    while IFS= read -r line; do
        ...
        [[ "$line" == ~* ]] && line="${line/#~/$DIGGORY_USER_HOME}"
        line="${line//\$HOME/$DIGGORY_USER_HOME}"
        line="${line//\$\{HOME\}/$DIGGORY_USER_HOME}"
        if [[ "$line" =~ \.\. ]]; then
    ...
    done < "$DIGGORY_USER_HOME/.config/diggory/whitelist"
else
...
expand_whitelist_patterns() {
    if [[ ${#WHITELIST_PATTERNS[@]} -gt 0 ]]; then
        ...
        for pattern in "${WHITELIST_PATTERNS[@]}"; do
            local expanded="${pattern/#\~/$DIGGORY_USER_HOME}"
...
prepare_clean_preview_file() {
    ...
    if is_root_user && [[ -n "${SUDO_USER:-}" && "${SUDO_USER:-}" != "root" ]]; then
        ensure_diggory_temp_root || return 1
        local root_temp_dir="$DIGGORY_RESOLVED_TMPDIR"

        CLEAN_PREVIEW_STAGING_FILE=$(umask 077 && mktemp "$root_temp_dir/diggory.clean-preview.XXXXXX") || return 1
        [[ -f "$CLEAN_PREVIEW_STAGING_FILE" && ! -L "$CLEAN_PREVIEW_STAGING_FILE" && -O "$CLEAN_PREVIEW_STAGING_FILE" ]] || return 1
        DIGGORY_TEMP_FILES+=("$CLEAN_PREVIEW_STAGING_FILE")
...
register_dry_run_cleanup_target() {
    ...
    local path="$1"
    local identity
    identity=$(diggory_path_identity "$path")

    if [[ ${#DRY_RUN_SEEN_IDENTITIES[@]} -gt 0 ]] && diggory_identity_in_list "$identity" "${DRY_RUN_SEEN_IDENTITIES[@]}"; then
        return 1
    fi
...
record_dry_run_cleanup_target() {
    ...
    if declare -f _diggory_should_refuse_live_user_cache_path > /dev/null 2>&1 &&
        _diggory_should_refuse_live_user_cache_path "$path"; then
        return 1
    fi
    if declare -f _diggory_is_sqlite_database_path > /dev/null 2>&1 &&
        _diggory_is_sqlite_database_path "$path" &&
        declare -f _diggory_sqlite_database_in_use > /dev/null 2>&1; then
        local sqlite_state=0
        _diggory_sqlite_database_in_use "$path" || sqlite_state=$?
...
emit_deduplicated_dry_run_ledger() {
    ...
    while IFS= read -r -d '' identity &&
        ...
        if [[ ${#seen_identities[@]} -gt 0 ]] && diggory_identity_in_list "$identity" "${seen_identities[@]}"; then
            continue
        fi
...
# Diggory Cleanup Preview - $(date '+%Y-%m-%d %H:%M:%S')
#
    ...
    if [[ -n "${CLEAN_PREVIEW_LEDGER_FILE:-}" && -f "$CLEAN_PREVIEW_LEDGER_FILE" ]]; then
        while IFS= read -r -d '' identity &&
            ...
            if [[ "$section" != "$current_rendered_section" ]]; then
                ...
                if [[ ${#seen_sections[@]} -eq 0 ]] || ! diggory_identity_in_list "$section" "${seen_sections[@]}"; then
...
read_clean_sudo_choice() {
    ...
    local previous_force_char="${DIGGORY_READ_KEY_FORCE_CHAR:-}"
    if [[ ${DIGGORY_READ_KEY_FORCE_CHAR+x} ]]; then
        had_force_char=true
    fi

    export DIGGORY_READ_KEY_FORCE_CHAR=1
    local choice
    choice=$(read_key)

    if [[ "$had_force_char" == "true" ]]; then
        export DIGGORY_READ_KEY_FORCE_CHAR="$previous_force_char"
    else
        unset DIGGORY_READ_KEY_FORCE_CHAR
    fi
...
_safe_clean_impl() {
    ...
    local _DIGGORY_SAFE_CLEAN_BOUND_PATH=""
    local _DIGGORY_SAFE_CLEAN_EXPECTED_PARENT=""
    local _DIGGORY_SAFE_CLEAN_EXPECTED_PARENT_ID=""
    local _DIGGORY_SAFE_CLEAN_EXPECTED_TARGET_ID=""
    local permission_start=${DIGGORY_PERMISSION_DENIED_COUNT:-0}
    ...
    if [[ ${#existing_paths[@]} -gt 3 ]]; then
        ...
        if [[ $dir_count -lt 5 && ${#existing_paths[@]} -gt 20 ]]; then
            ...
            if [[ ${#existing_paths[@]} -gt 0 ]]; then
                for path in "${existing_paths[@]}"; do
                    ...
                    if ((${#pids[@]} >= DIGGORY_MAX_PARALLEL_JOBS)); then
                        local wait_rc=0
        ...
        for _t_file in "$temp_dir"/result_*; do
            ...
            [[ "$_t_flag" == "1" ]] && DIGGORY_CLEAN_SIZING_TIMEOUTS=$((${DIGGORY_CLEAN_SIZING_TIMEOUTS:-0} + 1))
        done
        ...
        if [[ $cleanup_interrupt_rc -ne 0 ]]; then
            ...
            DIGGORY_CLEAN_CANCEL_STATUS=$cleanup_interrupt_rc
                ...
                if [[ -f "$result_file" ]]; then
                    ...
                    if [[ "$DRY_RUN" != "true" ]]; then
                        if [[ -n "$delete_guard" ]]; then
                            _DIGGORY_SAFE_CLEAN_BOUND_PATH=""
                            _DIGGORY_SAFE_CLEAN_EXPECTED_PARENT=""
                            _DIGGORY_SAFE_CLEAN_EXPECTED_PARENT_ID=""
                            _DIGGORY_SAFE_CLEAN_EXPECTED_TARGET_ID=""
                        ...
                        if [[ "$_DIGGORY_SAFE_CLEAN_BOUND_PATH" == "$path" ]]; then
                            bound_parent="$_DIGGORY_SAFE_CLEAN_EXPECTED_PARENT"
                            bound_parent_id="$_DIGGORY_SAFE_CLEAN_EXPECTED_PARENT_ID"
                            bound_target_id="$_DIGGORY_SAFE_CLEAN_EXPECTED_TARGET_ID"
                        fi
    ...
    local permission_end=${DIGGORY_PERMISSION_DENIED_COUNT:-0}
...
start_cleanup() {
    ...
    DIGGORY_CLEAN_CANCEL_STATUS=0
    export DIGGORY_CLEAN_CANCEL_STATUS
    DIGGORY_CLEAN_SIZING_TIMEOUTS=0
    export DIGGORY_CLEAN_SIZING_TIMEOUTS
    DIGGORY_CLEAN_REMOVAL_TIMEOUTS=0
    export DIGGORY_CLEAN_REMOVAL_TIMEOUTS
...
perform_cleanup() {
    ...
    if [[ -z "$EXTERNAL_VOLUME_TARGET" && "${DIGGORY_TEST_MODE:-0}" == "1" ]]; then
    ...
    _run_cleanup_step() {
        ...
        if [[ $step_rc -eq 124 || $step_rc -ge 128 ]]; then
            DIGGORY_CLEAN_CANCEL_STATUS=$step_rc
            export DIGGORY_CLEAN_CANCEL_STATUS
            return "$step_rc"
        fi
        if [[ $pending_clean_cancel -eq 124 || $pending_clean_cancel -ge 128 ]]; then
    ...
    if [[ $total_size_cleaned -gt 0 ||
        ...
        if [[ "$DRY_RUN" == "true" ]]; then
            ...
            if ((total_size_cleaned >= DIGGORY_ONE_GIB_KB)); then
                local freed_gb=$((total_size_cleaned / DIGGORY_ONE_GIB_KB))
    ...
    # Caches of running apps are deferred silently: skipping them is Diggory's
    ...
    if [[ ${DIGGORY_CLEAN_SIZING_TIMEOUTS:-0} -gt 0 ]]; then
        summary_details+=("${GRAY}${ICON_WARNING}${NC} Some items exceeded the ${DIGGORY_TIMEOUT_DISK_VERIFY_SEC}s size-check budget and were counted as 0, so the total is under-reported. Raise ${GRAY}DIGGORY_TIMEOUT_DISK_VERIFY_SEC${NC} to measure them.")
    fi

    if [[ ${DIGGORY_CLEAN_REMOVAL_TIMEOUTS:-0} -gt 0 ]]; then
        summary_details+=("${GRAY}${ICON_WARNING}${NC} ${DIGGORY_CLEAN_REMOVAL_TIMEOUTS} item(s) exceeded the ${DIGGORY_TIMEOUT_DISK_VERIFY_SEC}s removal budget and may be only partly removed. Run clean again, or raise ${GRAY}DIGGORY_TIMEOUT_DISK_VERIFY_SEC${NC} for slower disks.")
    fi
