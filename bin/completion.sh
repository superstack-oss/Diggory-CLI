...
emit_fish_completions() {
    ...
    for entry in "${DIGGORY_COMMANDS[@]}"; do
        local name="${entry%%:*}"
        local desc="${entry#*:}"
        printf 'complete -f -c %s -n "__fish_diggory_no_subcommand" -a %s -d "%s"\n' "$cmd" "$name" "$desc"
    done
    ...
    printf '\n'
    printf 'complete -f -c %s -n "not __fish_diggory_no_subcommand" -a bash -d "generate bash completion" -n "__fish_see_subcommand_path completion"\n' "$cmd"
    printf 'complete -f -c %s -n "not __fish_diggory_no_subcommand" -a zsh -d "generate zsh completion" -n "__fish_see_subcommand_path completion"\n' "$cmd"
    printf 'complete -f -c %s -n "not __fish_diggory_no_subcommand" -a fish -d "generate fish completion" -n "__fish_see_subcommand_path completion"\n' "$cmd"
}
...
if [[ $# -eq 0 ]]; then
    ...
    if [[ "$current_shell" == "fish" ]]; then
        ...
        if [[ -z "$completion_name" ]]; then
            # Clean up any stale config.fish entries even when diggory is not in PATH
            if [[ "${DIGGORY_DRY_RUN:-0}" != "1" ]]; then
                remove_stale_completion_entries "$config_fish" "Removed stale completion entries from config.fish" || true
            fi
            log_error "diggory not found in PATH, install Diggory before enabling completion"
            exit 1
        fi
    ...
    case "$current_shell" in
        ...
        *)
            ...
            echo "  diggory completion <bash|zsh|fish>"
            exit 1
            ;;
    esac
    ...
    # Remove previous Diggory completion lines to avoid duplicates
    if [[ -f "$config_file" ]]; then
        original_mode=""
        original_mode="$(stat -f '%Mp%Lp' "$config_file" 2> /dev/null || true)"
        temp_file="$(mktemp)"
...
Auto-install:
  ...
  diggory completion --dry-run    # Preview config changes without writing files

Manual install:
  diggory completion bash         # Generate bash completion script
  diggory completion zsh          # Generate zsh completion script
  diggory completion fish         # Generate fish completion script
...
Examples:
  ...
  # Manual install - Bash
  eval "$(diggory completion bash)"

  # Manual install - Zsh
  eval "$(diggory completion zsh)"

  # Manual install - Fish
  diggory completion fish | source
EOF
        exit 1
        ;;
esac
