...
declare DIGGORY_SELECTION_RESULT
declare DIGGORY_INSTALLER_SCAN_MAX_DEPTH

export LC_ALL=C
export LANG=C
export DIGGORY_CURRENT_COMMAND="${DIGGORY_CURRENT_COMMAND:-installer}"
...
scan_installers_in_path() {
    local path="$1"
    local max_depth="${DIGGORY_INSTALLER_SCAN_MAX_DEPTH:-$INSTALLER_SCAN_MAX_DEPTH_DEFAULT}"
    ...
    while true; do
        ...
        case "$key" in
            ...
            "" | $'\n' | $'\r') # Enter - confirm
                ...
                for ((i = 0; i < total_items; i++)); do
                    if [[ ${selected[i]} == true ]]; then
                        [[ -n "$DIGGORY_SELECTION_RESULT" ]] && DIGGORY_SELECTION_RESULT+=","
                        DIGGORY_SELECTION_RESULT+="$i"
                    fi
                done
                restore_terminal
                return 0
...
build_installer_delete_plan() {
    ...
    for idx in "$@"; do
        ...
        INSTALLER_DELETE_IDENTITIES+=("$(diggory_path_identity "$file_path")")
    done
...
execute_installer_delete_plan() {
    ...
    for ((plan_index = 0; plan_index < ${#INSTALLER_DELETE_PATHS[@]}; plan_index++)); do
        ...
        current_identity=$(diggory_path_identity "$file_path")
        ...
        if diggory_delete "$file_path" false; then
            if [[ "${DIGGORY_DRY_RUN:-0}" == "1" ]] || [[ ! -e "$file_path" && ! -L "$file_path" ]]; then
