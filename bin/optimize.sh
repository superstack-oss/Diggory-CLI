...
show_optimization_summary() {
    ...
    applied=$(optimize_outcome_count "$DIGGORY_OPTIMIZE_OUTCOME_APPLIED")
    unchanged=$(optimize_outcome_count "$DIGGORY_OPTIMIZE_OUTCOME_UNCHANGED")
    skipped=$(optimize_outcome_count "$DIGGORY_OPTIMIZE_OUTCOME_SKIPPED")
    unavailable=$(optimize_outcome_count "$DIGGORY_OPTIMIZE_OUTCOME_UNAVAILABLE")
    attention=$(optimize_outcome_count "$DIGGORY_OPTIMIZE_OUTCOME_ATTENTION")
    failed=$(optimize_outcome_count "$DIGGORY_OPTIMIZE_OUTCOME_FAILED")
...
main() {
    ...
    export DIGGORY_CURRENT_COMMAND="optimize"
    ...
    export DIGGORY_OPTIMIZE_SUDO_AVAILABLE="false"
    if [[ "${DIGGORY_DRY_RUN:-0}" == "1" ]]; then
        DIGGORY_OPTIMIZE_SUDO_AVAILABLE="true"
    ...
    for ((index = 0; index < ${#DIGGORY_OPTIMIZE_ACTIONS[@]}; index++)); do
        ...
        health_name=${DIGGORY_OPTIMIZE_HEALTH_NAMES[$index]}
        announce_action "$health_name"
        execute_optimization "$action"
    done

    if [[ "$(optimize_outcome_total)" -ne ${#DIGGORY_OPTIMIZE_ACTIONS[@]} ]]; then
