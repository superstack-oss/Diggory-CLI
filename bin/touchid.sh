...
PAM_SUDO_FILE="${DIGGORY_PAM_SUDO_FILE:-/etc/pam.d/sudo}"
PAM_SUDO_LOCAL_FILE="${DIGGORY_PAM_SUDO_LOCAL_FILE:-$(dirname "$PAM_SUDO_FILE")/sudo_local}"
...
enable_touchid() {
    ...
    if [[ ! -f "${PAM_SUDO_FILE}.diggory-backup" ]]; then
        if ! sudo cp "$PAM_SUDO_FILE" "${PAM_SUDO_FILE}.diggory-backup" 2> /dev/null; then
