...
readonly DIGGORY_UNINSTALL_USER_LC_ALL="${LC_ALL:-}"
readonly DIGGORY_UNINSTALL_USER_LANG="${LANG:-}"
...
readonly DIGGORY_UNINSTALL_META_CACHE_DIR="$HOME/.cache/diggory"
readonly DIGGORY_UNINSTALL_META_CACHE_FILE="$DIGGORY_UNINSTALL_META_CACHE_DIR/uninstall_app_metadata_v2"
readonly DIGGORY_UNINSTALL_META_CACHE_LOCK="${DIGGORY_UNINSTALL_META_CACHE_FILE}.lock"
readonly DIGGORY_UNINSTALL_META_REFRESH_TTL=604800 # 7 days
readonly DIGGORY_UNINSTALL_EPOCH_FLOOR=978307200
# Display-name mdls lookup budget during scan; overridable for slow disks or
# cold Spotlight.
readonly DIGGORY_UNINSTALL_INLINE_MDLS_DISPLAY_TIMEOUT_SEC="${DIGGORY_UNINSTALL_INLINE_MDLS_DISPLAY_TIMEOUT_SEC:-0.04}"
readonly DIGGORY_UNINSTALL_INLINE_MDLS_SIZE_TIMEOUT_SEC="${DIGGORY_UNINSTALL_INLINE_MDLS_SIZE_TIMEOUT_SEC:-0.04}"
...
readonly DIGGORY_UNINSTALL_INLINE_DU_SIZE_TIMEOUT_SEC="${DIGGORY_UNINSTALL_INLINE_DU_SIZE_TIMEOUT_SEC:-2}"
readonly DIGGORY_UNINSTALL_INLINE_DU_MAX_COLD_ROWS="${DIGGORY_UNINSTALL_INLINE_DU_MAX_COLD_ROWS:-20}"
...
uninstall_quick_app_size_kb() {
    ...
    local physical_size
    physical_size=$(run_with_timeout "$DIGGORY_UNINSTALL_INLINE_MDLS_SIZE_TIMEOUT_SEC" mdls -name kMDItemPhysicalSize -raw "$app_path" 2> /dev/null || echo "")
...
uninstall_inline_du_size_kb() {
    ...
    local du_size_kb
    du_size_kb=$(run_with_timeout "$DIGGORY_UNINSTALL_INLINE_DU_SIZE_TIMEOUT_SEC" du -skP "$app_path" 2> /dev/null | awk '{print $1; exit}') || du_size_kb=""
...
uninstall_resolve_display_name() {
    ...
    if [[ -f "$app_path/Contents/Info.plist" ]]; then
        ...
        if [[ -n "$DIGGORY_UNINSTALL_USER_LC_ALL" ]]; then
            md_display_name=$(run_with_timeout "$DIGGORY_UNINSTALL_INLINE_MDLS_DISPLAY_TIMEOUT_SEC" env LC_ALL="$DIGGORY_UNINSTALL_USER_LC_ALL" LANG="$DIGGORY_UNINSTALL_USER_LANG" mdls -name kMDItemDisplayName -raw "$app_path" 2> /dev/null || echo "")
        elif [[ -n "$DIGGORY_UNINSTALL_USER_LANG" ]]; then
            md_display_name=$(run_with_timeout "$DIGGORY_UNINSTALL_INLINE_MDLS_DISPLAY_TIMEOUT_SEC" env LANG="$DIGGORY_UNINSTALL_USER_LANG" mdls -name kMDItemDisplayName -raw "$app_path" 2> /dev/null || echo "")
        else
            md_display_name=$(run_with_timeout "$DIGGORY_UNINSTALL_INLINE_MDLS_DISPLAY_TIMEOUT_SEC" mdls -name kMDItemDisplayName -raw "$app_path" 2> /dev/null || echo "")
        fi
    ...
    (
        _refresh_debug() {
            if [[ "${MO_DEBUG:-}" == "1" ]]; then
                ...
                echo "[$ts] DEBUG: [metadata-refresh] $*" >> "${HOME}/.config/diggory/diggory_debug_session.log" 2> /dev/null || true
            fi
        }

        ensure_user_dir "$DIGGORY_UNINSTALL_META_CACHE_DIR"
        ensure_user_file "$DIGGORY_UNINSTALL_META_CACHE_FILE"
        if [[ ! -r "$DIGGORY_UNINSTALL_META_CACHE_FILE" ]]; then
            if ! : > "$DIGGORY_UNINSTALL_META_CACHE_FILE" 2> /dev/null; then
                _refresh_debug "Cannot create cache file, aborting"
                exit 0
            fi
        fi
        if [[ ! -w "$DIGGORY_UNINSTALL_META_CACHE_FILE" ]]; then
        ...
        while IFS='|' read -r app_path app_mtime bundle_id display_name; do
            ...
            (
                ...
                if [[ ! "$last_used_epoch" =~ ^[0-9]+$ || $last_used_epoch -le 0 || $last_used_epoch -lt $DIGGORY_UNINSTALL_EPOCH_FLOOR ]]; then
                    last_used_epoch=0
                fi
        ...
        if ! uninstall_acquire_metadata_lock "$DIGGORY_UNINSTALL_META_CACHE_LOCK"; then
        ...
        refresh_merged_file=$(mktemp 2> /dev/null) || {
            ...
            uninstall_release_metadata_lock "$DIGGORY_UNINSTALL_META_CACHE_LOCK"
            rm -f "$updates_file"
            exit 0
        }
        ...
        ' "$updates_file" "$DIGGORY_UNINSTALL_META_CACHE_FILE" > "$refresh_merged_file"

        uninstall_persist_cache_file "$refresh_merged_file" "$DIGGORY_UNINSTALL_META_CACHE_FILE"
...
_scan_resolve_uncached() {
    ...
    if [[ "$DIGGORY_UNINSTALL_INLINE_DU_MAX_COLD_ROWS" =~ ^[0-9]+$ ]] &&
        ((total_apps > 0 && total_apps <= DIGGORY_UNINSTALL_INLINE_DU_MAX_COLD_ROWS)); then
...
#         ${temp_file}.sorted, DIGGORY_UNINSTALL_META_CACHE_FILE
...
_scan_finalize_index() {
    ...
    awk -F'|' \
        -v now="$current_epoch" \
        -v floor="$DIGGORY_UNINSTALL_EPOCH_FLOOR" \
        -v ttl="$DIGGORY_UNINSTALL_META_REFRESH_TTL" \
    ...
    if [[ -s "$cache_snapshot_file" ]]; then
        if uninstall_acquire_metadata_lock "$DIGGORY_UNINSTALL_META_CACHE_LOCK"; then
            uninstall_persist_cache_file "$cache_snapshot_file" "$DIGGORY_UNINSTALL_META_CACHE_FILE"
            uninstall_release_metadata_lock "$DIGGORY_UNINSTALL_META_CACHE_LOCK"
        fi
    fi
...
scan_applications() {
    ...
    ensure_user_dir "$DIGGORY_UNINSTALL_META_CACHE_DIR"
    ensure_user_file "$DIGGORY_UNINSTALL_META_CACHE_FILE"
    local cache_source="$DIGGORY_UNINSTALL_META_CACHE_FILE"
    ...
    start_scan_spinner() {
        ...
        [[ -t 2 || "${DIGGORY_TEST_FORCE_SCAN_SPINNER:-0}" == "1" ]] || return 0
        (
            # shellcheck disable=SC2329  # Function invoked indirectly via trap
...
start_uninstall_interactive_screen() {
    if [[ -t 1 && -t 2 && "${DIGGORY_ALT_SCREEN_ACTIVE:-}" != "1" ]]; then
        enter_alt_screen
...
stop_uninstall_interactive_screen() {
    if [[ "${DIGGORY_ALT_SCREEN_ACTIVE:-}" == "1" ]]; then
        leave_alt_screen
    fi
...
main() {
    ...
    export DIGGORY_CURRENT_COMMAND="uninstall"
    ...
    export DIGGORY_DELETE_MODE="${DIGGORY_DELETE_MODE:-trash}"
    ...
    unset DIGGORY_INLINE_LOADING DIGGORY_MANAGED_ALT_SCREEN DIGGORY_ALT_SCREEN_ACTIVE
    while true; do
        unset DIGGORY_INLINE_LOADING
        ...
        if [[ $exit_code -ne 0 ]]; then
            rm -f "$apps_file"
            [[ "$apps_file" == "$cached_apps_file" ]] && cached_apps_file=""
            if [[ "${_DIGGORY_MENU_USER_QUIT:-0}" == "1" ]]; then
                # A deliberate q is a cancel, not a failure: leave quietly
                # with success, matching diggory's other cancel flows. Only a
        ...
        # parent shell. Reading while Diggory is no longer the foreground process
        ...
        if ! diggory_tty_is_foreground; then
            show_cursor
            return 0
        fi
