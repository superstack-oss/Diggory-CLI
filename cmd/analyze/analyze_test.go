...
// The legacy flat store shares `~/.cache/diggory` with shell-side state, so the
...
func TestGetCacheDirIsAnalyzerScoped(t *testing.T) {
	...
	root, err := getDiggoryCacheRoot()
	if err != nil {
		t.Fatalf("getDiggoryCacheRoot: %v", err)
	}
	cacheDir, err := getCacheDir()
	if err != nil {
		t.Fatalf("getCacheDir: %v", err)
...
// ~/.cache/diggory to 1.88M files / 7.82GB on a user's Mac.
...
func TestIsHandledByMoClean(t *testing.T) {
	tests := []struct {
		...
		// Paths digg clean does NOT handle.
...
func TestIsCleanableDir(t *testing.T) {
	tests := []struct {
		...
		// Paths handled by digg clean (should NOT be cleanable).
