...
// diggoryCacheRoot is the single definition of the shared cache location; both
// accessors below build on it so the layout is stated once.
func diggoryCacheRoot(home string) string {
	return filepath.Join(home, ".cache", "diggory")
}

// getDiggoryCacheRoot is the shared `~/.cache/diggory` directory. The shell side
// keeps its own state files there, so nothing may be swept from it wholesale.
func getDiggoryCacheRoot() (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return diggoryCacheRoot(home), nil
}
...
func getCacheDir() (string, error) {
	...
	dir := filepath.Join(diggoryCacheRoot(home), analyzerCacheDirName)
...
func pruneAnalyzerCache() {
	...
	// cache housekeeping.
	if root, err := getDiggoryCacheRoot(); err == nil {
...
// used to write directly into `~/.cache/diggory`, plus the overview snapshot that
