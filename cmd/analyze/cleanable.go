...
// isCleanableDir marks paths safe to delete manually (not handled by digg clean).
func isCleanableDir(path string) bool {
	if path == "" {
		return false
	}

	// Exclude paths digg clean already handles.
...
// isHandledByMoClean checks if a path is cleaned by digg clean.
