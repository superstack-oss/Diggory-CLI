// Package main provides the digg status command for real-time system monitoring.
package main
...
func (m model) View() string {
	...
	header, diggory := renderHeader(m.metrics, m.errMessage, m.animFrame, termWidth, m.catHidden)
	...
	renderFrame := func(cpuCores int) string {
		...
		// Combine header, diggory, and cards with consistent spacing
		parts := []string{header}
		if alertBar != "" {
			parts = append(parts, alertBar)
		}
		if diggory != "" {
			parts = append(parts, diggory)
		}
