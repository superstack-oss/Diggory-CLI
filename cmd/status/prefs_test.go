...
func TestSavePrefConcurrentProcessesPreserveEveryKey(t *testing.T) {
	...
	for i := range writers {
		...
		cmd.Env = append(os.Environ(),
			"HOME="+home,
			"DIGGORY_PREFS_TEST_HELPER=1",
			fmt.Sprintf("DIGGORY_PREFS_TEST_KEY=key_%02d", i),
			"DIGGORY_PREFS_TEST_VALUE="+strconv.Itoa(i),
			"DIGGORY_PREFS_TEST_BARRIER="+barrier,
			fmt.Sprintf("DIGGORY_PREFS_TEST_READY=%s", filepath.Join(readyDir, strconv.Itoa(i))),
		)
...
func TestSavePrefProcessHelper(t *testing.T) {
	if os.Getenv("DIGGORY_PREFS_TEST_HELPER") != "1" {
		return
	}
	if barrier := os.Getenv("DIGGORY_PREFS_TEST_BARRIER"); barrier != "" {
		if ready := os.Getenv("DIGGORY_PREFS_TEST_READY"); ready != "" {
	...
	savePref(os.Getenv("DIGGORY_PREFS_TEST_KEY"), os.Getenv("DIGGORY_PREFS_TEST_VALUE"))
}
