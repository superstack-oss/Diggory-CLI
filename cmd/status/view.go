...
// Diggory body frames (facing right).
var diggoryBody = [][]string{
	{
		`     /\_/\`,
		` ___/ o o \`,
		`/___   =-= /`,
...
// Mirror diggory body frames (facing left).
var diggoryBodyMirror = [][]string{
	{
		`    /\_/\`,
		`   / o o \___`,
		`  \ =-=   ___\`,
...
// getDiggoryFrame renders the animated diggory.
func getDiggoryFrame(animFrame int, termWidth int) string {
	diggoryWidth := 15
	maxPos := max(termWidth-diggoryWidth, 0)
...
func renderHeader(m MetricsSnapshot, errMsg string, animFrame int, termWidth int, catHidden bool) (string, string) {
	...
	// Show cat unless hidden - render diggory centered below header
	var diggory string
	if !catHidden {
		diggory = getDiggoryFrame(animFrame, termWidth)
	}
	...
	if errMsg != "" {
		...
		return lipgloss.JoinVertical(lipgloss.Left, headerLine, "", diggory, dangerStyle.Render("ERROR: "+errMsg)), ""
	}
	if diggory == "" {
		return headerLine, ""
	}
	return headerLine, diggory
}
