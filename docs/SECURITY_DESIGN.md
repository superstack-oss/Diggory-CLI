# Diggory Security Design

This document describes the safety mechanisms that prevent diggory from
destroying data it shouldn't. It is written for reviewers, contributors,
and anyone evaluating diggory for production use.
...
Diggory is a user-invoked CLI that performs three classes of destructive
...
- A user who runs `sudo diggory` with malicious flags they typed in deliberately.
- A compromised macOS host where SIP is disabled and `/System` is writable.
- Supply-chain compromise of the diggory binary itself (covered separately
...
Every removal in diggory funnels through `diggory_delete` /
...
`cleanup_temp_files` and the Diggory-temp-root sweeps in `lib/core/base.sh`, and
...
- `DIGGORY_DRY_RUN=1`: every safe-remove logs what it would do and
...
- `DIGGORY_TEST_NO_AUTH=1`: refuses to call `sudo`, `osascript`,
...
- **No code signing of the cleanup config.** We rely on filesystem
  ...
  edit `diggory` itself; the threat model says we don't defend that.
- **No anti-rollback.** A user who restores an old diggory binary or
...
- **No protection for arbitrary user paths.** `~/Documents/important`
  ...
  cleanup targets; diggory only guarantees system integrity.
- **No telemetry.** We never report what was scanned, deleted, or
  attempted. Mistakes are diagnosed locally via `~/.cache/diggory/`
  operation logs (path is `DIGGORY_OPLOG_PATH` overridable;
...
Last reviewed: 2026-05-21 (diggory V1.39.0).
