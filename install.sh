#!/bin/bash
# Diggory - Installer for manual installs.
...
curl_download_with_retry() {
    ...
    local curl_err=""
    curl_err="$(mktemp "${TMPDIR:-/tmp}/diggory-curl-err.XXXXXX")" || curl_err=""
...
ensure_sudo_ready() {
    ...
    if [[ "${DIGGORY_ASSUME_SUDO_AUTH:-0}" == "1" ]]; then
        sudo -n -v
        return
    fi
...
install_lock_current_shell_pid() {
    ...
    pid_file=$(/usr/bin/mktemp /tmp/diggory-install-pid.XXXXXX) || return 1
...
install_lock_prepare_dir() {
    local use_sudo="$1"
    local lock_dir="$INSTALL_DIR/.diggory-update.lock"
...
install_lock_remove_control() {
    ...
    local use_sudo="$2"
    local control_prefix="$INSTALL_DIR/.diggory-update.lock/control."
...
acquire_install_lock() {
    local lock_path="$INSTALL_DIR/.diggory-update.lock/kernel.lock"
    ...
    if [[ "$use_sudo" == "true" ]] &&
        ...
        # A caller that set DIGGORY_ASSUME_SUDO_AUTH did authenticate, but the
    ...
    if [[ ! -x /usr/bin/lockf ]]; then
        ...
        mutex_dir="$INSTALL_DIR/.diggory-update.lock/holder"
    fi
    install_lock_current_shell_pid owner_pid || return 1
    owner_start=$(install_lock_process_start "$owner_pid")
    [[ -n "$owner_start" ]] || return 1
    control_path=$(install_lock_command "$use_sudo" /usr/bin/mktemp "$INSTALL_DIR/.diggory-update.lock/control.XXXXXX") || return 1
...
report_install_lock_failure() {
    ...
    case "$INSTALL_LOCK_FAILURE" in
        ...
        unsafe_ancestor)
            ...
            ;;
        lock_dir)
            log_error "$INSTALL_DIR/.diggory-update.lock is not a usable lock directory"
            log_error "Inspect it, then retry: sudo ls -lde $INSTALL_DIR/.diggory-update.lock"
            ;;
        lock_path)
            log_error "$INSTALL_DIR/.diggory-update.lock/kernel.lock is not a regular file"
            log_error "Remove it, then retry: sudo rm -f $INSTALL_DIR/.diggory-update.lock/kernel.lock"
            ;;
        no_mutex)
...
resolve_source_dir() {
    if [[ -n "$SOURCE_DIR" && -d "$SOURCE_DIR" && -f "$SOURCE_DIR/diggory" ]]; then
        return 0
    fi
    ...
    if [[ -n "${BASH_SOURCE[0]:-}" && -f "${BASH_SOURCE[0]}" ]]; then
        ...
        if [[ -f "$script_dir/diggory" ]]; then
            SOURCE_DIR="$script_dir"
            return 0
        fi
    fi

    if [[ -n "${CLEAN_SOURCE_DIR:-}" && -d "$CLEAN_SOURCE_DIR" && -f "$CLEAN_SOURCE_DIR/diggory" ]]; then
    ...
    tmp="$(mktemp -d "${TMPDIR:-/tmp}/diggory.XXXXXX")"
    INSTALL_SOURCE_TMP="$tmp"

    local branch="${DIGGORY_VERSION:-}"
    if [[ -z "$branch" ]]; then
    ...
    local source_commit=""
    if [[ "$branch" == "main" ]]; then
        source_commit="${DIGGORY_INSTALL_COMMIT:-}"
    ...
    start_line_spinner "Fetching Diggory source, ${branch}..."
    if command -v curl > /dev/null 2>&1; then
        if curl_download_with_retry "$url" "$tmp/diggory.tar.gz" 2> /dev/null; then
            if tar -xzf "$tmp/diggory.tar.gz" -C "$tmp" 2> /dev/null; then
                stop_line_spinner

                local extracted_dir
                extracted_dir=$(find "$tmp" -mindepth 1 -maxdepth 1 -type d | head -n 1)

                if [[ -n "$extracted_dir" && -f "$extracted_dir/diggory" ]]; then
    ...
    start_line_spinner "Cloning Diggory source..."
...
# Version helpers
get_source_version() {
    local source_diggory="$SOURCE_DIR/diggory"
    if [[ -f "$source_diggory" ]]; then
        sed -n 's/^VERSION="\(.*\)"$/\1/p' "$source_diggory" | head -n1
    fi
}
...
verify_release_asset_checksum() {
    ...
    checksums_file="$(mktemp "${TMPDIR:-/tmp}/diggory-checksums.XXXXXX")" || return 1
    ...
    if download_release_checksums "$tag" "$checksums_file" > /dev/null 2>&1; then
        ...
        if [[ "$attestation_status" -eq 2 && "${DIGGORY_REQUIRE_ATTESTATION:-0}" == "1" ]]; then
            stop_line_spinner
            log_error "DIGGORY_REQUIRE_ATTESTATION=1 set but gh CLI unavailable or unauthenticated"
            rm -f "$checksums_file"
            return 1
        fi
...
get_installed_version() {
    local binary="$INSTALL_DIR/diggory"
    if [[ -x "$binary" ]]; then
        local version
        version=$(run_install_probe_with_timeout 5 "$binary" --version 2> /dev/null |
            awk '/Diggory version/ {print $NF; exit}' || true)
        if [[ -n "$version" ]]; then
            echo "$version"
        else
...
resolve_install_channel() {
    ...
    if [[ "${DIGGORY_EDGE_INSTALL:-}" == "true" ]]; then
        printf 'nightly\n'
        return 0
    fi
...
parse_args() {
    ...
    for i in "${!args[@]}"; do
        ...
        case "$token" in
            latest | main)
                export DIGGORY_VERSION="main"
                export DIGGORY_EDGE_INSTALL="true"
                version_token="$token"
                unset 'args[$i]'
                ;;
            dev)
                export DIGGORY_VERSION="dev"
                export DIGGORY_EDGE_INSTALL="true"
                version_token="$token"
                unset 'args[$i]'
                ;;
            [0-9]* | V[0-9]* | v[0-9]*)
                export DIGGORY_VERSION="$token"
                version_token="$token"
                unset 'args[$i]'
                ;;
            *)
...
check_requirements() {
    ...
    if homebrew_owns_diggory; then
        local diggory_path
        diggory_path=$(command -v diggory 2> /dev/null || true)
        local is_homebrew_binary=false

        if [[ -n "$diggory_path" && -L "$diggory_path" ]]; then
            if readlink "$diggory_path" | grep -q "Cellar/diggory"; then
        ...
        if [[ "$is_homebrew_binary" == "true" ]]; then
            ...
            echo -e "${YELLOW}Diggory is installed via Homebrew${NC}"
            echo ""
            echo "Choose one:"
            echo -e "  1. Update via Homebrew: ${GREEN}brew upgrade diggory${NC}"
            echo -e "  2. Switch to manual: ${GREEN}brew uninstall --force diggory${NC} then re-run this"
            echo ""
            exit 1
        else
            log_warning "Cleaning up stale Homebrew installation..."
            brew uninstall --force diggory > /dev/null 2>&1 || true
        fi
    fi
...
download_binary() {
    ...
    if curl_download_with_retry "$url" "$staged_path" 2> /dev/null; then
        ...
        # DIGGORY_VERSION=main / DIGGORY_EDGE_INSTALL=true.
        log_error "Verification failed for ${binary_name}; aborting instead of falling back to an unverified build"
        log_error "Retry later, or opt into a source build explicitly: DIGGORY_VERSION=main ./install.sh (piping from curl: | bash -s latest)"
        return 1
    fi
    rm -f "$staged_path"
    if [[ -t 1 ]]; then stop_line_spinner; fi
...
install_files() {
    ...
    if [[ -f "$SOURCE_DIR/diggory" ]]; then
        if [[ "$source_dir_abs" != "$install_dir_abs" ]]; then
            ...
            if ! maybe_sudo cp "$SOURCE_DIR/diggory" "$INSTALL_DIR/diggory.new" ||
        ...
        log_error "diggory executable not found in ${SOURCE_DIR:-unknown}"
        exit 1
    fi
    ...
    if [[ "$source_dir_abs" != "$install_dir_abs" ]]; then
        ...
        if ! maybe_sudo /usr/bin/sed -i '' "s|SCRIPT_DIR=.*|SCRIPT_DIR=\"$CONFIG_DIR\"|" "$INSTALL_DIR/diggory"; then
            log_error "Failed to point $INSTALL_DIR/diggory at $CONFIG_DIR"
            return 1
        fi
    fi
...
verify_installation() {
    ...
    if [[ -x "$INSTALL_DIR/diggory" ]] && [[ -f "$CONFIG_DIR/lib/core/common.sh" ]]; then
        ...
        if run_install_probe_with_timeout 5 "$INSTALL_DIR/diggory" --help > /dev/null 2>&1; then
            return 0
        else
            log_error "Installed Diggory did not answer the bounded help probe"
            return 1
        fi
    else
...
setup_path() {
    ...
    if [[ "$INSTALL_DIR" != "/usr/local/bin" ]]; then
        ...
        echo ""
        echo "To use diggory from anywhere, add this line to your shell profile:"
...
print_usage_summary() {
    ...
    local message="Diggory ${action} successfully"
...
# Whether Homebrew owns a diggory install, decided from the Cellar on disk and
...
homebrew_owns_diggory() {
    local -a brew_prefixes=("${HOMEBREW_PREFIX:-}" /opt/homebrew /usr/local)
    # A custom prefix need not export HOMEBREW_PREFIX, and `brew list diggory`
    ...
    for brew_prefix in "${brew_prefixes[@]}"; do
        [[ -n "$brew_prefix" && -d "$brew_prefix/Cellar/diggory" ]] && return 0
    done
    return 1
}
...
perform_install() {
    ...
    if ! write_install_channel_metadata "$install_channel" "$commit_hash" "${DIGGORY_INSTALL_RECEIPT:-}"; then
...
perform_update() {
    ...
    if homebrew_owns_diggory; then
        ...
        if [[ -f "$SOURCE_DIR/lib/core/common.sh" ]]; then
            ...
            log_error "Cannot update Homebrew-managed Diggory without full installation"
            echo ""
            echo "Please update via Homebrew:"
            echo -e "  ${GREEN}brew upgrade diggory${NC}"
            exit 1
        fi
        exit 0
    fi
    ...
    if [[ -z "$installed_version" ]]; then
        log_warning "Diggory is not currently installed in $INSTALL_DIR. Running fresh installation."
        perform_install
        return
    fi
    ...
    if [[ -z "$target_version" ]]; then
        log_error "Unable to determine the latest Diggory version."
        exit 1
    fi
