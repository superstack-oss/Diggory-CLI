...
if [[ -n "${DIGGORY_HEALTH_JSON_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_HEALTH_JSON_LOADED=1

# Ensure dependencies are loaded (only if running standalone)
if [[ -z "${DIGGORY_FILE_OPS_LOADED:-}" ]]; then
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
    source "$SCRIPT_DIR/lib/core/file_ops.sh"
fi

_DIGGORY_HEALTH_JSON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
readonly _DIGGORY_HEALTH_JSON_DIR
source "$_DIGGORY_HEALTH_JSON_DIR/../optimize/catalog.sh"
...
EOF
    ...
    for ((index = 0; index < ${#DIGGORY_OPTIMIZE_ACTIONS[@]}; index++)); do
        ...
        desc=${DIGGORY_OPTIMIZE_DESCRIPTIONS[$index]}
        safe=${DIGGORY_OPTIMIZE_SAFE_VALUES[$index]}
