...
_xcode_app_cache_delete_guard_allows() {
    ...
    diggory_clean_process_guard _xcode_cleanup_process_state "Xcode or build tooling running"
}

_simulator_app_cache_delete_guard_allows() {
    diggory_clean_process_guard _simulator_cleanup_process_state "Simulator or CoreSimulator running"
}

_final_cut_pro_delete_guard_allows() {
    diggory_clean_process_guard final_cut_pro_is_running "Final Cut Pro started"
}

_defer_app_cache_guard_family() {
    case "$1" in
        _simulator_app_cache_delete_guard_allows) diggory_defer_cleanup_family "Simulator" ;;
        _final_cut_pro_delete_guard_allows) diggory_defer_cleanup_family "Final Cut Pro" ;;
        _autodesk_cache_delete_guard_allows) diggory_defer_cleanup_family "Autodesk" ;;
...
_app_cache_safe_clean_guarded() {
    ...
    if ! declare -f safe_clean_guarded > /dev/null 2>&1; then
        if ! "$delete_guard"; then
            diggory_report_guard_stop "$display_name" _defer_app_cache_guard_family "$delete_guard"
            return 1
        fi
        safe_clean "$@"
        return $?
...
clean_xcode_tools() {
    ...
    if diggory_cleanup_targets_exist "$HOME/Library/Developer/Xcode/Products"/* ||
...
clean_final_cut_pro_generated_caches() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            ...
            diggory_defer_cleanup_family "Final Cut Pro"
        fi
        return 0
    fi
...
_autodesk_cache_delete_guard_allows() {
    diggory_clean_process_guard autodesk_cache_process_state "Autodesk running"
}
...
clean_3d_tools() {
    ...
    for autodesk_entry in "$HOME"/Library/Caches/com.autodesk.*; do
        ...
        if diggory_cleanup_targets_exist "$autodesk_entry"/*; then
            autodesk_targets+=("$autodesk_entry"/*)
        elif diggory_cleanup_targets_exist "$autodesk_entry"; then
            autodesk_targets+=("$autodesk_entry")
        fi
    done
    if [[ ${#autodesk_targets[@]} -gt 0 ]]; then
        ...
        if [[ $process_state -ne 1 ]]; then
            if [[ $process_state -eq 2 ]]; then
                ...
                diggory_defer_cleanup_family "Autodesk"
            fi
        else
            _app_cache_safe_clean_guarded \
                _autodesk_cache_delete_guard_allows \
...
# directories whose seg.x0 file is older than DIGGORY_ORPHAN_AGE_DAYS are removed.
...
clean_neatdm_stale_segments() {
    ...
    for seg_dir in "$neatdm_dir"/*/; do
        ...
        if [[ $age_days -ge ${DIGGORY_ORPHAN_AGE_DAYS:-30} ]]; then
            stale_dirs+=("$seg_dir")
        fi
    done
