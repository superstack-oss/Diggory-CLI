...
readonly ORPHAN_AGE_THRESHOLD=${ORPHAN_AGE_THRESHOLD:-${DIGGORY_ORPHAN_AGE_DAYS:-30}}
readonly CLAUDE_VM_ORPHAN_AGE_THRESHOLD=${DIGGORY_CLAUDE_VM_ORPHAN_AGE_DAYS:-7}
readonly INSTALLED_APPS_CACHE_COMPLETE_MARKER="# diggory-installed-apps-cache:v3:complete"
...
clean_ds_store_tree() {
    ...
    local scan_rc=0
    run_with_timeout "$DIGGORY_TIMEOUT_HINT_SCAN_SEC" "${find_cmd[@]}" \
    ...
    while IFS= read -r -d '' ds_file; do
        ...
        if [[ $file_count -ge $DIGGORY_MAX_DS_STORE_FILES ]]; then
            break
        fi
    done < "$scan_file"
    rm -f -- "$scan_file" 2> /dev/null || true # SAFE: exact tracked temp file created above
...
scan_installed_apps() {
    ...
    (
        ...
        if [[ "${DIGGORY_TEST_MODE:-0}" != "1" && "${DIGGORY_TEST_NO_AUTH:-0}" != "1" ]]; then
            ...
            if command -v osascript > /dev/null 2>&1 &&
                running_apps=$(run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" osascript -e 'tell application "System Events" to get bundle identifier of every application process' 2> /dev/null); then
                printf '%s\n' "$running_apps" | tr ',' '\n' |
        ...
        if command -v lsappinfo > /dev/null 2>&1; then
            ...
            if lsappinfo_output=$(run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" lsappinfo list 2> /dev/null); then
                printf '%s\n' "$lsappinfo_output" |
    ...
    (
        ...
        if ! run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" find "${agent_dirs[@]}" \
    ...
    if [[ $app_scan_failed -ne 0 ]]; then
        ...
        if [[ -s "$scan_tmp_dir/scan_failures.list" ]]; then
            ...
            DIGGORY_APP_SCAN_FAILURE_DETAIL=$(diggory_terminal_safe_text "${raw_failure_detail##*/}")
        fi
...
_DIGGORY_MDFIND_FOUND=""
_DIGGORY_MDFIND_NOTFOUND=""

_mdfind_cache_check() {
    local bundle_id="$1"
    local _nl=$'\n'
    case "${_nl}${_DIGGORY_MDFIND_FOUND}${_nl}" in
        *"${_nl}${bundle_id}${_nl}"*) return 0 ;;
    esac
    case "${_nl}${_DIGGORY_MDFIND_NOTFOUND}${_nl}" in
...
_mdfind_cache_store() {
    ...
    local found="$2"
    if [[ "$found" == "true" ]]; then
        _DIGGORY_MDFIND_FOUND="${_DIGGORY_MDFIND_FOUND:+${_DIGGORY_MDFIND_FOUND}
}${bundle_id}"
    else
        _DIGGORY_MDFIND_NOTFOUND="${_DIGGORY_MDFIND_NOTFOUND:+${_DIGGORY_MDFIND_NOTFOUND}
}${bundle_id}"
    fi
}
...
is_bundle_orphaned() {
    ...
    if diggory_is_reverse_dns_bundle_id "$bundle_id"; then
        ...
        if [[ $_cache_rc -eq 0 ]]; then
            ...
            app_exists=$(run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" mdfind "kMDItemCFBundleIdentifier == '$bundle_id'" 2> /dev/null) || _mdfind_rc=$?
...
is_claude_vm_bundle_orphaned() {
    ...
    if [[ $_cache_rc -eq 0 ]]; then
        ...
        local app_exists _mdfind_rc=0
        app_exists=$(run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" mdfind "kMDItemCFBundleIdentifier == '$claude_bundle_id'" 2> /dev/null) || _mdfind_rc=$?
        if [[ $_mdfind_rc -eq 124 || $_mdfind_rc -ge 128 ]]; then
            return "$_mdfind_rc"
        elif [[ $_mdfind_rc -ne 0 ]]; then
            return 1
...
orphan_cleanup_candidate_snapshot() {
    ...
    local snapshot_parent="$_DIGGORY_PATH_SNAPSHOT_PARENT"
    local snapshot_parent_id="$_DIGGORY_PATH_SNAPSHOT_PARENT_ID"
    local snapshot_target_id="$_DIGGORY_PATH_SNAPSHOT_TARGET_ID"
    local identity=""
    local identity_rc=0
...
orphan_cleanup_candidate_still_eligible() {
    ...
    bundle_has_installed_app "${_ORPHAN_CLEANUP_BUNDLE_ID:-}" \
        "$((SECONDS + DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC))" || resolver_rc=$?
    ...
    _DIGGORY_SAFE_CLEAN_BOUND_PATH="$path"
    _DIGGORY_SAFE_CLEAN_EXPECTED_PARENT="$_ORPHAN_CANDIDATE_PARENT"
    _DIGGORY_SAFE_CLEAN_EXPECTED_PARENT_ID="$_ORPHAN_CANDIDATE_PARENT_ID"
    _DIGGORY_SAFE_CLEAN_EXPECTED_TARGET_ID="$_ORPHAN_CANDIDATE_TARGET_ID"
    return 0
}
...
clean_orphaned_app_data() {
    ...
    if [[ $scan_status -ne 0 || -z "$installed_bundles" ]]; then
        stop_section_spinner
        local scan_detail="${DIGGORY_APP_SCAN_FAILURE_DETAIL:-}"
    ...
    if [[ -d "$claude_support_dir" ]]; then
        ...
        run_with_timeout "$DIGGORY_TIMEOUT_HINT_SCAN_SEC" find "$claude_support_dir" \
        ...
        if [[ ${#file_patterns[@]} -gt 0 ]]; then
            ...
            for item_path in "${file_patterns[@]}"; do
                ...
                for match in "${matches[@]}"; do
                    ...
                    if [[ $iteration_count -gt $DIGGORY_MAX_ORPHAN_ITERATIONS ]]; then
                        break
                    fi
...
_privileged_helper_bundle_id_from_binary() {
    ...
    case "$binary" in
        /Library/PrivilegedHelperTools/*.bundle/Contents/MacOS/*)
            ...
            local plist_timeout="$DIGGORY_TIMEOUT_QUICK_DETECT_SEC"
            local plist_rc=0
            if [[ -n "$deadline_seconds" ]]; then
                plist_timeout=$(_diggory_timeout_with_deadline "$plist_timeout" \
...
clean_orphaned_system_services() {
    ...
    service_auth_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
        "$service_cleanup_deadline") || service_auth_rc=$?
    if [[ $service_auth_rc -eq 0 ]]; then
        _diggory_bounded_sudo "$service_auth_timeout" \
    ...
    _system_service_app_exists() {
        ...
        if diggory_is_reverse_dns_bundle_id "$bundle_id"; then
            ...
            if [[ $_cache_rc -eq 0 ]]; then
                ...
                mdfind_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" \
    ...
    _plist_program_value() {
        ...
        plist_probe_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" \
            "$service_cleanup_deadline") || plist_probe_rc=$?
        if [[ $plist_probe_rc -eq 0 ]]; then
            value=$(_diggory_bounded_sudo "$plist_probe_timeout" \
    ...
    _plist_is_orphaned() {
        ...
        if [[ -e "$binary" ]]; then
            ...
            binary_probe_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
                "$service_cleanup_deadline") || binary_probe_rc=$?
            if [[ $binary_probe_rc -eq 0 ]]; then
                _diggory_bounded_sudo "$binary_probe_timeout" \
            ...
            if [[ $binary_probe_rc -eq 0 ]]; then
                ...
                binary_auth_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
                    "$service_cleanup_deadline") || binary_auth_rc=$?
                if [[ $binary_auth_rc -eq 0 ]]; then
                    _diggory_bounded_sudo "$binary_auth_timeout" \
    ...
    _orphan_service_identity() {
        ...
        if [[ $identity_rc -eq 0 ]]; then
            identity=$(_diggory_bounded_sudo "$identity_timeout" \
    ...
    if [[ -d /Library/LaunchDaemons ]]; then
        ...
        launch_daemon_scan_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" \
            "$service_cleanup_deadline") || launch_daemon_scan_rc=$?
        if [[ $launch_daemon_scan_rc -eq 0 ]]; then
            _diggory_materialize_bounded_sudo_find "$service_scan_file" \
    ...
    if [[ $service_scan_status -eq 0 && -d /Library/LaunchAgents ]]; then
        ...
        launch_agent_scan_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" \
    ...
    if [[ $service_scan_status -eq 0 && -d /Library/PrivilegedHelperTools ]]; then
        ...
        helper_scan_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" \
    ...
    if [[ $orphaned_count -gt 0 ]]; then
        ...
        for ((orphan_index = 0; orphan_index < orphaned_count; orphan_index++)); do
            ...
            # DIGGORY_UNINSTALL_MODE is scoped to the call and never leaks to later
            # cleanup sections; SYSTEM_CRITICAL_BUNDLES stay protected. See #1082.
            if DIGGORY_UNINSTALL_MODE=1 should_protect_path "$orphan_file"; then
            ...
            if [[ "$DRY_RUN" == "true" ]]; then
                ...
                orphan_size_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
                    "$service_cleanup_deadline") || orphan_size_rc=$?
                if [[ $orphan_size_rc -eq 0 ]]; then
                    orphan_size_kb=$(_diggory_bounded_sudo "$orphan_size_timeout" \
                ...
                file_size_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
                    "$service_cleanup_deadline") || file_size_rc=$?
                if [[ $file_size_rc -eq 0 ]]; then
                    file_size_kb=$(_diggory_bounded_sudo "$file_size_timeout" \
                ...
                if [[ "$orphan_file" == *.plist ]]; then
                    ...
                    unload_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" \
                        "$service_cleanup_deadline") || unload_rc=$?
                    if [[ $unload_rc -eq 0 ]]; then
                        _diggory_bounded_sudo "$unload_timeout" \
                ...
                elif [[ $remove_rc -eq $DIGGORY_ERR_PROTECTED_PATH ]]; then
                    debug_log "Skipping protected orphaned service: $orphan_file"
                    skipped_protected_count=$((skipped_protected_count + 1))
...
# Whether a stub container's app is still installed anywhere Diggory scans.
...
_container_stub_app_exists() {
    ...
    if diggory_is_reverse_dns_bundle_id "$bundle_id"; then
        ...
        if [[ $_cache_rc -eq 0 ]]; then
            ...
            # live app's service/container as an orphan; do not cache.
            local app_found _mdfind_rc=0
            app_found=$(run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" mdfind "kMDItemCFBundleIdentifier == '$bundle_id'" 2> /dev/null) || _mdfind_rc=$?
            if [[ $_mdfind_rc -eq 124 || $_mdfind_rc -ge 128 ]]; then
                return "$_mdfind_rc"
            elif [[ $_mdfind_rc -ne 0 ]]; then
                return 0
...
clean_orphaned_container_stubs() {
    ...
    for pattern_entry in "${stub_patterns[@]}"; do
        ...
        for container_dir in "$containers_dir"/$bundle_glob; do
            ...
            sibling_entry=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" find \
            ...
            if [[ "$DRY_RUN" != "true" ]]; then
                ...
                if [[ $stub_remove_rc -eq 124 || $stub_remove_rc -ge 128 ]]; then
                    ...
                    removed_count=$((removed_count + 1))
                    log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "REMOVED" "$container_dir" "stub-container"
                else
                    debug_log "Failed to remove stub container: $container_dir"
                    failed_count=$((failed_count + 1))
                    log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$container_dir" "stub-container"
                fi
            else
                if declare -f record_dry_run_cleanup_target > /dev/null 2>&1; then
                    local stub_size_kb
                ...
                removed_count=$((removed_count + 1))
                log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "SKIPPED" "$container_dir" "dry-run stub-container"
            fi
        done
    done
