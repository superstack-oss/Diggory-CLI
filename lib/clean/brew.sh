...
# readlink -f (unavailable on the macOS versions Diggory supports).
...
snapshot_homebrew_active_links() {
    ...
    prefix=$(HOMEBREW_NO_ENV_HINTS=1 HOMEBREW_NO_AUTO_UPDATE=1 \
        run_with_timeout "$DIGGORY_TIMEOUT_PKG_LIST_SEC" brew --prefix 2> /dev/null) || return 0
    cellar=$(HOMEBREW_NO_ENV_HINTS=1 HOMEBREW_NO_AUTO_UPDATE=1 \
        run_with_timeout "$DIGGORY_TIMEOUT_PKG_LIST_SEC" brew --cellar 2> /dev/null) || return 0
...
clean_homebrew() {
    ...
    local cleanup_timeout="${DIGGORY_TIMEOUT_PKG_CLEANUP_SEC:-20}"
    local autoremove_preview_timeout="${DIGGORY_TIMEOUT_PKG_LIST_SEC:-10}"
    ...
    local brew_cache_file="${HOME}/.cache/diggory/brew_last_cleanup"
    ...
    if [[ -d ~/Library/Caches/Homebrew ]]; then
        brew_cache_size=$(run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" du -skP ~/Library/Caches/Homebrew 2> /dev/null | awk '{print $1}')
        local du_exit=$?
    ...
    if [[ "$skip_cleanup" == "false" ]]; then
        ...
        if [[ -t 1 ]]; then DIGGORY_SPINNER_PREFIX="  " start_inline_spinner "Homebrew cleanup..."; fi
