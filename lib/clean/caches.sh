...
check_tcc_permissions() {
    [[ -t 1 ]] || return 0
    local permission_flag="$HOME/.cache/diggory/permissions_granted"
    ...
    if [[ "$needs_permission_check" == "true" ]]; then
        ...
        read -r
        DIGGORY_SPINNER_PREFIX="" start_inline_spinner "Requesting permissions..."
...
clean_service_worker_cache() {
    ...
    while IFS= read -r cache_dir; do
        ...
        local size=0
        local _du_out
        if _du_out=$(run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" du -skP "$cache_dir" 2> /dev/null); then
    ...
    done < <(run_with_timeout "$DIGGORY_TIMEOUT_PKG_LIST_SEC" sh -c 'find "$1" -type d -depth 2 2>/dev/null || true' _ "$cache_path")
    if [[ $cleaned_size -gt 0 ]]; then
        ...
        if [[ "$spinner_was_running" == "true" ]]; then
            DIGGORY_SPINNER_PREFIX="  " start_inline_spinner "Scanning browser Service Worker caches..."
        fi
    fi
...
project_cache_has_indicators() {
    local dir="$1"
    local max_depth="${2:-5}"
    local indicator_timeout="${DIGGORY_PROJECT_CACHE_DISCOVERY_TIMEOUT:-2}"
    [[ -d "$dir" ]] || return 1
...
discover_project_cache_roots() {
    ...
    for root in "${DIGGORY_PURGE_DEFAULT_SEARCH_PATHS[@]}"; do
        [[ -d "$root" ]] && roots+=("$root")
    done

    while IFS= read -r root; do
        [[ -d "$root" ]] && roots+=("$root")
    done < <(diggory_purge_read_paths_config "$HOME/.config/diggory/purge_paths")
    ...
    for root in "${roots[@]}"; do
        local identity
        identity=$(diggory_path_identity "$root")
...
scan_project_cache_root() {
    local root="$1"
    local output_file="$2"
    local scan_timeout="${DIGGORY_PROJECT_CACHE_SCAN_TIMEOUT:-6}"
    [[ -d "$root" ]] || return 0
...
project_cache_group_root() {
    ...
    while [[ -n "$candidate" && "$candidate" != "/" ]]; do
        if diggory_purge_is_project_root "$candidate"; then
            printf '%s\n' "$candidate"
            return 0
        fi
