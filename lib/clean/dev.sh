...
clean_corepack_cache() {
    ...
    if command -v corepack > /dev/null 2>&1 && COREPACK_ENABLE_DOWNLOAD_PROMPT=0 run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" corepack --version > /dev/null 2>&1; then
        COREPACK_ENABLE_DOWNLOAD_PROMPT=0 clean_tool_cache "Corepack cache" "$corepack_home" run_with_timeout "$DIGGORY_TIMEOUT_PKG_CLEANUP_SEC" corepack cache clean
    else
...
clean_uv_cache() {
    ...
    if command -v uv > /dev/null 2>&1 && run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" uv --version > /dev/null 2>&1; then
        local detected_cache
        detected_cache=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" uv cache dir 2> /dev/null || true)
        if [[ -n "$detected_cache" && "$detected_cache" == /* ]]; then
            uv_cache_path="$detected_cache"
        fi
        clean_tool_cache "uv cache" "$uv_cache_path" run_with_timeout "$DIGGORY_TIMEOUT_PKG_CLEANUP_SEC" uv cache prune
    else
...
conda_cache_whitelisted() {
    ...
    for root in "$@"; do
        [[ -n "$root" ]] || continue
        if is_path_whitelisted "$root" 2> /dev/null || is_path_whitelisted "$root/.diggory-cache-guard" 2> /dev/null; then
            return 0
        fi
    done
    return 1
...
clean_conda_metadata_caches() {
    ...
    if command -v conda > /dev/null 2>&1 && run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" conda --version > /dev/null 2>&1; then
        clean_tool_cache "conda index/tarball/log caches" "$conda_cache_hint" \
            run_with_timeout "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" conda clean --yes --index-cache --tarballs --logfiles
        note_activity
        return 0
    fi
...
clean_pnpm_stores() {
    ...
    for pnpm_bin in "${pnpm_bins[@]}"; do
        ...
        if ! COREPACK_ENABLE_DOWNLOAD_PROMPT=0 run_with_timeout \
            "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" "$pnpm_bin" --version > /dev/null 2>&1; then
        ...
        store_path=$(COREPACK_ENABLE_DOWNLOAD_PROMPT=0 run_with_timeout \
            "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" "$pnpm_bin" store path 2> /dev/null) || store_path=""
        stop_section_spinner
        ...
        COREPACK_ENABLE_DOWNLOAD_PROMPT=0 clean_tool_cache "pnpm cache" "$store_path" \
            run_with_timeout "$DIGGORY_TIMEOUT_PKG_CLEANUP_SEC" \
...
clean_dev_npm() {
    ...
    if command -v npm > /dev/null 2>&1; then
        ...
        npm_cache_path=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" npm config get cache 2> /dev/null) || npm_cache_path=""
        stop_section_spinner
    ...
    if command -v bun > /dev/null 2>&1 && bun --version > /dev/null 2>&1; then
        ...
        bun_cache_path=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" bun pm cache 2> /dev/null) || bun_cache_path=""
        ...
        if [[ "$bun_protected" == "true" ]]; then
            ...
            if run_with_timeout "$DIGGORY_TIMEOUT_PKG_LIST_SEC" bun pm cache rm > /dev/null 2>&1; then
                bun_cache_cleaned=true
            fi
            if [[ -t 1 ]]; then
                stop_section_spinner
...
clean_dev_python() {
    ...
    if command -v pip3 > /dev/null 2>&1 && pip3 --version > /dev/null 2>&1; then
        ...
        pip_cache_path=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" pip3 cache dir 2> /dev/null) || pip_cache_path=""
...
get_mise_cache_path() {
    ...
    if command -v mise > /dev/null 2>&1; then
        ...
        mise_cache_path=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" mise cache path 2> /dev/null || echo "")
...
rust_cache_cleanup_state() {
    ...
    physical_now=$(rust_cache_root_physical_path \
        "$_DIGGORY_RUST_CARGO_HOME" "$_DIGGORY_RUST_CACHE_ROOT") || return 2
    [[ "$physical_now" == "$_DIGGORY_RUST_CACHE_PHYSICAL" ]] || return 2
    ...
    local guarded_path="${_DIGGORY_DEV_GUARDED_PATH:-}"
    if [[ -n "$guarded_path" ]]; then
        _diggory_snapshot_path_identity "$guarded_path" || return 2
        [[ "$_DIGGORY_PATH_SNAPSHOT_PARENT" == "$physical_now" ]] || return 2
        ...
        _DIGGORY_SAFE_CLEAN_BOUND_PATH="$guarded_path"
        _DIGGORY_SAFE_CLEAN_EXPECTED_PARENT="$_DIGGORY_PATH_SNAPSHOT_PARENT"
        _DIGGORY_SAFE_CLEAN_EXPECTED_PARENT_ID="$_DIGGORY_PATH_SNAPSHOT_PARENT_ID"
        _DIGGORY_SAFE_CLEAN_EXPECTED_TARGET_ID="$_DIGGORY_PATH_SNAPSHOT_TARGET_ID"
    fi
    return 1
}
...
clean_rust_dependency_cache_root() {
    ...
    local _DIGGORY_RUST_CARGO_HOME="$cargo_home"
    local _DIGGORY_RUST_CACHE_ROOT="$cache_root"
    local _DIGGORY_RUST_CACHE_PHYSICAL="$physical_root"
    local _DIGGORY_DEV_PROCESS_GUARD_UNKNOWN_REASON="process or cache path state unknown"
...
clean_dev_rust() {
    ...
    if diggory_cleanup_targets_exist \
        ...
        if [[ $rust_state -eq 0 ]]; then
            diggory_defer_cleanup_family "Rust"
...
clean_dev_docker() {
    ...
    if command -v orb > /dev/null 2>&1 || command -v orbctl > /dev/null 2>&1 || [[ -d "$HOME/.orbstack" || -n "$orb_data" ]]; then
        ...
        if [[ -n "$orb_data" ]]; then
            ...
            [[ $size_rc -eq 0 ]] || _diggory_record_clean_cancellation "$size_rc"
...
clean_xcode_documentation_cache() {
    local doc_cache_root="${DIGGORY_XCODE_DOCUMENTATION_CACHE_DIR:-/Library/Developer/Xcode/DocumentationCache}"
    ...
    if [[ $process_state -ne 1 ]]; then
        ...
        if [[ $process_state -eq 2 ]]; then
            ...
            diggory_defer_cleanup_family "Xcode"
        fi
        return 0
    fi
...
_coresimulator_cache_process_running() {
    ...
    while IFS='|' read -r match_mode match_pattern match_label; do
        ...
        if pgrep "$match_mode" "$match_pattern" > /dev/null 2>&1; then
            _DIGGORY_XCODE_PROCESS_MATCH="$match_label"
...
_coresimulator_booted_device_state() {
    if [[ "$_DIGGORY_SIMCTL_RESOLUTION_STATUS" != "ready" ]]; then
    ...
    booted_output=$(_run_simctl "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" list devices booted -j 2> /dev/null) || probe_status=$?
    ...
    if [[ "$booted_output" == *'"udid"'* ]]; then
        _DIGGORY_XCODE_PROCESS_MATCH="booted simulator"
...
_xcode_xctest_devices_process_running() {
    ...
    if [[ $probe_status -eq 0 ]]; then
        _DIGGORY_XCODE_PROCESS_MATCH="Xcode/build tooling"
        return 0
    fi
...
_xcode_delete_guard_allows() {
    diggory_clean_process_guard _xcode_xctest_devices_process_running "Xcode or build tools started"
}

_coresimulator_delete_guard_allows() {
    diggory_clean_process_guard _coresimulator_activity_state "CoreSimulator started"
}
...
_xctest_devices_delete_guard_allows() {
    diggory_clean_process_guard _xctest_devices_activity_state "Xcode, XCTest, or Simulator started"
}
...
_dev_process_delete_guard_allows() {
    ...
    local _DIGGORY_DEV_GUARDED_PATH="${1:-}"
    diggory_clean_process_guard \
        "$_DIGGORY_DEV_PROCESS_GUARD_PROBE" \
        "$_DIGGORY_DEV_PROCESS_GUARD_FAMILY started" \
        "${_DIGGORY_DEV_PROCESS_GUARD_UNKNOWN_REASON:-process state unknown}"
}
...
_dev_report_process_guard_stop() {
    ...
    if [[ "$reason" == "process state unknown" || "$reason" == "process or cache path state unknown" ]]; then
        ...
        diggory_defer_cleanup_family "$family"
    fi
}
...
_dev_safe_clean_process_guarded() {
    ...
    local display_name="$3"
    shift 3
    local _DIGGORY_DEV_PROCESS_GUARD_PROBE="$probe"
    local _DIGGORY_DEV_PROCESS_GUARD_FAMILY="$family"
    local _DIGGORY_CLEAN_GUARD_REASON="${family} started"

    if ! declare -f safe_clean_guarded > /dev/null 2>&1; then
        if ! _dev_process_delete_guard_allows; then
            _dev_report_process_guard_stop "$display_name" "$family" "$_DIGGORY_CLEAN_GUARD_REASON"
            return 1
        fi
        safe_clean "$@"
        return $?
...
_xcode_safe_clean_guarded() {
    ...
    shift 2
    local _DIGGORY_CLEAN_GUARD_REASON="process state changed"
    ...
    if ! declare -f safe_clean_guarded > /dev/null 2>&1; then
        if ! "$delete_guard"; then
            if [[ "$_DIGGORY_CLEAN_GUARD_REASON" == "process state unknown" ]]; then
                ...
                diggory_defer_cleanup_family "Simulator"
            else
                diggory_defer_cleanup_family "Xcode"
            fi
            return 1
...
clean_xcode_xctest_devices() {
    local xctest_devices_dir="${DIGGORY_XCODE_XCTEST_DEVICES_DIR:-$HOME/Library/Developer/XCTestDevices}"
...
clean_xcode_system_coresimulator_caches() {
    local cache_root="${DIGGORY_XCODE_SYSTEM_CORESIMULATOR_CACHE_DIR:-/Library/Developer/CoreSimulator/Caches}"
    [[ -d "$cache_root" ]] || return 0
...
clean_xcode_device_support() {
    local ds_dir="$1"
    local display_name="$2"
    local keep_count="${DIGGORY_XCODE_DEVICE_SUPPORT_KEEP:-2}"
    ...
    local scan_file=""
    scan_file=$(mktemp "${TMPDIR:-/tmp}/diggory-device-support.XXXXXX") || {
    ...
    for candidate in "$ds_dir"/*/Symbols/System/Library/Caches/*; do
        ...
        diggory_cleanup_targets_exist "$candidate" && inner_cache_targets+=("$candidate")
    done
    for candidate in "$ds_dir"/*.log; do
        diggory_cleanup_targets_exist "$candidate" && log_targets+=("$candidate")
    done
...
_sim_runtime_mount_points() {
    if [[ -n "${DIGGORY_XCODE_SIM_RUNTIME_MOUNT_POINTS:-}" ]]; then
        printf '%s\n' "$DIGGORY_XCODE_SIM_RUNTIME_MOUNT_POINTS"
        return 0
    fi
...
_sim_runtime_size_kb() {
    ...
    local size_kb=0
    if has_sudo_session; then
        size_kb=$(run_with_timeout "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" sudo -n du -skP "$target_path" 2> /dev/null | command awk 'NR==1 {print $1; exit}' || echo "0")
    else
        size_kb=$(run_with_timeout "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" du -skP "$target_path" 2> /dev/null | command awk 'NR==1 {print $1; exit}' || echo "0")
    fi
...
clean_xcode_simulator_runtime_volumes() {
    local volumes_root="${DIGGORY_XCODE_SIM_RUNTIME_VOLUMES_ROOT:-/Library/Developer/CoreSimulator/Volumes}"
    local cryptex_root="${DIGGORY_XCODE_SIM_RUNTIME_CRYPTEX_ROOT:-/Library/Developer/CoreSimulator/Cryptex}"
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        local dryrun_max_items="${DIGGORY_SIM_RUNTIME_DRYRUN_MAX_ITEMS:-20}"
...
_DIGGORY_SIMCTL_DEVELOPER_DIR=""
_DIGGORY_SIMCTL_RESOLUTION_STATUS="unavailable"
_DIGGORY_SIMCTL_XCODE_APP_ROOTS=(
    "/Applications"
    "$HOME/Applications"
)
...
_simctl_developer_dir_is_usable() {
    ...
    run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
...
_resolve_simctl_developer_dir() {
    ...
    if [[ -n "${DEVELOPER_DIR:-}" ]]; then
        if _simctl_developer_dir_is_usable "$DEVELOPER_DIR"; then
            _DIGGORY_SIMCTL_DEVELOPER_DIR="$DEVELOPER_DIR"
            _DIGGORY_SIMCTL_RESOLUTION_STATUS="ready"
            return 0
        fi

        _DIGGORY_SIMCTL_RESOLUTION_STATUS="explicit-invalid"
    ...
    case "$selected_developer_dir" in
        ...
        *)
            if _simctl_developer_dir_is_usable "$selected_developer_dir"; then
                _DIGGORY_SIMCTL_DEVELOPER_DIR="$selected_developer_dir"
                _DIGGORY_SIMCTL_RESOLUTION_STATUS="ready"
                return 0
            fi
    ...
    shopt -s nullglob
    for app_root in "${_DIGGORY_SIMCTL_XCODE_APP_ROOTS[@]}"; do
        [[ -d "$app_root" ]] || continue
    ...
    if [[ ${#candidates[@]} -eq 1 ]]; then
        _DIGGORY_SIMCTL_DEVELOPER_DIR="${candidates[0]}"
        _DIGGORY_SIMCTL_RESOLUTION_STATUS="ready"
    ...
    if [[ ${#candidates[@]} -gt 1 ]]; then
        _DIGGORY_SIMCTL_RESOLUTION_STATUS="ambiguous"
...
_run_simctl() {
    ...
    [[ "$_DIGGORY_SIMCTL_RESOLUTION_STATUS" == "ready" ]] || return 127
    run_with_timeout "$timeout_seconds" \
        env "DEVELOPER_DIR=$_DIGGORY_SIMCTL_DEVELOPER_DIR" xcrun simctl "$@"
}
...
clean_dev_mobile() {
    ...
    if command -v xcrun > /dev/null 2>&1; then
        ...
        if [[ "$_DIGGORY_SIMCTL_RESOLUTION_STATUS" == "ambiguous" ]]; then
            ...
            if ensure_diggory_temp_root && [[ -n "${DIGGORY_RESOLVED_TMPDIR:-}" ]]; then
                simctl_probe_stderr_file=$(mktemp "$DIGGORY_RESOLVED_TMPDIR/diggory-simctl-probe.XXXXXX" 2> /dev/null || true)
            fi
            ...
            if _run_simctl "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" list devices > /dev/null 2> "$simctl_probe_stderr_target"; then
            ...
            if [[ "$simctl_available" == "true" ]]; then
                ...
                unavailable_devices_output=$(_run_simctl "$DIGGORY_TIMEOUT_PKG_LIST_SEC" list devices unavailable 2> /dev/null) || unavailable_list_exit_code=$?
            ...
            if [[ "$simctl_available" == "true" ]]; then
                ...
                if [[ "$DRY_RUN" == "true" ]]; then
                    ...
                    if ((unavailable_before == 0)); then
                        ...
                        delete_output=$(_run_simctl "$DIGGORY_TIMEOUT_PKG_CLEANUP_SEC" delete unavailable 2>&1) || delete_exit_code=$?

                        if [[ $delete_exit_code -eq 0 ]]; then
                            stop_section_spinner
                            local recount_exit_code=0
                            unavailable_devices_output=$(_run_simctl "$DIGGORY_TIMEOUT_PKG_LIST_SEC" list devices unavailable 2> /dev/null) || recount_exit_code=$?
...
clean_dev_jvm() {
    ...
    if diggory_cleanup_targets_exist \
        ...
        if [[ $gradle_state -eq 0 ]]; then
            diggory_defer_cleanup_family "Gradle"
...
clean_dev_jetbrains_toolbox() {
    ...
    local keep_previous="${DIGGORY_JETBRAINS_TOOLBOX_KEEP:-1}"
...
_DIGGORY_VERSIONED_AGENT_CLEANUP_TARGETS=()
_DIGGORY_VERSIONED_AGENT_RETENTION_TARGETS=()
_DIGGORY_VERSIONED_AGENT_ACTIVE_PATH=""

_versioned_agent_scan_deadline() {
    local timeout_seconds="${1:-$DIGGORY_TIMEOUT_DISK_VERIFY_SEC}"
    [[ "$timeout_seconds" =~ ^[0-9]+(\.[0-9]+)?$ ]] || timeout_seconds="$DIGGORY_TIMEOUT_DISK_VERIFY_SEC"
...
_materialize_versioned_agent_entries() {
    ...
    local output_file="$2"
    local timeout_seconds="${3:-$DIGGORY_TIMEOUT_DISK_VERIFY_SEC}"
...
_plan_versioned_agent_cleanup_targets() {
    ...
    local scan_timeout=""
    local deadline_rc=0
    scan_timeout=$(_diggory_timeout_with_deadline \
        "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" "$scan_deadline") || deadline_rc=$?
    ...
    while IFS= read -r -d '' entry; do
        ...
        stat_timeout=$(_diggory_timeout_with_deadline \
            "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" "$scan_deadline") || stat_rc=$?
    ...
    for target in "${entries[@]}"; do
        ...
        _DIGGORY_VERSIONED_AGENT_RETENTION_TARGETS+=("$target")
        if diggory_cleanup_targets_exist "$target"; then
            _DIGGORY_VERSIONED_AGENT_CLEANUP_TARGETS+=("$target")
        fi
        idx=$((idx + 1))
    done
}
...
_resolve_versioned_agent_active_path() {
    ...
    while IFS= read -r -d '' entry; do
        ...
        case "$target/" in
            "$entry_resolved"/*)
                _DIGGORY_VERSIONED_AGENT_ACTIVE_PATH="$entry"
                found_active=true
                break
                ;;
        esac
...
_DIGGORY_VERSIONED_AGENT_GUARD_ROOT=""
_DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_SYMLINK=""
_DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_REQUIRED=false
_DIGGORY_VERSIONED_AGENT_GUARD_KEEP=1
...
_versioned_agent_delete_guard_allows() {
    ...
    if [[ -n "$_DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_SYMLINK" ]]; then
        _resolve_versioned_agent_active_path \
            "$_DIGGORY_VERSIONED_AGENT_GUARD_ROOT" \
            "$_DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_SYMLINK" || active_status=$?
        if [[ $active_status -eq 124 || $active_status -ge 128 ]]; then
            _DIGGORY_CLEAN_GUARD_REASON="inventory interrupted"
            return "$active_status"
        fi
        if [[ $active_status -eq 0 ]]; then
            active_path="$_DIGGORY_VERSIONED_AGENT_ACTIVE_PATH"
        elif [[ $active_status -eq 1 && "$_DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_REQUIRED" != "true" ]]; then
            : # This agent currently has no active launcher symlink to preserve.
        elif [[ $active_status -eq 1 ]]; then
            _DIGGORY_CLEAN_GUARD_REASON="active version changed"
            return 1
        else
            _DIGGORY_CLEAN_GUARD_REASON="active version unknown"
            return "$active_status"
        fi
    fi
    ...
    _plan_versioned_agent_cleanup_targets \
        "$_DIGGORY_VERSIONED_AGENT_GUARD_ROOT" \
        "$_DIGGORY_VERSIONED_AGENT_GUARD_KEEP" \
        "$active_path" || plan_rc=$?
    if [[ $plan_rc -ne 0 ]]; then
        _DIGGORY_CLEAN_GUARD_REASON="inventory unknown"
    ...
    if [[ -n "$_DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_SYMLINK" ]]; then
        ...
        _resolve_versioned_agent_active_path \
            "$_DIGGORY_VERSIONED_AGENT_GUARD_ROOT" \
            "$_DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_SYMLINK" || verified_active_status=$?
        if [[ $verified_active_status -eq 124 || $verified_active_status -ge 128 ]]; then
            _DIGGORY_CLEAN_GUARD_REASON="inventory interrupted"
            return "$verified_active_status"
        fi
        [[ $verified_active_status -ne 0 ]] || verified_active_path="$_DIGGORY_VERSIONED_AGENT_ACTIVE_PATH"
        if [[ $verified_active_status -ne 0 && $verified_active_status -ne 1 ]]; then
            _DIGGORY_CLEAN_GUARD_REASON="active version unknown"
            return "$verified_active_status"
        fi
        if [[ $verified_active_status -eq 1 && "$_DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_REQUIRED" == "true" ]]; then
            _DIGGORY_CLEAN_GUARD_REASON="active version changed"
            return 1
        fi
    ...
    if [[ ${#_DIGGORY_VERSIONED_AGENT_CLEANUP_TARGETS[@]} -gt 0 ]]; then
        for planned_target in "${_DIGGORY_VERSIONED_AGENT_CLEANUP_TARGETS[@]}"; do
            [[ "$planned_target" == "$target" ]] && return 0
        done
    fi

    _DIGGORY_CLEAN_GUARD_REASON="retention changed"
    return 1
}

_report_versioned_agent_guard_stop() {
    local label="$1"
    echo -e "  ${GRAY}${ICON_WARNING}${NC} ${label} · stopped (${_DIGGORY_CLEAN_GUARD_REASON})"
    note_activity
}
...
clean_versioned_agent_root() {
    ...
    [[ ${#_DIGGORY_VERSIONED_AGENT_RETENTION_TARGETS[@]} -gt 0 ]] || return 0

    _DIGGORY_VERSIONED_AGENT_GUARD_ROOT="$versions_root"
    _DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_SYMLINK="$active_symlink"
    _DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_REQUIRED=false
    [[ -n "$active_path" ]] && _DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_REQUIRED=true
    _DIGGORY_VERSIONED_AGENT_GUARD_KEEP="$keep_previous"
    _DIGGORY_CLEAN_GUARD_REASON="retention changed"
    ...
    local target
    for target in "${_DIGGORY_VERSIONED_AGENT_RETENTION_TARGETS[@]}"; do
        local guard_rc=0
...
count_versioned_agent_entries() {
    ...
    local scan_timeout="$DIGGORY_TIMEOUT_DISK_VERIFY_SEC"
    local scan_file=""
    scan_file=$(create_temp_file) || return 1
    local scan_rc=0
    _materialize_versioned_agent_entries \
...
_DIGGORY_CLAUDE_DESKTOP_GUARD_SUPPORT=""
_DIGGORY_CLAUDE_DESKTOP_GUARD_SDK_VERSION=""
_DIGGORY_CLAUDE_DESKTOP_GUARD_VERSIONS_ROOT=""
_DIGGORY_CLAUDE_DESKTOP_GUARD_CLI_ROOT=""
_DIGGORY_CLAUDE_DESKTOP_GUARD_VM_ROOT=""
_DIGGORY_CLAUDE_DESKTOP_GUARD_KEEP=1
_DIGGORY_CLEAN_GUARD_REASON=""

_claude_desktop_delete_guard_allows() {
    local target="${1:-}"
    diggory_clean_process_guard claude_desktop_running "Claude Desktop started" || return 1

    local current_sdk=""
    current_sdk=$(claude_desktop_sdk_version "$_DIGGORY_CLAUDE_DESKTOP_GUARD_SUPPORT" || true)
    if [[ -z "$current_sdk" || "$current_sdk" != "$_DIGGORY_CLAUDE_DESKTOP_GUARD_SDK_VERSION" ]]; then
        _DIGGORY_CLEAN_GUARD_REASON="active version changed"
        return 1
    fi
    ...
    for active_root in \
        "$_DIGGORY_CLAUDE_DESKTOP_GUARD_CLI_ROOT" \
        "$_DIGGORY_CLAUDE_DESKTOP_GUARD_VM_ROOT"; do
    ...
    _plan_versioned_agent_cleanup_targets \
        "$_DIGGORY_CLAUDE_DESKTOP_GUARD_VERSIONS_ROOT" \
        "$_DIGGORY_CLAUDE_DESKTOP_GUARD_KEEP" \
        "$_DIGGORY_CLAUDE_DESKTOP_GUARD_VERSIONS_ROOT/$current_sdk" || plan_rc=$?
    if [[ $plan_rc -ne 0 ]]; then
        _DIGGORY_CLEAN_GUARD_REASON="inventory unknown"
        [[ $plan_rc -eq 124 || $plan_rc -ge 128 ]] && return "$plan_rc"
        return 1
    ...
    local verified_sdk=""
    verified_sdk=$(claude_desktop_sdk_version "$_DIGGORY_CLAUDE_DESKTOP_GUARD_SUPPORT" || true)
...
_claude_desktop_safe_clean_guarded() {
    local display_name="$1"
    shift
    _DIGGORY_CLEAN_GUARD_REASON="process state changed"
    ...
    if ! declare -f safe_clean_guarded > /dev/null 2>&1; then
        ...
        for ((index = 0; index < cleanup_arg_count - 1; index++)); do
            ...
            if [[ $guard_rc -ne 0 ]]; then
                [[ $guard_rc -eq 124 || $guard_rc -ge 128 ]] && return "$guard_rc"
                if [[ "$_DIGGORY_CLEAN_GUARD_REASON" == "Claude Desktop started" ]]; then
                    diggory_defer_cleanup_family "Claude Desktop"
                else
                    echo -e "  ${GRAY}${ICON_WARNING}${NC} ${display_name} · stopped (${_DIGGORY_CLEAN_GUARD_REASON})"
                    note_activity
                fi
...
clean_claude_desktop_bundled_versions() {
    ...
    [[ -d "$claude_support/claude-code" ]] && _DIGGORY_CLAUDE_DESKTOP_GUARD_CLI_ROOT="$claude_support/claude-code"
    [[ -d "$claude_support/claude-code-vm" ]] && _DIGGORY_CLAUDE_DESKTOP_GUARD_VM_ROOT="$claude_support/claude-code-vm"
    ...
    for spec in "${desktop_specs[@]}"; do
        ...
        if [[ ${#_DIGGORY_VERSIONED_AGENT_RETENTION_TARGETS[@]} -gt 0 ]]; then
    ...
    if [[ "$has_cleanup_target" != "true" ]]; then
        for spec in "${desktop_specs[@]}"; do
            ...
            _plan_versioned_agent_cleanup_targets \
                "$versions_root" "$keep_previous" "$versions_root/$sdk_version" || plan_rc=$?
            [[ $plan_rc -eq 0 ]] || return "$plan_rc"
            [[ ${#_DIGGORY_VERSIONED_AGENT_RETENTION_TARGETS[@]} -gt 0 ]] || continue
            if declare -f safe_clean_guarded > /dev/null 2>&1; then
                safe_clean_guarded \
                    _deny_versioned_agent_delete \
                    "${_DIGGORY_VERSIONED_AGENT_RETENTION_TARGETS[@]}" \
                ...
                safe_clean "${_DIGGORY_VERSIONED_AGENT_RETENTION_TARGETS[@]}" "$label"
            fi
        done
        return 0
    fi
    ...
    for spec in "${desktop_specs[@]}"; do
        ...
        _DIGGORY_CLAUDE_DESKTOP_GUARD_SUPPORT="$claude_support"
        _DIGGORY_CLAUDE_DESKTOP_GUARD_SDK_VERSION="$sdk_version"
        _DIGGORY_CLAUDE_DESKTOP_GUARD_VERSIONS_ROOT="$versions_root"
        _DIGGORY_CLAUDE_DESKTOP_GUARD_KEEP="$keep_previous"
        local clean_rc=0
        _claude_desktop_safe_clean_guarded \
            "$label" \
            "${_DIGGORY_VERSIONED_AGENT_RETENTION_TARGETS[@]}" \
...
clean_dev_ai_agents() {
    local keep_previous="${DIGGORY_AI_AGENTS_KEEP:-1}"
    [[ "$keep_previous" =~ ^[0-9]+$ ]] || keep_previous=1
...
codex_runtime_process_state() {
    ...
    diggory_pgrep_any -x "codex" || cli_state=$?
...
_codex_desktop_cache_delete_guard_allows() {
    diggory_clean_process_guard codex_desktop_process_state "Codex started"
}
...
_codex_desktop_safe_clean_guarded() {
    ...
    if ! declare -f safe_clean_guarded > /dev/null 2>&1; then
        if ! _codex_desktop_cache_delete_guard_allows; then
            diggory_report_guard_stop "$display_name" diggory_defer_cleanup_family "Codex"
            return 1
        fi
        safe_clean "$@"
        return $?
...
clean_codex_desktop_caches() {
    ...
    for profile in "${profiles[@]}"; do
        for leaf in "${leaves[@]}"; do
            ...
            if physical_leaf=$(codex_desktop_cache_physical_path "$profile/$leaf"); then
                if diggory_cleanup_targets_exist "$physical_leaf"/*; then
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            ...
            diggory_defer_cleanup_family "Codex"
        fi
        return 0
    fi
...
codex_sparkle_staging_has_open_files() {
    ...
    if lsof_output=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" lsof -Fn +D "$staging_root" 2> "$lsof_error_file"); then
        [[ -n "$lsof_output" ]]
        return
    else
        lsof_rc=$?
...
_DIGGORY_CODEX_STAGING_ROOT=""
_DIGGORY_CODEX_STAGING_ENTRY=""
_DIGGORY_CLEAN_GUARD_REASON=""

_codex_staging_entry_is_still_stale() {
    local staging_root="$_DIGGORY_CODEX_STAGING_ROOT"
    local stale_entry="$_DIGGORY_CODEX_STAGING_ENTRY"
    ...
    if [[ "${_DIGGORY_CODEX_STAGING_MODE:-age}" == "superseded" ]]; then
        local installed_build="${_DIGGORY_CODEX_INSTALLED_BUILD:-}"
    ...
    while IFS= read -r -d '' stale_match; do
        ...
        [[ "$physical_after" == "$physical_before" ]] || return 1
        return 0
    done < <(command find -P "$stale_entry" -maxdepth 0 -type d -mtime +"$DIGGORY_ORPHAN_AGE_DAYS" -print0 2> /dev/null)
    return 1
}

_codex_staging_delete_guard_allows() {
    _DIGGORY_CLEAN_GUARD_REASON="staging entry changed"
    _codex_staging_entry_is_still_stale || return 1

    diggory_clean_process_guard codex_desktop_process_state "Codex started" || return 1

    diggory_clean_process_guard codex_sparkle_updater_running "Sparkle updater started" "updater state unknown" || return 1

    local open_file_state=0
    if codex_sparkle_staging_has_open_files "$_DIGGORY_CODEX_STAGING_ROOT"; then
        _DIGGORY_CLEAN_GUARD_REASON="staging files opened"
        return 1
    else
        open_file_state=$?
    fi
    if [[ $open_file_state -eq 2 ]]; then
        _DIGGORY_CLEAN_GUARD_REASON="open-file check unavailable"
        return 1
    fi
...
_codex_staging_safe_clean_guarded() {
    local staging_root="$1"
    local stale_entry="$2"
    _DIGGORY_CODEX_STAGING_ROOT="$staging_root"
    _DIGGORY_CODEX_STAGING_ENTRY="$stale_entry"
    _DIGGORY_CODEX_STAGING_MODE="${3:-age}"
...
_codex_app_build_version() {
    ...
    [[ -d "$candidate" ]] || return 1
    local cand_id=""
    cand_id=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
        /usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" \
        "$candidate/Contents/Info.plist" 2> /dev/null) || return 1
    [[ "$cand_id" == "com.openai.codex" ]] || return 1
    local build=""
    build=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
...
_codex_installed_build_version() {
    ...
    mdfind_out=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" mdfind \
...
clean_codex_desktop_staging() {
    ...
    _DIGGORY_CODEX_INSTALLED_BUILD="$installed_build"
    ...
    while IFS= read -r -d '' stale_entry; do
        ...
        if ! diggory_cleanup_targets_exist "$stale_entry"; then
            continue
        fi
        local staged_build=""
        ...
        if [[ -n "$(command find -P "$stale_entry" -maxdepth 0 -type d \
            -mtime +"$DIGGORY_ORPHAN_AGE_DAYS" 2> /dev/null)" ]]; then
            stale_entries+=("$stale_entry")
    ...
    for stale_entry in "${stale_entries[@]}"; do
        ...
        if [[ $guarded_rc -eq 75 ]]; then
            case "$_DIGGORY_CLEAN_GUARD_REASON" in
                "process state unknown" | "updater state unknown" | "open-file check unavailable")
                    echo -e "  ${GRAY}${ICON_WARNING}${NC} Codex Desktop update staging · stopped (${_DIGGORY_CLEAN_GUARD_REASON})"
                    note_activity
                    ;;
...
chrome_devtools_mcp_running() {
    diggory_pgrep_any -f "chrome-devtools-mcp"
}
...
_codex_runtime_delete_guard_allows() {
    diggory_clean_process_guard codex_runtime_process_state "Codex started" || return 1

    if is_codex_runtime_active "$_DIGGORY_CODEX_RUNTIME_GUARD_PATH" ||
        ! is_codex_runtime_stale "$_DIGGORY_CODEX_RUNTIME_GUARD_PATH"; then
        _DIGGORY_CLEAN_GUARD_REASON="runtime state changed"
        return 1
    fi
    return 0
}
...
_codex_runtime_safe_clean_guarded() {
    local runtime_dir="$1"
    local _DIGGORY_CODEX_RUNTIME_GUARD_PATH="$runtime_dir"
    local _DIGGORY_CLEAN_GUARD_REASON="Codex started"
    local guarded_rc=0
    ...
    if [[ $guarded_rc -eq 75 ]]; then
        if [[ "$_DIGGORY_CLEAN_GUARD_REASON" == "Codex started" ]]; then
            diggory_defer_cleanup_family "Codex"
        else
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Codex runtimes · stopped (${_DIGGORY_CLEAN_GUARD_REASON})"
            note_activity
        fi
        return 1
    fi
...
clean_codex_runtimes() {
    ...
    while IFS= read -r -d '' runtime_dir; do
        if ! is_codex_runtime_active "$runtime_dir" &&
            ...
            diggory_cleanup_targets_exist "$runtime_dir"; then
...
_DIGGORY_CODEX_MARKETPLACE_STAGING_ROOT=""
_DIGGORY_CODEX_MARKETPLACE_STAGING_ENTRY=""

_codex_marketplace_staging_entry_is_still_stale() {
    local staging_root="$_DIGGORY_CODEX_MARKETPLACE_STAGING_ROOT"
    local stale_entry="$_DIGGORY_CODEX_MARKETPLACE_STAGING_ENTRY"
    [[ -n "$staging_root" && -n "$stale_entry" ]] || return 1
    [[ "${stale_entry%/*}" == "$staging_root" ]] || return 1
    [[ -d "$stale_entry" && ! -L "$stale_entry" ]] || return 1
...
_codex_marketplace_staging_delete_guard_allows() {
    ...
    diggory_clean_process_guard codex_runtime_process_state "Codex started" || return 1
    if codex_sparkle_staging_has_open_files "$_DIGGORY_CODEX_MARKETPLACE_STAGING_ROOT"; then
        _DIGGORY_CLEAN_GUARD_REASON="open files"
        return 1
    else
...
_codex_marketplace_staging_safe_clean_guarded() {
    ...
    local stale_entry="$2"
    local display_name="$3"
    _DIGGORY_CODEX_MARKETPLACE_STAGING_ROOT="$staging_root"
    _DIGGORY_CODEX_MARKETPLACE_STAGING_ENTRY="$stale_entry"
    local _DIGGORY_CLEAN_GUARD_REASON="staging entry changed"
...
# only exact staging prefixes under fixed roots, aged by DIGGORY_ORPHAN_AGE_DAYS.
        ...
        if [[ $guarded_rc -eq 75 ]]; then
            case "${_DIGGORY_CLEAN_GUARD_REASON:-}" in
                "process state unknown" | "open-file check unavailable")
                    echo -e "  ${GRAY}${ICON_WARNING}${NC} Codex marketplace staging · stopped (${_DIGGORY_CLEAN_GUARD_REASON})"
                    note_activity
                    ;;
                "staging entry changed")
...
clean_antigravity_caches() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            ...
            diggory_defer_cleanup_family "Antigravity/Gemini"
        fi
        return 0
    fi
...
clean_chrome_devtools_mcp_caches() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            ...
            diggory_defer_cleanup_family "Chrome DevTools MCP"
        fi
        return 0
    fi
...
clean_dev_misc() {
    ...
    [[ -d "$HOME/.local/share/cursor-agent" ]] && safe_find_delete "$HOME/.local/share/cursor-agent" "*.log" "$DIGGORY_LOG_AGE_DAYS" "f"
...
_run_developer_cleanup_step() {
    ...
    if [[ $step_rc -eq 124 || $step_rc -ge 128 ]]; then
        _diggory_record_clean_cancellation "$step_rc"
        return "$step_rc"
    fi

    pending_clean_cancel="${DIGGORY_CLEAN_CANCEL_STATUS:-0}"
    if [[ $pending_clean_cancel -eq 124 || $pending_clean_cancel -ge 128 ]]; then
        return "$pending_clean_cancel"
    fi
