...
readonly PURGE_TARGETS=("${DIGGORY_PURGE_TARGETS[@]}")
...
readonly DEFAULT_PURGE_SEARCH_PATHS=("${DIGGORY_PURGE_DEFAULT_SEARCH_PATHS[@]}")

# Config file for custom purge paths.
readonly PURGE_CONFIG_FILE="$HOME/.config/diggory/purge_paths"
...
readonly MONOREPO_INDICATORS=("${DIGGORY_PURGE_MONOREPO_INDICATORS[@]}")
readonly PROJECT_INDICATORS=("${DIGGORY_PURGE_PROJECT_INDICATORS[@]}")
...
discover_project_dirs() {
    ...
    for path in "${DEFAULT_PURGE_SEARCH_PATHS[@]}"; do
        if [[ -d "$path" ]]; then
            ...
            discovered+=("$(diggory_purge_resolve_path_case "$path")")
        fi
    done
    ...
    for dir in "$HOME"/*/; do
        ...
        dir=$(diggory_purge_resolve_path_case "$dir")
...
write_purge_config() {
    ...
    local tmp_file
    tmp_file=$(mktemp_file "diggory-purge-paths") || return 1
...
load_purge_config() {
    ...
    done < <(diggory_purge_read_paths_config "$PURGE_CONFIG_FILE")
...
scan_purge_targets() {
    ...
    local stats_dir="${XDG_CACHE_HOME:-$HOME/.cache}/diggory"
    ...
    emit_valid_cachedir_tag_dirs() {
        while IFS= read -r tag_file; do
            ...
            if [[ -n "$cache_dir" ]] && diggory_dir_has_cachedir_tag "$cache_dir"; then
                printf '%s\n' "$cache_dir"
            fi
        done
    }
    ...
    if [[ "${MO_USE_FIND:-0}" == "1" ]]; then
        ...
        if run_with_timeout "$_scan_timeout" fd "${fd_args[@]}" "$pattern" "$search_path" 2> /dev/null > "$output_file.raw"; then
            run_with_timeout "$_scan_timeout" fd "${fd_tag_args[@]}" "^${DIGGORY_CACHEDIR_TAG_NAME}$" "$search_path" \
    ...
    if [[ "$use_find" == "true" ]]; then
        ...
        run_with_timeout "$_scan_timeout" find "$search_path" -mindepth "$cachedir_tag_min_depth" -maxdepth "$cachedir_tag_max_depth" \
            \( "${prune_expr[@]}" \) -prune -o \
            -type f -name "$DIGGORY_CACHEDIR_TAG_NAME" -print \
            2> /dev/null | emit_valid_cachedir_tag_dirs >> "$output_file.raw" || true
...
classify_purge_activity() {
    ...
    local probe_timeout="${MO_PURGE_ACTIVITY_TIMEOUT_SEC:-$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC}"
    if [[ ! "$probe_timeout" =~ ^[1-9][0-9]*$ ]]; then
        probe_timeout="$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC"
    fi
...
clean_project_artifacts() {
    ...
    local dedupe_output
    dedupe_output=$(mktemp_file "diggory-purge-dedupe") || return 1
    ...
    local _activity_total_timeout="${MO_PURGE_ACTIVITY_TOTAL_TIMEOUT_SEC:-$DIGGORY_TIMEOUT_HINT_SCAN_SEC}"
    if [[ ! "$_activity_total_timeout" =~ ^[1-9][0-9]*$ ]]; then
        _activity_total_timeout="$DIGGORY_TIMEOUT_HINT_SCAN_SEC"
    fi
    ...
    for item in "${safe_to_clean[@]}"; do
        ...
        if [[ "$size_raw" == "TIMEOUT" ]]; then
            ...
            size_kb="$size_raw"
            if [[ $size_kb -eq 0 && "${DIGGORY_PURGE_INCLUDE_EMPTY:-0}" != "1" ]]; then
                continue
            fi
        ...
        if diggory_purge_is_cloud_synced_path "$item"; then
            is_cloud=true
        fi
    ...
    if [[ -t 0 ]]; then
        ...
        for ((i = 0; i < ${#menu_options[@]}; i++)); do
            if [[ "${item_cloud_flags[i]:-false}" == "true" && "${DIGGORY_DRY_RUN:-0}" != "1" ]]; then
    ...
    local dry_run_mode="${DIGGORY_DRY_RUN:-0}"
    for idx in "${selected_indices[@]}"; do
