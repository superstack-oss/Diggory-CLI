...
gpu_cache_dir_is_stale() {
    local cache_dir="$1"
    local age_days="${2:-${DIGGORY_GPU_CACHE_AGE_DAYS:-1}}"
    ...
    local recent_file=""
    local probe_rc=0
    recent_file=$(run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" \
...
software_update_pending_or_unknown() {
    ...
    [[ -f "$plist" ]] || return 0
    local recommended=""
    local probe_timeout="$DIGGORY_TIMEOUT_QUICK_DETECT_SEC"
    if [[ -n "$deadline_seconds" ]]; then
        probe_timeout=$(_diggory_timeout_with_deadline "$probe_timeout" \
...
show_large_active_powerlog_notice() {
    ...
    local probe_rc=0
    size_bytes=$(_diggory_bounded_sudo "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
        -n "$STAT_BSD" -f%z "$DIGGORY_ACTIVE_POWERLOG_DB_PATH" < /dev/null 2> /dev/null) || probe_rc=$?
    if [[ $probe_rc -ge 128 ]]; then
        return "$probe_rc"
    fi
    ...
    echo -e "  ${YELLOW}${ICON_WARNING}${NC} Power telemetry database · ${GREEN}${size_human}${NC} · ${GRAY}active, kept · $(format_path_link "$DIGGORY_ACTIVE_POWERLOG_DB_PATH")${NC}"
}
...
macos_installer_process_is_idle() {
    ...
    process_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
        "$deadline_seconds") || return $?
    local process_rc=0
...
macos_installer_candidate_still_eligible() {
    ...
    if [[ -n "$current_macos_version" ]]; then
        ...
        version_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
            "$deadline_seconds") || return $?
...
clean_deep_system() {
    ...
    if [[ "$DIGGORY_LOG_AGE_DAYS" -eq "$DIGGORY_TEMP_FILE_AGE_DAYS" ]]; then
        cache_extra_patterns+=("*.log")
    fi
    local cache_rc=0
    safe_sudo_find_delete "/Library/Caches" "*.cache" "$DIGGORY_TEMP_FILE_AGE_DAYS" "f" "5" \
        "$system_cleanup_deadline" "${cache_extra_patterns[@]}" || cache_rc=$?
    if [[ $cache_rc -ge 128 ]]; then
        stop_section_spinner
        return "$cache_rc"
    fi
    if [[ $cache_rc -eq 0 && ${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-0} -gt 0 ]]; then
        cache_cleaned=1
    ...
    if [[ "$DIGGORY_LOG_AGE_DAYS" -ne "$DIGGORY_TEMP_FILE_AGE_DAYS" && $cache_rc -eq 0 ]] &&
        ! system_cleanup_budget_reached "$system_cleanup_deadline"; then
        cache_rc=0
        safe_sudo_find_delete "/Library/Caches" "*.log" "$DIGGORY_LOG_AGE_DAYS" "f" "5" \
            "$system_cleanup_deadline" || cache_rc=$?
        if [[ $cache_rc -ge 128 ]]; then
            stop_section_spinner
            return "$cache_rc"
        elif [[ $cache_rc -eq 0 && ${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-0} -gt 0 ]]; then
            cache_cleaned=1
    ...
    safe_sudo_find_delete "/Library/Logs/DiagnosticReports" "*" \
        "$DIGGORY_CRASH_REPORT_AGE_DAYS" "f" "1" "$system_cleanup_deadline" || crash_rc=$?
    if [[ $crash_rc -ge 128 ]]; then
        stop_section_spinner
        return "$crash_rc"
    fi
    local crash_cleaned=${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-0}
    stop_section_spinner
    ...
    safe_sudo_find_delete "/private/var/log" "*.log" "$DIGGORY_LOG_AGE_DAYS" \
        "f" "3" "$system_cleanup_deadline" "*.gz" "*.asl" || system_log_rc=$?
    if [[ $system_log_rc -ge 128 ]]; then
        stop_section_spinner
        return "$system_log_rc"
    fi
    if [[ $system_log_rc -eq 0 && ${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-0} -gt 0 ]]; then
    ...
    for third_party_log_dir in "${third_party_log_dirs[@]}"; do
        ...
        safe_sudo_find_delete "$third_party_log_dir" "*" "$DIGGORY_LOG_AGE_DAYS" "f" "5" \
            "$system_cleanup_deadline" || third_party_rc=$?
        if [[ $third_party_rc -ge 128 ]]; then
            stop_section_spinner
            return "$third_party_rc"
        fi
        if [[ $third_party_rc -eq 0 && ${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-0} -gt 0 ]]; then
    ...
    if [[ $third_party_logs_status -eq 124 ]] || system_cleanup_budget_reached "$system_cleanup_deadline"; then
        ...
        local adobegc_rc=0
        safe_sudo_find_delete "/Library/Logs" "adobegc.log" "$DIGGORY_LOG_AGE_DAYS" "f" "1" \
            "$system_cleanup_deadline" || adobegc_rc=$?
        if [[ $adobegc_rc -ge 128 ]]; then
            stop_section_spinner
            return "$adobegc_rc"
        fi
        if [[ $adobegc_rc -eq 0 && ${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-0} -gt 0 ]]; then
            third_party_logs_cleaned=1
    ...
    current_macos_version_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
    ...
    for installer_app in /Applications/Install\ macOS*.app; do
        ...
        if ! installer_size_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
    ...
    if code_sign_scan_file=$(create_temp_file 2> /dev/null); then
        ...
        code_sign_scan_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" \
    ...
    for rebuildable_cache_dir in "${rebuildable_cache_dirs[@]}"; do
        ...
        rebuildable_probe_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
            "$system_cleanup_deadline") || rebuildable_exists_rc=$?
        if [[ $rebuildable_exists_rc -eq 0 ]]; then
            _diggory_bounded_sudo "$rebuildable_probe_timeout" \
    ...
    if gpu_scan_file=$(create_temp_file 2> /dev/null); then
        ...
        gpu_scan_timeout=$(_diggory_timeout_with_deadline 8 "$system_cleanup_deadline") || gpu_scan_rc=$?
        ...
        if [[ $gpu_scan_rc -eq 0 ]]; then
            while IFS= read -r -d '' gpu_cache_dir; do
                ...
                gpu_cache_dir_is_stale "$gpu_cache_dir" "$DIGGORY_GPU_CACHE_AGE_DAYS" || gpu_stale_rc=$?
    ...
    if idle_scan_file=$(create_temp_file 2> /dev/null); then
        ...
        idle_scan_timeout=$(_diggory_timeout_with_deadline 8 "$system_cleanup_deadline") || idle_tmp_status=$?
        if [[ $idle_tmp_status -eq 0 ]]; then
            _diggory_materialize_bounded_sudo_find "$idle_scan_file" "$idle_scan_timeout" \
        ...
        if [[ $idle_tmp_status -eq 0 ]]; then
            while IFS= read -r -d '' idle_tmp_dir; do
                ...
                safe_sudo_find_delete "$idle_tmp_dir" "CFNetworkDownload_*.tmp" \
                    "$DIGGORY_TEMP_FILE_AGE_DAYS" "f" "5" "$system_cleanup_deadline" || idle_tmp_rc=$?
                if [[ $idle_tmp_rc -ge 128 ]]; then
                    idle_tmp_status=$idle_tmp_rc
                    break
                fi
                if [[ $idle_tmp_rc -eq 0 && ${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-0} -gt 0 ]]; then
    ...
    local diag_status=0
    local diag_rc=0
    safe_sudo_find_delete "$diag_base" "*" "$DIGGORY_LOG_AGE_DAYS" "f" "5" \
        "$system_cleanup_deadline" || diag_rc=$?
    if [[ $diag_rc -ge 128 ]]; then
        stop_section_spinner
        return "$diag_rc"
    fi
    if [[ $diag_rc -eq 0 && ${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-0} -gt 0 ]]; then
        diag_cleaned=1
    ...
    if system_cleanup_budget_reached "$system_cleanup_deadline"; then
        ...
        safe_sudo_find_delete "/private/var/db/DiagnosticPipeline" "*" \
            "$DIGGORY_LOG_AGE_DAYS" "f" "5" "$system_cleanup_deadline" || diag_rc=$?
        if [[ $diag_rc -ge 128 ]]; then
            stop_section_spinner
            return "$diag_rc"
        fi
    ...
    local power_rc=0
    safe_sudo_find_delete "/private/var/db/powerlog" "*" "$DIGGORY_LOG_AGE_DAYS" "f" "5" \
        "$system_cleanup_deadline" || power_rc=$?
    if [[ $power_rc -ge 128 ]]; then
        stop_section_spinner
        return "$power_rc"
    fi
    local power_cleaned=${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-0}
    stop_section_spinner
    ...
    if system_cleanup_budget_reached "$system_cleanup_deadline"; then
        ...
        local stats_timeout=""
        stats_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
            "$system_cleanup_deadline") || stats_rc=$?
        if [[ $stats_rc -eq 0 ]]; then
            stats_out=$(_diggory_bounded_sudo_find "$stats_timeout" \
    ...
    local mem_removed_count=${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-0}
...
# tmutil remains the owner of backup deletion, but Diggory must not hand it a path
...
clean_time_machine_failed_backups() {
    ...
    local tm_info=""
    local tm_info_rc=0
    tm_info=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" tmutil destinationinfo 2>&1) || tm_info_rc=$?
    ...
    for volume in "${backup_volumes[@]}"; do
        ...
        if [[ -d "$backupdb_dir" ]]; then
            ...
            backupdb_scan_timeout=$(_diggory_timeout_with_deadline 15 "$tm_scan_deadline") || backupdb_scan_rc=$?
            ...
            while IFS= read -r -d '' inprogress_file; do
                ...
                if [[ $hours_old -lt $DIGGORY_TM_BACKUP_SAFE_HOURS ]]; then
                    continue
                fi
                local size_kb
                local size_timeout=""
                ...
                time_machine_candidate_still_eligible "$inprogress_file" "$candidate_identity" \
                    "$DIGGORY_TM_BACKUP_SAFE_HOURS" "$tm_scan_deadline" || eligibility_rc=$?
                ...
                tm_delete_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
        ...
        for bundle in "$volume"/*.backupbundle "$volume"/*.sparsebundle; do
            ...
            local hdiutil_info=""
            local hdiutil_rc=0
            hdiutil_info=$(run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" hdiutil info 2> /dev/null) || hdiutil_rc=$?
            ...
            if [[ -n "$mounted_path" && -d "$mounted_path" ]]; then
                ...
                mounted_scan_timeout=$(_diggory_timeout_with_deadline 15 "$tm_scan_deadline") || mounted_scan_rc=$?
...
tm_is_running() {
    ...
    status_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
...
clean_local_snapshots() {
    ...
    local snapshot_list=""
    local snapshot_rc=0
    snapshot_list=$(run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" \
