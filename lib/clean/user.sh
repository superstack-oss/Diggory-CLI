...
_user_process_delete_guard_allows() {
    diggory_clean_process_guard "$_DIGGORY_USER_PROCESS_GUARD_PROBE" "$_DIGGORY_USER_PROCESS_GUARD_FAMILY started"
}
...
_user_safe_clean_process_guarded() {
    ...
    shift 3
    local _DIGGORY_USER_PROCESS_GUARD_PROBE="$probe"
    local _DIGGORY_USER_PROCESS_GUARD_FAMILY="$family"
    ...
    if ! declare -f safe_clean_guarded > /dev/null 2>&1; then
        if ! _user_process_delete_guard_allows; then
            diggory_report_guard_stop "$display_name" diggory_defer_cleanup_family "$family"
            return 1
        fi
        safe_clean "$@"
        return $?
...
clean_trash() {
    ...
    if [[ -t 1 ]]; then
        DIGGORY_SPINNER_PREFIX="  " start_inline_spinner "Emptying trash..."
    fi
...
clean_user_essentials() {
    ...
    if [[ "${DIGGORY_SKIP_TRASH_CLEANUP:-0}" != "1" ]]; then
        clean_trash
    fi
    stop_section_spinner
...
_clean_mail_downloads() {
    local mail_age_days=${DIGGORY_MAIL_AGE_DAYS:-}
    ...
    [[ "${DRY_RUN:-false}" == "true" || "${DIGGORY_DRY_RUN:-0}" == "1" ]] && dry_run_mode=true
    for target_path in "${mail_dirs[@]}"; do
        if [[ -d "$target_path" ]]; then
            ...
            local min_kb="${DIGGORY_MAIL_DOWNLOADS_MIN_KB:-}"
            if ! [[ "$min_kb" =~ ^[0-9]+$ ]]; then
                min_kb=5120
            fi
            ...
            while IFS= read -r -d '' file_path; do
                if [[ -f "$file_path" ]]; then
                    ...
                    if [[ "$dry_run_mode" == "true" ]]; then
                        ...
                        DIGGORY_DRY_RUN=1 safe_remove "$file_path" true "$file_size_kb" && remove_rc=0
    ...
    for app_path in "${app_paths[@]}"; do
        ...
        if [[ $process_state -ne 1 ]]; then
            if [[ $process_state -eq 2 ]]; then
                ...
                diggory_defer_cleanup_family "$label"
            fi
            return 0
        fi
...
is_microsoft_edge_running() {
    diggory_pgrep_any -x "Microsoft Edge"
}

is_brave_browser_running() {
    diggory_pgrep_any -x "Brave Browser"
}

_firefox_process_state() {
    diggory_pgrep_any -x "Firefox"
}

_dropbox_process_state() {
    diggory_pgrep_any -x "Dropbox"
}

_google_drive_process_state() {
    diggory_pgrep_any -x "Google Drive"
}

_onedrive_process_state() {
    diggory_pgrep_any -x "OneDrive"
}
...
clean_chrome_old_versions() {
    local -a app_paths
    if [[ -n "${DIGGORY_CHROME_APP_PATHS:-}" ]]; then
        IFS=':' read -ra app_paths <<< "$DIGGORY_CHROME_APP_PATHS"
    else
        app_paths=(
...
clean_edge_old_versions() {
    local -a app_paths
    if [[ -n "${DIGGORY_EDGE_APP_PATHS:-}" ]]; then
        IFS=':' read -ra app_paths <<< "$DIGGORY_EDGE_APP_PATHS"
    else
        app_paths=(
...
clean_edge_updater_old_versions() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            ...
            diggory_defer_cleanup_family "Edge"
        fi
        return 0
    fi
...
clean_brave_old_versions() {
    local -a app_paths
    if [[ -n "${DIGGORY_BRAVE_APP_PATHS:-}" ]]; then
        IFS=':' read -ra app_paths <<< "$DIGGORY_BRAVE_APP_PATHS"
    else
        app_paths=(
...
clean_support_app_data() {
    local support_age_days="${DIGGORY_SUPPORT_CACHE_AGE_DAYS:-30}"
...
clean_app_caches() {
    ...
    local precise_size_limit="${DIGGORY_CONTAINER_CACHE_PRECISE_SIZE_LIMIT:-64}"
...
external_volume_root() {
    printf '%s\n' "${DIGGORY_EXTERNAL_VOLUMES_ROOT:-/Volumes}"
}
...
validate_external_volume_target() {
    ...
    local disk_info=""
    disk_info=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" command diskutil info "$resolved" 2> /dev/null || echo "")
    if [[ -n "$disk_info" ]]; then
...
clean_external_volume_target() {
    ...
    local metadata_scan_timeout="${DIGGORY_EXTERNAL_VOLUME_SCAN_TIMEOUT:-15}"
...
clean_browsers() {
    ...
    if [[ $chrome_state -eq 1 ]]; then
        ...
        diggory_defer_cleanup_family "Chrome"
    ...
    diggory_cleanup_targets_exist "$HOME/Library/Caches/Firefox"/* && firefox_cache_targets=true
    diggory_cleanup_targets_exist "$HOME/Library/Application Support/Firefox/Profiles"/*/cache2/* && firefox_profile_cache_targets=true
    if [[ $firefox_state -eq 0 ]]; then
        if [[ "$firefox_cache_targets" == "true" || "$firefox_profile_cache_targets" == "true" ]]; then
            diggory_defer_cleanup_family "Firefox"
        fi
...
clean_cloud_storage() {
    ...
    if [[ $dropbox_state -eq 0 ]]; then
        if diggory_cleanup_targets_exist \
            ...
            diggory_defer_cleanup_family "Dropbox"
        fi
    elif [[ $dropbox_state -eq 1 ]]; then
        _clean_dropbox_caches_guarded || true
    elif diggory_cleanup_targets_exist \
        "$HOME/Library/Caches/com.getdropbox.dropbox" \
        "$HOME/Library/Caches"/com.dropbox.*; then
    ...
    if [[ $google_drive_state -eq 0 ]]; then
        if diggory_cleanup_targets_exist "$HOME/Library/Caches/com.google.GoogleDrive"; then
            diggory_defer_cleanup_family "Google Drive"
        fi
    ...
    elif diggory_cleanup_targets_exist "$HOME/Library/Caches/com.google.GoogleDrive"; then
    ...
    if [[ $onedrive_state -eq 0 ]]; then
        if diggory_cleanup_targets_exist "$HOME/Library/Caches/com.microsoft.OneDrive"; then
            diggory_defer_cleanup_family "OneDrive"
        fi
        ...
        _user_safe_clean_process_guarded \
            ...
            "OneDrive cache" || true
    elif diggory_cleanup_targets_exist "$HOME/Library/Caches/com.microsoft.OneDrive"; then
...
clean_tart_caches() {
    ...
    if [[ "${DRY_RUN:-false}" == "true" ]]; then
        echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} Tart caches · would prune items older than ${DIGGORY_ORPHAN_AGE_DAYS} days (${cache_size_human})"
        echo -e "    ${GRAY}tart prune --entries caches --older-than ${DIGGORY_ORPHAN_AGE_DAYS}${NC}"
        note_activity
        return 0
    fi
    ...
    if [[ $tart_state -ne 1 ]]; then
        ...
        return 0
    elif run_with_timeout "$DIGGORY_TIMEOUT_PKG_CLEANUP_SEC" tart prune --entries caches --older-than "$DIGGORY_ORPHAN_AGE_DAYS" > /dev/null 2>&1; then
    ...
    if [[ "$prune_succeeded" != "true" ]]; then
        ...
        debug_log "tart prune failed for cache-only ${DIGGORY_ORPHAN_AGE_DAYS}-day policy"
        note_activity
        return 0
    fi
...
clean_application_support_logs() {
    ...
    local cleaned_count=0
    local found_any=false
    local size_timeout_seconds="${DIGGORY_APP_SUPPORT_ITEM_SIZE_TIMEOUT_SEC:-0.4}"
...
report_agent_worktree_candidates() {
    ...
    for root in "${roots[@]}"; do
        ...
        while IFS= read -r -d '' container; do
            ...
            note_activity
        done < <(run_with_timeout "$DIGGORY_TIMEOUT_PKG_CLEANUP_SEC" command find "$root" -maxdepth 6 -type d -path "*/.claude/worktrees" -prune -print0 2> /dev/null)
    done
    return 0
}
...
check_large_file_candidates() {
    ...
    _large_candidate_size_kb() {
        local path="$1"
        local timeout_seconds="${2:-${DIGGORY_LARGE_CANDIDATE_SIZE_TIMEOUT:-3}}"
    ...
    _large_dir_newest_date() {
        local path="$1"
        local mtimes="" newest=""
        mtimes=$(run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" \
    ...
    if [[ "${SYSTEM_CLEAN:-false}" != "true" ]] && command -v tmutil > /dev/null 2>&1 &&
        ...
        snapshot_list=$(run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" tmutil listlocalsnapshots / 2> /dev/null || true)
    ...
    if command -v docker > /dev/null 2>&1; then
        local docker_output
        docker_output=$(run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" docker system df --format '{{.Type}}\t{{.Size}}\t{{.Reclaimable}}' 2> /dev/null || true)
        if [[ -n "$docker_output" ]]; then
            ...
            docker_output=$(run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" docker system df 2> /dev/null || true)
            if [[ -n "$docker_output" ]]; then
                stop_section_spinner
    ...
    _report_large_review_dir "iOS backups" "$HOME/Library/Application Support/MobileSync/Backup" "$DIGGORY_TIMEOUT_HINT_SCAN_SEC" "date"
