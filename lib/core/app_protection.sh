#!/bin/bash
# Diggory - Application Protection
# System critical and data-protected application lists

set -euo pipefail

if [[ -n "${DIGGORY_APP_PROTECTION_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_APP_PROTECTION_LOADED=1

_DIGGORY_CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
[[ -z "${DIGGORY_BASE_LOADED:-}" ]] && source "$_DIGGORY_CORE_DIR/base.sh"
...
source "$_DIGGORY_CORE_DIR/app_protection_data.sh"
...
# uninstaller instead of Diggory's generic Trash/delete path.
...
is_endpoint_security_cache_path() {
    ...
    case "$path" in
        ...
        *)
            if ! declare -f _diggory_path_is_within_existing_root > /dev/null 2>&1 ||
                ! _diggory_path_is_within_existing_root "$path" "/private/var/folders"; then
                return 1
            fi
            ;;
    esac
...
#
# In uninstall mode (DIGGORY_UNINSTALL_MODE=1), only system-critical components are protected.
...
should_protect_path() {
    ...
    if [[ "$path" =~ /Library/Containers/([^/]+) ]] || [[ "$path" =~ /Library/Group\ Containers/([^/]+) ]]; then
        ...
        elif [[ "${DIGGORY_UNINSTALL_MODE:-0}" != "1" ]] && should_protect_data "$bundle_id"; then
            return 0
        fi
    fi
    ...
    case "$path" in
        */Library/Preferences/com.apple.dock.plist | */Library/Preferences/com.apple.finder.plist)
            return 0
            ;;
        # Protect Diggory's own runtime logs so cleanup cannot delete its active log targets.
        */Library/Logs/diggory | */Library/Logs/diggory/ | */Library/Logs/diggory/*)
            return 0
            ;;
    ...
    if [[ "$_container_cache_path" != "true" && "$_known_rebuildable_cache_path" != "true" ]]; then
        if [[ "${DIGGORY_UNINSTALL_MODE:-0}" == "1" ]]; then
...
_diggory_uninstall_lower() {
    printf '%s' "${1:-}" | tr '[:upper:]' '[:lower:]'
}

_diggory_uninstall_is_common_app_name() {
    local lower_name
    lower_name=$(_diggory_uninstall_lower "${1:-}")
    case "$lower_name" in
...
_diggory_uninstall_vendor_product_tokens() {
    local bundle_id="${1:-}"
...
_diggory_uninstall_name_variant_matches() {
...
# discovery output.
_diggory_uninstall_materialize_find0() {
    local output_file="$1"
    shift

    : > "$output_file" || return 1
    local scan_timeout="$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC"
    if [[ -n "${_DIGGORY_UNINSTALL_DISCOVERY_DEADLINE:-}" ]]; then
        scan_timeout=$(_diggory_timeout_with_deadline "$scan_timeout" \
            "$_DIGGORY_UNINSTALL_DISCOVERY_DEADLINE") || return $?
    fi
    local scan_rc=0
...
find_vendor_nested_app_paths() {
    ...
    _diggory_uninstall_is_common_app_name "$app_name" && return 0

    local token_pair
    token_pair=$(_diggory_uninstall_vendor_product_tokens "$bundle_id" 2> /dev/null) || return 0
    ...
    vendor_lower=$(_diggory_uninstall_lower "$vendor_token")
    product_lower=$(_diggory_uninstall_lower "$product_token")
    app_lower=$(_diggory_uninstall_lower "$app_name")
    nospace_lower=$(_diggory_uninstall_lower "${app_name// /}")
    hyphen_lower=$(_diggory_uninstall_lower "${app_name// /-}")
    underscore_lower=$(_diggory_uninstall_lower "${app_name// /_}")
    ...
    for root in "$@"; do
        ...
        while IFS= read -r -d '' candidate; do
            ...
            parent_lower=$(_diggory_uninstall_lower "$parent_base")
            [[ "$parent_lower" == "$vendor_lower" ]] || continue

            child_base="${candidate##*/}"
            child_lower=$(_diggory_uninstall_lower "$child_base")
            if _diggory_uninstall_name_variant_matches "$child_lower" \
...
find_shared_app_paths() {
    ...
    local product_token=""
    local token_pair
    if token_pair=$(_diggory_uninstall_vendor_product_tokens "$bundle_id" 2> /dev/null); then
    ...
    for root in "$@"; do
        ...
        while IFS= read -r -d '' candidate; do
            base="${candidate##*/}"
            lower_base=$(_diggory_uninstall_lower "$base")
            if _diggory_uninstall_name_variant_matches "$lower_base" \
                "$app_lower" "$nospace_lower" "$hyphen_lower" "$underscore_lower" "$product_lower"; then
                matched_paths+=("$candidate")
            fi
        done < "$scan_file"
...
_diggory_uninstall_embedded_bundle_ids() {
    local app_path="${1:-}"
    ...
    scan_file=$(create_temp_file) || return 1
    local scan_rc=0
    _diggory_uninstall_materialize_find0 "$scan_file" "$app_path/Contents" \
    ...
    while IFS= read -r -d '' info; do
        ...
        diggory_is_reverse_dns_bundle_id "$embedded_id" || continue
...
find_app_files() {
    ...
    if [[ "$bundle_id_valid" == "true" && "$bundle_id" =~ ^dev\.zed\.Zed- ]] && [[ -d "$HOME/Library/HTTPStorages" ]]; then
        discovery_scan_rc=0
        _diggory_uninstall_materialize_find0 "$discovery_scan_file" \
    ...
    if [[ "$bundle_id_valid" == "true" ]]; then
        ...
        if [[ -d ~/Library/Preferences/ByHost ]]; then
            ...
            while IFS= read -r -d '' pref; do
                if diggory_name_starts_with_bundle_id_boundary "$pref" "$bundle_id"; then
                    files_to_clean+=("$pref")
                fi
            done < "$discovery_scan_file"
        fi
        ...
        if [[ -d ~/Library/Group\ Containers ]]; then
            ...
            while IFS= read -r -d '' container; do
                if diggory_name_has_bundle_id_boundary "$container" "$bundle_id"; then
        ...
        for derived_root in "${derived_bundle_roots[@]}"; do
            ...
            while IFS= read -r -d '' derived_path; do
                diggory_name_has_bundle_id_boundary "$derived_path" "$bundle_id" || continue
                already_added=false
    ...
    if [[ "$bundle_id_valid" == "true" && -n "$app_path" ]]; then
        ...
        discovery_scan_rc=0
        embedded_ids_output=$(_diggory_uninstall_embedded_bundle_ids \
    ...
    #
    # DIGGORY_UNINSTALL_SIBLING_SURVIVES=1 skips the whole heuristic section
    ...
    if [[ "${DIGGORY_UNINSTALL_SIBLING_SURVIVES:-0}" == "1" ]]; then
...
get_diagnostic_report_paths_for_app() {
    ...
    scan_file=$(create_temp_file) || return 1
    local scan_rc=0
    _diggory_uninstall_materialize_find0 "$scan_file" "$dir_abs" \
...
find_app_system_files() {
    ...
    if diggory_is_reverse_dns_bundle_id "$bundle_id"; then
        for base in /Library/LaunchAgents /Library/LaunchDaemons; do
            [[ -d "$base" ]] || continue
            system_scan_rc=0
            _diggory_uninstall_materialize_find0 "$system_scan_file" "$base" \
    ...
    if diggory_is_reverse_dns_bundle_id "$bundle_id"; then
        if [[ -d /Library/PrivilegedHelperTools ]]; then
            system_scan_rc=0
            _diggory_uninstall_materialize_find0 "$system_scan_file" \
            ...
            while IFS= read -r -d '' helper; do
                if diggory_name_starts_with_bundle_id_boundary "$helper" "$bundle_id"; then
                    system_files+=("$helper")
                fi
            done < "$system_scan_file"
        fi
        ...
        if [[ -d /private/var/db/receipts ]]; then
            ...
            while IFS= read -r -d '' receipt; do
                if diggory_name_starts_with_bundle_id_boundary "$receipt" "$bundle_id"; then
    ...
    if ! _diggory_uninstall_is_common_app_name "$app_name"; then
        local name_variant
    ...
    if [[ ${#helper_name_variants[@]} -gt 0 && -d /Library/PrivilegedHelperTools ]]; then
        ...
        while IFS= read -r -d '' helper; do
            ...
            helper_lower=$(_diggory_uninstall_lower "$helper_name")
            if _diggory_uninstall_name_variant_matches "$helper_lower" "${helper_name_variants[@]}"; then
                system_files+=("$helper")
            fi
        done < "$system_scan_file"
    fi
...
find_app_receipt_files() {
    ...
    if [[ -d /private/var/db/receipts ]]; then
        ...
        _diggory_uninstall_materialize_find0 "$receipt_scan_file" \
        ...
        while IFS= read -r -d '' bom; do
            if diggory_name_starts_with_bundle_id_boundary "$bom" "$bundle_id"; then
                bom_files+=("$bom")
            fi
    ...
    if [[ ${#bom_files[@]} -gt 0 ]]; then
        for bom_file in "${bom_files[@]}"; do
            ...
            local bom_rc=0
            local bom_timeout=""
            bom_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" \
                "$_DIGGORY_UNINSTALL_DISCOVERY_DEADLINE") || bom_rc=$?
            if [[ $bom_rc -eq 0 ]]; then
...
receipt_payload_path_is_allowlisted() {
    ...
    case "$clean_path" in
        ...
        /Library/PrivilegedHelperTools/*)
            diggory_name_starts_with_bundle_id_boundary "$base" "$bundle_id"
            return
            ;;
...
force_kill_app() {
    ...
    if [[ "${DIGGORY_TEST_MODE:-0}" != "1" && "${DIGGORY_TEST_NO_AUTH:-0}" != "1" ]] &&
        ...
        run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" osascript -e "tell application $quit_target to quit" > /dev/null 2>&1 < /dev/null &
        local quit_pid=$!
