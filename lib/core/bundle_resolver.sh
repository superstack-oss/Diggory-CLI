...
if [[ -n "${_DIGGORY_BUNDLE_RESOLVER_LOADED:-}" ]]; then
    return 0
fi
readonly _DIGGORY_BUNDLE_RESOLVER_LOADED=1

_DIGGORY_BUNDLE_RESOLVER_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -z "${DIGGORY_TIMEOUTS_LOADED:-}" ]]; then
    # shellcheck source=lib/core/timeouts.sh
    source "$_DIGGORY_BUNDLE_RESOLVER_DIR/timeouts.sh"
fi

# Standard locations for installed apps on macOS. Overridable from tests.
_DIGGORY_BUNDLE_RESOLVER_APP_ROOTS=(
    "/Applications"
    "/Applications/Setapp"
...
bundle_has_installed_app() {
    local bundle_id="$1"
    local deadline_seconds="${2:-$((SECONDS + DIGGORY_TIMEOUT_DISK_VERIFY_SEC))}"
    ...
    if command -v mdfind > /dev/null 2>&1; then
        ...
        if declare -f run_with_timeout > /dev/null 2>&1; then
            local mdfind_timeout="$DIGGORY_TIMEOUT_QUICK_DETECT_SEC"
            if [[ -n "$deadline_seconds" ]] && declare -f _diggory_timeout_with_deadline > /dev/null 2>&1; then
                mdfind_timeout=$(_diggory_timeout_with_deadline "$mdfind_timeout" "$deadline_seconds") || return 0
            fi
            local mdfind_rc=0
    ...
    for app_root in "${_DIGGORY_BUNDLE_RESOLVER_APP_ROOTS[@]}"; do
        [[ -d "$app_root" ]] || continue
        ...
        local app_scan_timeout="$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC"
        local app_scan_rc=0
        ...
        app_scan_timeout=$(_diggory_timeout_with_deadline "$app_scan_timeout" \
        ...
        while IFS= read -r -d '' app; do
            ...
            [[ -f "$info" ]] || continue
            local plist_timeout=""
            plist_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
                "$deadline_seconds") || {
                app_scan_found_or_unknown=true
                break
            }
