#!/bin/bash
# Diggory - Common Functions Library
# Main entry point that loads all core modules

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${DIGGORY_COMMON_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_COMMON_LOADED=1

_DIGGORY_CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Load core modules
source "$_DIGGORY_CORE_DIR/base.sh"
prepare_diggory_tmpdir > /dev/null
source "$_DIGGORY_CORE_DIR/log.sh"

source "$_DIGGORY_CORE_DIR/timeout.sh"
source "$_DIGGORY_CORE_DIR/timeouts.sh"
source "$_DIGGORY_CORE_DIR/file_ops.sh"
source "$_DIGGORY_CORE_DIR/help.sh"
source "$_DIGGORY_CORE_DIR/ui.sh"
source "$_DIGGORY_CORE_DIR/app_protection.sh"
source "$_DIGGORY_CORE_DIR/bundle_resolver.sh"
source "$_DIGGORY_CORE_DIR/pkg_receipts.sh"

# Load sudo management if available
if [[ -f "$_DIGGORY_CORE_DIR/sudo.sh" ]]; then
    source "$_DIGGORY_CORE_DIR/sudo.sh"
fi

# Normalize a path for comparisons while preserving root.
diggory_normalize_path() {
    local path="$1"
...
diggory_path_identity() {
    local path="$1"
    local normalized
    normalized=$(diggory_normalize_path "$path")
...
update_via_homebrew() {
    ...
    local brew_update_timeout="${DIGGORY_HOMEBREW_UPDATE_TIMEOUT:-120}"
    ...
    # Upgrade Diggory
    if [[ -t 1 ]]; then
        start_inline_spinner "Upgrading Diggory..."
    else
        echo "Upgrading Diggory..."
    fi

    local brew_upgrade_timeout="${DIGGORY_HOMEBREW_UPGRADE_TIMEOUT:-120}"
    local upgrade_status=0
    HOMEBREW_NO_ENV_HINTS=1 HOMEBREW_NO_AUTO_UPDATE=1 NONINTERACTIVE=1 \
        run_with_timeout "$brew_upgrade_timeout" brew upgrade diggory > "$temp_upgrade" 2>&1 || upgrade_status=$?
    ...
    if [[ "$upgrade_status" -ne 0 ]]; then
        ...
        if [[ -n "$upgrade_output" ]]; then
            ...
            printf 'brew upgrade diggory exited with status %s\n' "$upgrade_status" >&2
        fi
        return 1
    ...
    rm -f "$HOME/.cache/diggory/version_check" "$HOME/.cache/diggory/update_message" 2> /dev/null || true
}
