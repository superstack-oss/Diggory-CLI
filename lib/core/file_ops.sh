#!/bin/bash
# Diggory - File Operations
# Safe file and directory manipulation with validation

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${DIGGORY_FILE_OPS_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_FILE_OPS_LOADED=1

# Error codes for removal operations
readonly DIGGORY_ERR_SIP_PROTECTED=10
readonly DIGGORY_ERR_AUTH_FAILED=11
readonly DIGGORY_ERR_READONLY_FS=12
readonly DIGGORY_ERR_PROTECTED_PATH=13
readonly DIGGORY_ERR_PRIVACY_DENIED=14
readonly DIGGORY_ERR_MUTABLE_PARENT=15

# Ensure dependencies are loaded
_DIGGORY_CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -z "${DIGGORY_BASE_LOADED:-}" ]]; then
    # shellcheck source=lib/core/base.sh
    source "$_DIGGORY_CORE_DIR/base.sh"
fi
if [[ -z "${DIGGORY_LOG_LOADED:-}" ]]; then
    # shellcheck source=lib/core/log.sh
    source "$_DIGGORY_CORE_DIR/log.sh"
fi
if [[ -z "${DIGGORY_TIMEOUT_LOADED:-}" ]]; then
    # shellcheck source=lib/core/timeout.sh
    source "$_DIGGORY_CORE_DIR/timeout.sh"
fi
if [[ -z "${DIGGORY_TIMEOUTS_LOADED:-}" ]]; then
    # shellcheck source=lib/core/timeouts.sh
    source "$_DIGGORY_CORE_DIR/timeouts.sh"
fi
...
_diggory_bounded_sudo() {
    local duration="${1:-${DIGGORY_TIMEOUT_DISK_VERIFY_SEC:-30}}"
    shift || true
    [[ $# -gt 0 ]] || return 2
...
_diggory_bounded_sudo_until() {
    local deadline="$1"
    local requested="$2"
    shift 2
    local duration=""
    duration=$(_diggory_timeout_with_deadline "$requested" "$deadline") || return $?
    _diggory_bounded_sudo "$duration" "$@"
}
...
_diggory_normalize_deletion_policy_path() {
    local path="$1"
    local slash="/"
    local double_slash="//"
...
readonly DIGGORY_ACTIVE_POWERLOG_DB_PATH="/private/var/db/powerlog/Library/PerfPowerTelemetry/BackgroundProcessing/CurrentBackgroundProcessingDB.BGSQL"

_diggory_is_active_powerlog_database_path() {
    ...
    case "$1" in
        "$DIGGORY_ACTIVE_POWERLOG_DB_PATH" | \
            "$DIGGORY_ACTIVE_POWERLOG_DB_PATH-wal" | \
            "$DIGGORY_ACTIVE_POWERLOG_DB_PATH-shm")
            result=0
            ;;
    esac
...
_diggory_user_library_caches_prefix() {
    printf '%s\n' "${HOME%/}/Library/Caches"
}

_diggory_user_cache_owner_component() {
    local path="$1"
    local prefix
    prefix=$(_diggory_user_library_caches_prefix)
...
_diggory_is_sqlite_database_path() {
    local restore_nocasematch=false
    local result=1
    local path="$1"
    local base="${path%-wal}"
...
_diggory_is_user_cache_sqlite_family_path() {
    _diggory_is_sqlite_database_path "$1"
}

_diggory_user_cache_sqlite_main_path() {
    local path="$1"
    case "$path" in
        *-wal) printf '%s\n' "${path%-wal}" ;;
        *-shm) printf '%s\n' "${path%-shm}" ;;
...
_diggory_sqlite_family_base_path() {
    _diggory_user_cache_sqlite_main_path "$1"
}
...
# cache directory.
#
# Lines that must not vote are dropped here: Diggory's own process, and the
# measurement tools it forks over the very path being judged. `du -skPx
# ~/Library/Caches/<id>` puts <id> in the table purely because Diggory is looking
# at it, which would make every slowly-measured cache report its owner as live.
_DIGGORY_PROCESS_TABLE=""
_DIGGORY_PROCESS_TABLE_STATE=""

_diggory_process_table() {
    if [[ -n "${_DIGGORY_PROCESS_TABLE_STATE:-}" ]]; then
        [[ "$_DIGGORY_PROCESS_TABLE_STATE" == "ok" ]] || return 1
        printf '%s\n' "$_DIGGORY_PROCESS_TABLE"
        return 0
    fi
    ...
    if ! raw=$(ps -axo pid,ppid,comm,args 2> /dev/null) || [[ -z "$raw" ]]; then
        _DIGGORY_PROCESS_TABLE_STATE="unavailable"
        return 1
    fi
    ...
    _DIGGORY_PROCESS_TABLE="$filtered"
    _DIGGORY_PROCESS_TABLE_STATE="ok"
    printf '%s\n' "$_DIGGORY_PROCESS_TABLE"
    return 0
}
...
#      dataaccessd.
_diggory_user_cache_owner_process_state() {
    local owner="$1"
    [[ -n "$owner" ]] || return 2

    local cache_token="|${owner}:"
    case "${_DIGGORY_USER_CACHE_OWNER_STATE_CACHE:-}" in
    ...
    local table=""
    if ! table=$(_diggory_process_table); then
    ...
    if [[ $state -eq 1 && -n "$leaf" && "$leaf" != "$owner" && ${#leaf} -ge 4 ]]; then
        ...
        for component in "${components[@]}"; do
            if [[ $index -lt $last_index && ${#component} -ge 4 ]]; then
                case "$component" in
                    [cC][oO][mM]) ;;
                    *) corroborators+=("$(_diggory_regex_escape "$component")") ;;
                esac
            fi
            index=$((index + 1))
        done
        ...
        if [[ ${#corroborators[@]} -gt 0 ]]; then
            local escaped_leaf
            escaped_leaf=$(_diggory_regex_escape "$leaf")
            local alternation=""
    ...
    _DIGGORY_USER_CACHE_OWNER_STATE_CACHE="${_DIGGORY_USER_CACHE_OWNER_STATE_CACHE-}${cache_token}${state}|"
    return "$state"
}
...
_diggory_sqlite_database_in_use() {
    local path="$1"
    local base
    base=$(_diggory_sqlite_family_base_path "$path")
    ...
    for candidate in "$base" "${base}-wal" "${base}-shm"; do
        ...
        if declare -f run_with_timeout > /dev/null 2>&1; then
            run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" lsof -F n -- "$candidate" \
...
_diggory_user_cache_sqlite_has_open_handle() {
    local path="$1"
    local state=0
    _diggory_sqlite_database_in_use "$path" || state=$?
    return "$state"
}

# Return 0 when deletion must be refused (live owner or open SQLite handle).
_diggory_should_refuse_live_user_cache_path() {
    local path="$1"
    local owner=""
    owner=$(_diggory_user_cache_owner_component "$path") || return 1

    local process_state=0
    _diggory_user_cache_owner_process_state "$owner" || process_state=$?
    ...
    if _diggory_is_user_cache_sqlite_family_path "$path"; then
        local open_state=0
        _diggory_user_cache_sqlite_has_open_handle "$path" || open_state=$?
        ...
        for candidate in "$path/Cache.db" "$path"/*.db; do
            ...
            open_state=0
            _diggory_user_cache_sqlite_has_open_handle "$candidate" || open_state=$?
            if [[ $open_state -eq 0 ]]; then
...
_diggory_path_is_same_existing_file() {
    local path="$1"
    local protected_path="$2"
    [[ -e "$path" && -e "$protected_path" && "$path" -ef "$protected_path" ]]
}

_diggory_path_is_within_existing_root() {
    local path="$1"
    ...
    local probe="$path"
    while [[ "$probe" == /* ]]; do
        if _diggory_path_is_same_existing_file "$probe" "$protected_root"; then
            return 0
        fi
        [[ "$probe" == "/" ]] && break
        probe="${probe%/*}"
...
_diggory_is_critical_deletion_path() {
    local path="$1"
    ...
    for protected_root in "${exact_roots[@]}"; do
        if _diggory_path_is_same_existing_file "$path" "$protected_root"; then
            return 0
        fi
    done
    ...
    for protected_root in "${protected_trees[@]}"; do
        if _diggory_path_is_within_existing_root "$path" "$protected_root"; then
            return 0
        fi
    done
    ...
    if _diggory_path_is_same_existing_file "$parent_path" "/Users"; then
        return 0
    fi
...
validate_path_for_deletion() {
    ...
    local policy_path
    policy_path=$(_diggory_normalize_deletion_policy_path "$path")
    ...
    if [[ -L "$path" ]]; then
        ...
        if [[ -n "$resolved_target" ]]; then
            resolved_target=$(_diggory_normalize_deletion_policy_path "$resolved_target")
            if _diggory_is_critical_deletion_path "$resolved_target"; then
    ...
    if [[ "$ancestor_is_link" == "true" ]]; then
        ...
        if [[ -n "$resolved_parent" && "$resolved_parent" != "$parent_dir" ]]; then
            local resolved_path
            resolved_path=$(_diggory_normalize_deletion_policy_path "${resolved_parent}/${policy_path##*/}")
            if _diggory_is_critical_deletion_path "$resolved_path"; then
    ...
    if _diggory_is_active_powerlog_database_path "$policy_path"; then
    ...
    if _diggory_should_refuse_live_user_cache_path "$policy_path"; then
    ...
    if _diggory_is_sqlite_database_path "$policy_path"; then
        local sqlite_state=0
        _diggory_sqlite_database_in_use "$policy_path" || sqlite_state=$?
    ...
    if _diggory_is_critical_deletion_path "$policy_path"; then
...
_record_file_ops_dry_run_target() {
    ...
    if [[ -n "$precomputed_size_kb" && "$precomputed_size_kb" =~ ^[0-9]+$ ]]; then
        ...
        if [[ $measure_rc -eq 124 ]]; then
            ...
            DIGGORY_CLEAN_SIZING_TIMEOUTS=$((${DIGGORY_CLEAN_SIZING_TIMEOUTS:-0} + 1))
        fi
...
_diggory_record_clean_cancellation() {
    local status="$1"
    if [[ "${DIGGORY_CURRENT_COMMAND:-}" == "clean" &&
        ("$status" -eq 124 || "$status" -ge 128) ]]; then
        local existing="${DIGGORY_CLEAN_CANCEL_STATUS:-0}"
        if [[ $existing -ne 124 && $existing -lt 128 ]]; then
            DIGGORY_CLEAN_CANCEL_STATUS=$status
            export DIGGORY_CLEAN_CANCEL_STATUS
        fi
    fi
}
...
safe_remove() {
    ...
    local pending_clean_cancel="${DIGGORY_CLEAN_CANCEL_STATUS:-0}"
    if [[ "${DIGGORY_CURRENT_COMMAND:-}" == "clean" &&
    ...
    if declare -f is_path_whitelisted > /dev/null 2>&1 && is_path_whitelisted "$path"; then
        log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "SKIPPED" "$path" "whitelist"
        return 1
    fi
    ...
    if declare -f holds_compiled_model_cache > /dev/null 2>&1 && holds_compiled_model_cache "$path" 2> /dev/null; then
        ...
        log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "SKIPPED" "$path" "compiled model cache"
        return 1
    fi
    ...
    if _diggory_should_refuse_live_user_cache_path "$path"; then
        log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "SKIPPED" "$path" "live user cache"
        return 1
    fi
    ...
    if [[ "${DIGGORY_DRY_RUN:-0}" == "1" ]]; then
        ...
        if [[ $dry_record_rc -eq 124 || $dry_record_rc -ge 128 ]]; then
            _diggory_record_clean_cancellation "$dry_record_rc"
        ...
        if [[ "${MO_DEBUG:-}" == "1" ]]; then
            ...
            if [[ -e "$path" ]]; then
                ...
                if [[ $size_rc -eq 124 || $size_rc -ge 128 ]]; then
                    _diggory_record_clean_cancellation "$size_rc"
                    return "$size_rc"
                fi
                ...
                if [[ -f "$path" || -d "$path" ]] && ! [[ -L "$path" ]]; then
                    ...
                    if [[ $stat_rc -eq 124 || $stat_rc -ge 128 ]]; then
                        _diggory_record_clean_cancellation "$stat_rc"
                        return "$stat_rc"
                    fi
    ...
    if oplog_enabled; then
        if [[ -n "$precomputed_size_kb" ]]; then
            ...
            size_probe_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
            ...
            if [[ $size_probe_rc -ge 128 ]]; then
                _diggory_record_clean_cancellation "$size_probe_rc"
    ...
    if [[ -n "$expected_parent" ]] && ! _diggory_path_matches_identity \
        ...
        log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "SKIPPED" "$path" "identity changed"
        return 1
    fi
    ...
    if declare -F rm > /dev/null 2>&1; then
        ...
        local rm_timeout=""
        rm_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
    ...
    if [[ $rm_exit -eq 124 ]]; then
        ...
        if [[ $section_deadline_spent -eq 1 ]]; then
            ...
            log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "SKIPPED" "$path" "section time limit reached"
        else
            log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$path" "removal timed out"
            ...
            DIGGORY_CLEAN_REMOVAL_TIMEOUTS=$((${DIGGORY_CLEAN_REMOVAL_TIMEOUTS:-0} + 1))
        fi
        return 124
    fi
    ...
    if [[ $rm_exit -ge 128 ]]; then
        _diggory_record_clean_cancellation "$rm_exit"
        return "$rm_exit"
    fi

    if [[ $rm_exit -eq 0 ]]; then
        # Log successful removal
        log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "REMOVED" "$path" "$size_human"
        return 0
    else
        # Check if it's a permission error
        if [[ "$error_msg" == *"Permission denied"* ]] || [[ "$error_msg" == *"Operation not permitted"* ]]; then
            DIGGORY_PERMISSION_DENIED_COUNT=${DIGGORY_PERMISSION_DENIED_COUNT:-0}
            DIGGORY_PERMISSION_DENIED_COUNT=$((DIGGORY_PERMISSION_DENIED_COUNT + 1))
            export DIGGORY_PERMISSION_DENIED_COUNT
            debug_log "Permission denied: $path, may need Full Disk Access"
            log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$path" "permission denied"
        else
            [[ "$silent" != "true" ]] && log_error "Failed to remove: $path"
            log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$path" "error"
        fi
        return 1
    fi
}
...
safe_remove_symlink() {
    ...
    if [[ "$use_sudo" == "true" ]] && _diggory_privileged_path_has_mutable_ancestor "$path"; then
        if [[ ${EUID:-0} -ne 0 ]]; then
            use_sudo=false
        else
    ...
    if [[ "$use_sudo" == "true" ]]; then
        if [[ "${DIGGORY_TEST_MODE:-0}" == "1" || "${DIGGORY_TEST_NO_AUTH:-0}" == "1" ]]; then
            log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$path" "sudo blocked in test mode"
            return 1
        fi
    ...
    if [[ $rm_exit -eq 0 ]]; then
        log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "REMOVED" "$path" "symlink"
        return 0
    else
        ...
        log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$path" "symlink removal failed"
        return 1
    fi
}
...
_diggory_privileged_path_has_mutable_ancestor() {
    local path="$1"
    local probe="${path%/*}"
    local invoking_uid=""
    [[ -n "$probe" ]] || probe="/"
    ...
    while true; do
        ...
        if [[ "$invoking_uid" -ne 0 ]]; then
            if [[ ${EUID:-0} -eq "$invoking_uid" ]]; then
                ...
                _diggory_bounded_sudo "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
...
safe_sudo_remove() {
    ...
    if ! validate_path_for_deletion "$path"; then
        if declare -f should_protect_path > /dev/null 2>&1 && should_protect_path "$path"; then
            ...
            return "$DIGGORY_ERR_PROTECTED_PATH"
        else
    ...
    if _diggory_privileged_path_has_mutable_ancestor "$path"; then
        if [[ ${EUID:-0} -ne 0 ]]; then
    ...
    if [[ "${DIGGORY_DRY_RUN:-0}" == "1" ]]; then
        if [[ "${MO_DEBUG:-}" == "1" ]]; then
            ...
            if [[ $exists_rc -eq 124 || $exists_rc -ge 128 ]]; then
                _diggory_record_clean_cancellation "$exists_rc"
                return "$exists_rc"
            fi
            if [[ $exists_rc -eq 0 ]]; then
                ...
                if [[ -n "$precomputed_size_kb" ]]; then
                    ...
                    local size_rc=0
                    size_kb=$(_diggory_bounded_sudo "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
                ...
                if [[ $type_probe_rc -eq 124 || $type_probe_rc -ge 128 ]]; then
                    _diggory_record_clean_cancellation "$type_probe_rc"
                ...
                if [[ $type_probe_rc -eq 0 ]]; then
                    local mod_time=0
                    local stat_rc=0
                    mod_time=$(_diggory_bounded_sudo "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
    ...
    if oplog_enabled; then
        if [[ -n "$precomputed_size_kb" ]]; then
            ...
            exists_probe_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
                "$deadline_seconds") || exists_probe_rc=$?
            if [[ $exists_probe_rc -eq 0 ]]; then
                _diggory_bounded_sudo "$exists_probe_timeout" \
            ...
            if [[ $exists_probe_rc -ge 128 ]]; then
                _diggory_record_clean_cancellation "$exists_probe_rc"
            ...
            if [[ $exists_probe_rc -eq 0 ]]; then
                ...
                size_probe_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
                    "$deadline_seconds") || size_probe_rc=$?
                if [[ $size_probe_rc -eq 0 ]]; then
                    size_kb=$(_diggory_bounded_sudo "$size_probe_timeout" \
    ...
    local section_deadline_spent=0
    remove_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
        "$deadline_seconds") || ret=$?
    if [[ $ret -eq 0 ]]; then
        output=$(_diggory_bounded_sudo "$remove_timeout" \
    ...
    if [[ $ret -ge 128 ]]; then
        _diggory_record_clean_cancellation "$ret"
        return "$ret"
    fi

    case "$output" in
        *"a password is required"* | *"a terminal is required"* | *"Password:"*)
            log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$path" "auth required"
            return "$DIGGORY_ERR_AUTH_FAILED"
            ;;
        *"Operation not permitted"*)
            log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$path" "sip/mdm protected"
            return "$DIGGORY_ERR_SIP_PROTECTED"
            ;;
        *"Read-only file system"*)
            log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$path" "readonly filesystem"
            return "$DIGGORY_ERR_READONLY_FS"
            ;;
        *"Sorry, try again"* | *"incorrect passphrase"* | *"incorrect credentials"*)
            log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$path" "auth failed"
            return "$DIGGORY_ERR_AUTH_FAILED"
            ;;
        *)
            log_error "Failed to remove, sudo: $path"
            log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$path" "sudo error"
            return 1
            ;;
    esac
}
...
#
# Usage: diggory_delete <path> [needs_sudo=false] [expected_dev_inode_mtime]
#
# Environment:
#   DIGGORY_DELETE_MODE      "permanent" (default) or "trash"; other values fail
#   DIGGORY_DRY_RUN=1        Log intent, do not delete
#   DIGGORY_TEST_TRASH_DIR   Test-only override; Trash moves go here via `mv`
#                         instead of Finder/trash CLI. Required for bats.
#   DIGGORY_DELETE_LOG       Override the log file path (default:
#                         ~/Library/Logs/diggory/deletions.log)
#
# Returns 0 on success and a nonzero DIGGORY_ERR_* code on failure. Always appends a tab-separated line to
...
diggory_delete() {
    ...
    local mode="${DIGGORY_DELETE_MODE:-permanent}"
    ...
    case "$mode" in
        permanent | trash) ;;
        *)
            _diggory_delete_log "$mode" "unknown" "invalid-mode" "$path"
            if [[ -z "${_DIGGORY_INVALID_MODE_WARNED:-}" ]]; then
                _DIGGORY_INVALID_MODE_WARNED=1
                export _DIGGORY_INVALID_MODE_WARNED
                printf 'Error: invalid DIGGORY_DELETE_MODE: %s (expected "permanent" or "trash")\n' "$mode" >&2
            fi
            return 1
            ;;
    esac
    ...
    if ! validate_path_for_deletion "$path"; then
        _diggory_delete_log "$mode" "0" "rejected" "$path"
        return 1
    fi

    if [[ "$needs_sudo" == "true" ]] && _diggory_privileged_path_has_mutable_ancestor "$path"; then
        ...
        _diggory_delete_log "$mode" "unknown" "mutable-parent" "$path"
        debug_log "Refusing privileged delete below mutable parent: $path"
        return "$DIGGORY_ERR_MUTABLE_PARENT"
    fi
    ...
    if [[ -e "$path" ]]; then
        ...
        if [[ "$needs_sudo" == "true" ]]; then
            if [[ "${DIGGORY_TEST_MODE:-0}" == "1" || "${DIGGORY_TEST_NO_AUTH:-0}" == "1" ]]; then
                ...
                raw_size=$(run_with_timeout "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
        ...
        if [[ $du_rc -eq 124 || $du_rc -ge 128 ]]; then
            ...
            _diggory_delete_log "$mode" "$size_kb" "$cancel_status" "$path"
            return "$du_rc"
        fi
    fi
    ...
    if [[ -n "$expected_identity" ]]; then
        if ! _diggory_snapshot_path_identity "$path"; then
            _diggory_delete_log "$mode" "$size_kb" "identity-changed" "$path"
            return 1
        fi
        expected_parent="$_DIGGORY_PATH_SNAPSHOT_PARENT"
        expected_parent_id="$_DIGGORY_PATH_SNAPSHOT_PARENT_ID"
        expected_target_id="$_DIGGORY_PATH_SNAPSHOT_TARGET_ID"
        local current_identity=""
        local identity_rc=0
        current_identity=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
        ...
        if [[ $identity_rc -eq 124 || $identity_rc -ge 128 ]]; then
            ...
            _diggory_delete_log "$mode" "$size_kb" "$identity_status" "$path"
            return "$identity_rc"
        fi
    ...
    if [[ "${DIGGORY_DRY_RUN:-0}" == "1" ]]; then
        ...
        if [[ $preview_rc -eq 124 || $preview_rc -ge 128 ]]; then
            ...
            _diggory_delete_log "$mode" "$size_kb" "$preview_status" "$path"
            return "$preview_rc"
        fi
        debug_log "[DRY RUN] Would delete ($mode): $path"
        _diggory_delete_log "$mode" "$size_kb" "dry-run" "$path"
        return 0
    fi

    if [[ "$needs_sudo" == "true" ]]; then
        if [[ "${DIGGORY_TEST_MODE:-0}" == "1" || "${DIGGORY_TEST_NO_AUTH:-0}" == "1" ]]; then
            _diggory_delete_log "$mode" "$size_kb" "sudo-blocked-test-mode" "$path"
            return 1
        fi
    fi
    ...
    if [[ "$mode" == "trash" ]]; then
        local trash_rc=0
        _diggory_move_to_trash "$path" "$needs_sudo" \
            "$expected_parent" "$expected_parent_id" \
            "$expected_target_id" || trash_rc=$?
        if [[ $trash_rc -eq 0 ]]; then
            _diggory_delete_log "trash" "$size_kb" "ok" "$path"
            log_operation "${DIGGORY_CURRENT_COMMAND:-uninstall}" "TRASHED" "$path" "${size_kb}KB"
            return 0
        fi
        if [[ $trash_rc -eq $DIGGORY_ERR_PRIVACY_DENIED ]]; then
            _diggory_delete_log "trash" "$size_kb" "privacy-denied" "$path"
            log_operation "${DIGGORY_CURRENT_COMMAND:-uninstall}" "SKIPPED" "$path" "privacy permission denied"
            if [[ -z "${_DIGGORY_PRIVACY_DENIED_WARNED:-}" ]]; then
                _DIGGORY_PRIVACY_DENIED_WARNED=1
                export _DIGGORY_PRIVACY_DENIED_WARNED
            ...
            return "$DIGGORY_ERR_PRIVACY_DENIED"
        fi
        if [[ $trash_rc -eq $DIGGORY_ERR_MUTABLE_PARENT ]]; then
            _diggory_delete_log "trash" "$size_kb" "mutable-parent" "$path"
            log_operation "${DIGGORY_CURRENT_COMMAND:-uninstall}" "SKIPPED" "$path" "mutable-parent"
        ...
        if [[ $trash_rc -eq 124 || $trash_rc -ge 128 ]]; then
            ...
            _diggory_delete_log "trash" "$size_kb" "$trash_status" "$path"
            return "$trash_rc"
        fi
        _diggory_delete_log "trash" "$size_kb" "trash-failed" "$path"
        log_operation "${DIGGORY_CURRENT_COMMAND:-uninstall}" "SKIPPED" "$path" "trash-failed"
        if [[ -z "${_DIGGORY_TRASH_UNAVAILABLE_WARNED:-}" ]]; then
            _DIGGORY_TRASH_UNAVAILABLE_WARNED=1
            export _DIGGORY_TRASH_UNAVAILABLE_WARNED
    ...
    if [[ $rc -eq $DIGGORY_ERR_MUTABLE_PARENT ]]; then
    ...
    _diggory_delete_log "$mode" "$size_kb" "$status_label" "$path"
    return "$rc"
}

_diggory_valid_invoking_home() {
    local user_home=""
    if declare -f get_invoking_home > /dev/null 2>&1; then
        user_home=$(get_invoking_home)
    else
        user_home="${DIGGORY_USER_HOME:-${HOME:-}}"
    fi
...
_diggory_path_is_immediate_child_of() {
    local path="${1%/}"
    local parent="${2%/}"
...
_diggory_path_is_application_bundle() {
    local path="${1%/}"
    _diggory_path_is_immediate_child_of "$path" "/Applications" &&
        [[ "${path##*/}" == *.app ]]
}
...
_diggory_path_requires_direct_trash() {
    local path="${1%/}"
    if _diggory_path_is_application_bundle "$path"; then
        return 0
    fi

    local user_home
    user_home=$(_diggory_valid_invoking_home) || return 1
    _diggory_path_is_immediate_child_of "$path" "$user_home/Library/Containers" && return 0
    _diggory_path_is_immediate_child_of "$path" "$user_home/Library/Group Containers" && return 0
    _diggory_path_is_immediate_child_of "$path" "$user_home/Library/Application Scripts" && return 0
    return 1
}
...
_diggory_move_app_to_trash_via_finder() {
    local path="$1"
    ...
    _diggory_path_is_application_bundle "$path" || return 1
    _diggory_bound_path_matches "$path" "$expected_parent" \
        "$expected_parent_id" "$expected_target_id" || return 1

    run_with_timeout "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" osascript - "$path" > /dev/null 2>&1 << 'APPLESCRIPT' || finder_rc=$?
on run argv
...
# Move a path to the macOS Trash. Test harnesses set DIGGORY_TEST_TRASH_DIR to
# redirect the move to a tmpdir, avoiding any Finder/osascript interaction.
_diggory_move_to_trash() {
    local path="$1"
    local needs_sudo="${2:-false}"
    local expected_parent="${3:-}"
    local expected_parent_id="${4:-}"
    ...
    if [[ -n "${DIGGORY_TEST_TRASH_DIR:-}" ]]; then
        mkdir -p "$DIGGORY_TEST_TRASH_DIR" 2> /dev/null || return 1
        local dest="$DIGGORY_TEST_TRASH_DIR/$(basename "$path").$$.$(date +%s 2> /dev/null || echo 0)"
        _diggory_bound_path_matches "$path" "$expected_parent" \
            "$expected_parent_id" "$expected_target_id" || return 1
    ...
    if [[ "${DIGGORY_TEST_NO_AUTH:-0}" == "1" ]]; then
        return 1
    fi

    if [[ "$needs_sudo" == "true" ]]; then
        _diggory_move_path_to_user_trash "$path" "$needs_sudo" \
            "$expected_parent" "$expected_parent_id" "$expected_target_id"
        return $?
    fi

    if _diggory_path_requires_direct_trash "$path"; then
        local direct_rc=0
        _diggory_move_path_to_user_trash "$path" false \
            "$expected_parent" "$expected_parent_id" \
            "$expected_target_id" || direct_rc=$?
        if [[ $direct_rc -eq $DIGGORY_ERR_PRIVACY_DENIED ]] &&
            _diggory_path_is_application_bundle "$path"; then
            debug_log "Direct Trash move was denied; retrying application through Finder: $path"
            local finder_rc=0
            _diggory_move_app_to_trash_via_finder "$path" \
                "$expected_parent" "$expected_parent_id" \
    ...
    if command -v trash > /dev/null 2>&1; then
        ...
        _diggory_bound_path_matches "$path" "$expected_parent" \
            "$expected_parent_id" "$expected_target_id" || return 1
        run_with_timeout "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
...
_diggory_prepare_privileged_trash_stage_root() {
    local stage_root="$1"
    ...
    # test-then-mkdir: two concurrent Diggory processes can both see a missing root,
...
_diggory_create_privileged_trash_stage() {
    local stage_root="/Library/DiggoryTrashStaging"
    local stage_dir=""

    _diggory_prepare_privileged_trash_stage_root "$stage_root" || return 1
    ...
    if _diggory_privileged_path_has_mutable_ancestor "$stage_dir/item"; then
...
_diggory_move_path_to_user_trash() {
    local path="$1"
    local needs_sudo="${2:-false}"
    local expected_parent="${3:-}"
    local expected_parent_id="${4:-}"
    ...
    base="${base//:/__}"
    base="${base//\//__}"
    [[ -n "$base" && "$base" != "." && "$base" != ".." ]] || base="diggory-trash-item"
    ...
    if [[ -n "$expected_parent" ]]; then
        if ! _diggory_path_matches_identity \
            "$path" "$expected_parent" "$expected_parent_id" "$expected_target_id"; then
            debug_log "Refusing Trash move after selected path identity changed: $path"
            return 1
        fi
    elif [[ -n "${_DIGGORY_TRASH_MOVE_EXPECTED_PATH:-}" && "$_DIGGORY_TRASH_MOVE_EXPECTED_PATH" == "$path" ]]; then
        if ! _diggory_path_matches_identity \
            "$path" \
            "$_DIGGORY_TRASH_MOVE_EXPECTED_PARENT" \
            "$_DIGGORY_TRASH_MOVE_EXPECTED_PARENT_ID" \
            "$_DIGGORY_TRASH_MOVE_EXPECTED_TARGET_ID"; then
    ...
    if [[ "$needs_sudo" == "true" ]]; then
        ...
        local stage_dir=""
        local stage_path=""
        stage_dir=$(_diggory_create_privileged_trash_stage) || {
...
_DIGGORY_TRASH_BATCH_SNAPSHOT_PATHS=()
_DIGGORY_TRASH_BATCH_SNAPSHOT_PARENTS=()
_DIGGORY_TRASH_BATCH_SNAPSHOT_PARENT_IDS=()
_DIGGORY_TRASH_BATCH_SNAPSHOT_TARGET_IDS=()
_DIGGORY_TRASH_BATCH_MOVED_PATHS=()
_DIGGORY_TRASH_MOVE_EXPECTED_PATH=""
_DIGGORY_TRASH_MOVE_EXPECTED_PARENT=""
_DIGGORY_TRASH_MOVE_EXPECTED_PARENT_ID=""
_DIGGORY_TRASH_MOVE_EXPECTED_TARGET_ID=""

_DIGGORY_PATH_SNAPSHOT_PARENT=""
_DIGGORY_PATH_SNAPSHOT_PARENT_ID=""
_DIGGORY_PATH_SNAPSHOT_TARGET_ID=""
...
_diggory_snapshot_path_identity() {
    ...
    _DIGGORY_PATH_SNAPSHOT_PARENT="$physical_parent"
    _DIGGORY_PATH_SNAPSHOT_PARENT_ID="$parent_id"
    _DIGGORY_PATH_SNAPSHOT_TARGET_ID="$target_id"
}

_diggory_path_matches_identity() {
    local path="$1"
    local expected_parent="$2"
    local expected_parent_id="$3"
    local expected_target_id="$4"

    _diggory_snapshot_path_identity "$path" || return 1
    [[ "$_DIGGORY_PATH_SNAPSHOT_PARENT" == "$expected_parent" ]] || return 1
    [[ "$_DIGGORY_PATH_SNAPSHOT_PARENT_ID" == "$expected_parent_id" ]] || return 1
    [[ "$_DIGGORY_PATH_SNAPSHOT_TARGET_ID" == "$expected_target_id" ]]
}

_diggory_bound_path_matches() {
    local path="$1"
    local expected_parent="${2:-}"
    local expected_parent_id="${3:-}"
    local expected_target_id="${4:-}"
...
_diggory_move_to_trash_batch() {
    ...
    if [[ ${#_DIGGORY_TRASH_BATCH_SNAPSHOT_PATHS[@]} -eq ${#paths[@]} &&
        ${#_DIGGORY_TRASH_BATCH_SNAPSHOT_PARENTS[@]} -eq ${#paths[@]} &&
        ${#_DIGGORY_TRASH_BATCH_SNAPSHOT_PARENT_IDS[@]} -eq ${#paths[@]} &&
        ${#_DIGGORY_TRASH_BATCH_SNAPSHOT_TARGET_IDS[@]} -eq ${#paths[@]} ]]; then
    ...
    for ((index = 0; index < ${#paths[@]}; index++)); do
        ...
        if [[ "$use_bound_snapshots" == "true" ]]; then
            [[ "${_DIGGORY_TRASH_BATCH_SNAPSHOT_PATHS[$index]}" == "$p" ]] || return 1
            expected_parents+=("${_DIGGORY_TRASH_BATCH_SNAPSHOT_PARENTS[$index]}")
            expected_parent_ids+=("${_DIGGORY_TRASH_BATCH_SNAPSHOT_PARENT_IDS[$index]}")
            expected_target_ids+=("${_DIGGORY_TRASH_BATCH_SNAPSHOT_TARGET_IDS[$index]}")
        else
            _diggory_snapshot_path_identity "$p" || return 1
            expected_parents+=("$_DIGGORY_PATH_SNAPSHOT_PARENT")
            expected_parent_ids+=("$_DIGGORY_PATH_SNAPSHOT_PARENT_ID")
            expected_target_ids+=("$_DIGGORY_PATH_SNAPSHOT_TARGET_ID")
        fi
    done
    ...
    if [[ -n "${DIGGORY_TEST_TRASH_DIR:-}" ]]; then
        ...
        for ((index = 0; index < ${#paths[@]}; index++)); do
            ...
            dest="$DIGGORY_TEST_TRASH_DIR/$(basename "$p").$$.${ts}.$RANDOM"
            /bin/mv "$p" "$dest" 2> /dev/null || return 1
            _DIGGORY_TRASH_BATCH_MOVED_PATHS+=("$p")
        done
        return 0
    fi
    ...
    for ((index = 0; index < ${#paths[@]}; index++)); do
        ...
        _DIGGORY_TRASH_MOVE_EXPECTED_PATH="$p"
        _DIGGORY_TRASH_MOVE_EXPECTED_PARENT="${expected_parents[$index]}"
        _DIGGORY_TRASH_MOVE_EXPECTED_PARENT_ID="${expected_parent_ids[$index]}"
        _DIGGORY_TRASH_MOVE_EXPECTED_TARGET_ID="${expected_target_ids[$index]}"
        if _diggory_move_path_to_user_trash "$p" false; then
            _DIGGORY_TRASH_BATCH_MOVED_PATHS+=("$p")
        else
            failed=1
        fi
...
_diggory_delete_log() {
    ...
    local log_file="${DIGGORY_DELETE_LOG:-$HOME/Library/Logs/diggory/deletions.log}"
    local log_dir
    ...
    if ! mkdir -p "$log_dir" 2> /dev/null; then
        _diggory_warn_log_broken "create directory: $log_dir"
        return 0
    fi
    ...
    if ! printf '%s\t%s\t%s\t%s\t%s\n' \
        ...
        _diggory_warn_log_broken "write to: $log_file"
    fi
}

_diggory_warn_log_broken() {
    [[ -n "${_DIGGORY_DELETE_LOG_WARNED:-}" ]] && return 0
    _DIGGORY_DELETE_LOG_WARNED=1
    export _DIGGORY_DELETE_LOG_WARNED
...
safe_find_delete() {
    ...
    local scan_rc=0
    run_with_timeout "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" find \
    ...
    while IFS= read -r -d '' match; do
        ...
        if [[ "${DIGGORY_DRY_RUN:-0}" == "1" ]] && declare -f record_dry_run_cleanup_target > /dev/null 2>&1; then
...
_diggory_bounded_sudo_find() {
    local duration="${1:-${DIGGORY_TIMEOUT_DISK_VERIFY_SEC:-30}}"
    shift || true
    [[ $# -gt 0 ]] || return 2
    _diggory_bounded_sudo "$duration" -n find "$@" < /dev/null
}
...
# on a partial prefix.
_diggory_materialize_bounded_sudo_find() {
    local output_file="$1"
    local duration="$2"
    shift 2
    [[ $# -gt 0 ]] || return 2
    ...
    local scan_rc=0
    _diggory_bounded_sudo_find "$duration" "$@" > "$output_file" 2> /dev/null || scan_rc=$?
    if [[ $scan_rc -ne 0 ]]; then
        : > "$output_file" || true
        return "$scan_rc"
    fi
...
_diggory_privileged_batch_max_items() {
    printf '4096\n'
}
...
safe_sudo_find_delete() {
    ...
    DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT=0
    ...
    _diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
    ...
    local sudo_rc=0
    _diggory_bounded_sudo_until "$deadline_seconds" "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
    ...
    local scan_rc=0
    local scan_timeout=""
    scan_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
        "$deadline_seconds") || scan_rc=$?
    if [[ $scan_rc -eq 0 ]]; then
        _diggory_materialize_bounded_sudo_find "$scan_file" "$scan_timeout" \
    ...
    max_batch_items=$(_diggory_privileged_batch_max_items)
    ...
    while IFS= read -r -d '' match; do
        ...
        if _diggory_is_active_powerlog_database_path "$match"; then
            continue
        fi
        if declare -f is_path_whitelisted > /dev/null && is_path_whitelisted "$match"; then
            continue
        ...
        if _diggory_privileged_path_has_mutable_ancestor "$match"; then
        ...
        if [[ "${DIGGORY_DRY_RUN:-0}" == "1" ]] && declare -f record_dry_run_cleanup_target > /dev/null 2>&1; then
            ...
            local size_timeout=""
            local dry_size_rc=0
            size_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
                "$deadline_seconds") || dry_size_rc=$?
            if [[ $dry_size_rc -eq 0 ]]; then
                raw_match_size=$(_diggory_bounded_sudo "$size_timeout" \
        ...
        if [[ "$type_filter" == "f" && "${DIGGORY_DRY_RUN:-0}" != "1" && ! -L "$match" ]]; then
            local identity_timeout=""
            local match_identity=""
            local identity_rc=0
            identity_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
                "$deadline_seconds") || identity_rc=$?
            if [[ $identity_rc -eq 0 ]]; then
                match_identity=$(_diggory_bounded_sudo "$identity_timeout" \
    ...
    if [[ ${#batch_files[@]} -gt 0 && "$deadline_reached" != "true" && "$batch_aborted" != "true" && $delete_rc -ne 124 && $delete_rc -lt 128 ]]; then
        ...
        local batch_timeout=""
        if ! batch_timeout=$(_diggory_timeout_with_deadline "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
            ...
            printf '%s\0' "${batch_worker_records[@]}" |
                _diggory_bounded_sudo "$batch_timeout" \
                    -n xargs -0 /bin/sh -c '
                        stat_tool=$1
                        age_days=$2
                        shift 2
            ...
            while IFS= read -r -d '' batch_file; do
                ...
                if [[ -n "$batch_ts" ]]; then
                    removed_lines+=("[$batch_ts] [${DIGGORY_CURRENT_COMMAND:-clean}] REMOVED $batch_file (batch)")
                fi
        ...
        if [[ $batch_rc -ne 0 && $delete_rc -eq 0 ]]; then
            delete_rc=$batch_rc
            log_operation "${DIGGORY_CURRENT_COMMAND:-clean}" "FAILED" "$base_dir" \
                "batch removal incomplete (status $batch_rc)"
        fi
    fi

    DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT=$removed_count
...
get_path_size_kb() {
    local path="$1"
    local size_timeout="${2:-$DIGGORY_TIMEOUT_DISK_VERIFY_SEC}"
    [[ -z "$path" || ! -e "$path" ]] && {
        echo "0"
        return
    }
    ...
    if [[ ! "$size_timeout" =~ ^[0-9]+(\.[0-9]+)?$ || "$size_timeout" =~ ^0+(\.0+)?$ ]]; then
        size_timeout="$DIGGORY_TIMEOUT_DISK_VERIFY_SEC"
    fi
    ...
    if [[ "$path" == *.app || "$path" == *.app/ ]]; then
        ...
        mdls_timeout=$(_diggory_timeout_with_deadline \
    ...
    if [[ -f "$path" || -L "$path" ]]; then
        ...
        stat_timeout=$(_diggory_timeout_with_deadline \
    ...
    du_timeout=$(_diggory_timeout_with_deadline \
...
diagnose_removal_failure() {
    ...
    case "$exit_code" in
        "$DIGGORY_ERR_SIP_PROTECTED")
            reason="protected by macOS (SIP/MDM)"
            ;;
        "$DIGGORY_ERR_AUTH_FAILED")
            ...
            if declare -F check_touchid_support > /dev/null 2>&1 && check_touchid_support > /dev/null 2>&1; then
                ...
                suggestion="Try 'diggory touchid' to enable fingerprint auth"
            fi
            ;;
        "$DIGGORY_ERR_READONLY_FS")
            reason="filesystem is read-only"
            suggestion="Check if disk needs repair"
            ;;
        "$DIGGORY_ERR_PROTECTED_PATH")
            reason="protected by Diggory safety rules"
            ;;
        "$DIGGORY_ERR_PRIVACY_DENIED")
            ...
            ;;
        "$DIGGORY_ERR_MUTABLE_PARENT")
            reason="Diggory cannot safely use elevated deletion below a user-writable parent"
            suggestion="Move the app to Trash in Finder; Diggory will leave protected containers and app data untouched"
            ;;
        *)
            ...
            if declare -F check_touchid_support > /dev/null 2>&1 && check_touchid_support > /dev/null 2>&1; then
                ...
                suggestion="Try 'diggory touchid' or check with 'ls -l'"
            fi
            ;;
    esac
