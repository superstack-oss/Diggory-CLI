#!/bin/bash
# Diggory - History parsing and rendering.

set -euo pipefail

if [[ -n "${DIGGORY_HISTORY_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_HISTORY_LOADED=1
...
readonly DIGGORY_HISTORY_DEFAULT_LIMIT=20
readonly DIGGORY_HISTORY_MAX_LIMIT=200
...
history_operations_log_file() {
    printf '%s\n' "${DIGGORY_OPERATIONS_LOG:-${OPERATIONS_LOG_FILE:-$HOME/Library/Logs/diggory/operations.log}}"
}

history_deletions_log_file() {
    printf '%s\n' "${DIGGORY_DELETE_LOG:-$HOME/Library/Logs/diggory/deletions.log}"
}

history_normalize_limit() {
    local value="${1:-$DIGGORY_HISTORY_DEFAULT_LIMIT}"
    local normalized max_digits

    if ! normalized=$(history_normalize_decimal "$value"); then
        printf '%s\n' "$DIGGORY_HISTORY_DEFAULT_LIMIT"
        return 0
    fi
    ...
    max_digits=${#DIGGORY_HISTORY_MAX_LIMIT}
    if [[ "${#normalized}" -gt "$max_digits" ]]; then
        printf '%s\n' "$DIGGORY_HISTORY_MAX_LIMIT"
        return 0
    fi
    if [[ "$normalized" -gt "$DIGGORY_HISTORY_MAX_LIMIT" ]]; then
        printf '%s\n' "$DIGGORY_HISTORY_MAX_LIMIT"
        return 0
    fi
    printf '%s\n' "$normalized"
...
history_parse_limit() {
    ...
    [[ "$normalized" -le "$DIGGORY_HISTORY_MAX_LIMIT" ]] || return 1
    printf '%s\n' "$normalized"
}
...
history_render_text() {
    local limit
    limit=$(history_normalize_limit "${1:-$DIGGORY_HISTORY_DEFAULT_LIMIT}")
