#!/bin/bash
# Diggory - Logging System
# Centralized logging with rotation support

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${DIGGORY_LOG_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_LOG_LOADED=1
...
readonly LOG_FILE="${HOME}/Library/Logs/diggory/diggory.log"
readonly DEBUG_LOG_FILE="${HOME}/Library/Logs/diggory/diggory_debug_session.log"
readonly OPERATIONS_LOG_FILE="${HOME}/Library/Logs/diggory/operations.log"
...
rotate_log_once() {
    ...
    [[ -n "${DIGGORY_LOG_ROTATED:-}" ]] && return 0
    export DIGGORY_LOG_ROTATED=1
...
log_system_info() {
    ...
    [[ -n "${DIGGORY_SYS_INFO_LOGGED:-}" ]] && return 0
    export DIGGORY_SYS_INFO_LOGGED=1
    ...
    {
        echo "----------------------------------------------------------------------"
        echo "Diggory Debug Session, $(date '+%Y-%m-%d %H:%M:%S')"
        echo "----------------------------------------------------------------------"
        echo "User: $USER"
