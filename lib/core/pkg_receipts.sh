#!/bin/bash
# Diggory - pkgutil receipt helpers.
# Finds package-installed app bundles outside the standard app locations.

set -euo pipefail

if [[ -n "${DIGGORY_PKG_RECEIPTS_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_PKG_RECEIPTS_LOADED=1
...
pkg_receipt_nonstandard_app_paths() {
    ...
    local cache_file="${DIGGORY_PKG_RECEIPT_CACHE_FILE:-$HOME/.cache/diggory/pkg_receipt_apps_v1}"
    local cache_ttl="${DIGGORY_PKG_RECEIPT_CACHE_TTL:-3600}"
    local now_epoch=0
    ...
    if declare -f run_with_timeout > /dev/null 2>&1; then
        pkgs_output=$(run_with_timeout "${DIGGORY_PKG_RECEIPT_LIST_TIMEOUT:-3}" \
    ...
    if [[ "${DIGGORY_PKG_RECEIPT_CACHE_DISABLE:-0}" != "1" && -r "$cache_file" ]] &&
    ...
    local scan_timeout="${DIGGORY_PKG_RECEIPT_SCAN_TIMEOUT:-8}"
    ...
    if [[ "${DIGGORY_PKG_RECEIPT_CACHE_DISABLE:-0}" != "1" && -n "$cache_file" &&
        ...
        local cache_tmp
        cache_tmp=$(mktemp "${TMPDIR:-/tmp}/diggory.pkg_receipts.XXXXXX" 2> /dev/null || true)
        if [[ -n "$cache_tmp" ]]; then
