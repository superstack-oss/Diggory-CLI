...
check_touchid_support() {
    local pam_sudo_file="${DIGGORY_PAM_SUDO_FILE:-/etc/pam.d/sudo}"
    local pam_sudo_local_file="${DIGGORY_PAM_SUDO_LOCAL_FILE:-$(dirname "$pam_sudo_file")/sudo_local}"
...
request_sudo_access() {
    ...
    if [[ "$is_gui_mode" == true ]]; then
        ...
        local password
        password=$(osascript -e "display dialog \"$escaped_msg\" default answer \"\" with title \"Diggory\" with icon caution with hidden answer" -e 'text returned of result' 2> /dev/null)
...
DIGGORY_SUDO_ESTABLISHED="false"
...
adopt_sudo_session() {
    ...
    if [[ "$DIGGORY_SUDO_ESTABLISHED" == "true" && -n "$DIGGORY_SUDO_KEEPALIVE_PID" ]]; then
        if has_sudo_session; then
            return 0
        fi
        _stop_sudo_keepalive "$DIGGORY_SUDO_KEEPALIVE_PID"
        DIGGORY_SUDO_KEEPALIVE_PID=""
        DIGGORY_SUDO_ESTABLISHED="false"
    fi
    ...
    if [[ -n "$DIGGORY_SUDO_KEEPALIVE_PID" ]]; then
        _stop_sudo_keepalive "$DIGGORY_SUDO_KEEPALIVE_PID"
        DIGGORY_SUDO_KEEPALIVE_PID=""
    fi

    DIGGORY_SUDO_KEEPALIVE_PID=$(_start_sudo_keepalive)
...
ensure_sudo_session() {
    ...
    if has_sudo_session && [[ "$DIGGORY_SUDO_ESTABLISHED" == "true" ]]; then
        return 0
    fi
