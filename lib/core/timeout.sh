#!/bin/bash
# Diggory - Timeout Control
# Command execution with timeout support

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${DIGGORY_TIMEOUT_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_TIMEOUT_LOADED=1
...
_diggory_cleanup_timeout_killer() {
    local killer_pid="${1:-}"
...
# Return success when Diggory's process group still owns the controlling terminal.
# Checking this before a prompt avoids SIGTTIN if a nested interactive command
# returned the tty to Diggory's parent shell instead of restoring it to Diggory.
diggory_tty_is_foreground() {
...
run_with_timeout() {
    ...
    if [[ -n "${MO_TIMEOUT_PERL_BIN:-}" ]]; then
        ...
        "$MO_TIMEOUT_PERL_BIN" -e '
            ...
            # to a child. Diggory runs these helpers concurrently inside one process
            ...
            # dead process group. Diggory then no longer owns the terminal and the
    ...
    trap 'interrupted=1; kill -INT "$cmd_pid" 2>/dev/null || true; _diggory_cleanup_timeout_killer "$killer_pid"' INT
    ...
    _diggory_cleanup_timeout_killer "$killer_pid"
