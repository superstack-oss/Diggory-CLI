#!/bin/bash
# Diggory - UI Components
# Terminal UI utilities: cursor control, keyboard input, spinners, menus

set -euo pipefail

if [[ -n "${DIGGORY_UI_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_UI_LOADED=1
...
if [[ "${BASH_VERSINFO[0]:-0}" -ge 4 ]]; then
    readonly DIGGORY_KEY_SEQ_TIMEOUT="0.3"
else
    readonly DIGGORY_KEY_SEQ_TIMEOUT="1"
fi
...
read_key() {
    ...
    if [[ "${DIGGORY_READ_KEY_FORCE_CHAR:-}" == "1" ]]; then
        [[ -z "$key" ]] && {
            echo "ENTER"
            return 0
        }
    ...
    case "$key" in
        ...
        'G') echo "BOTTOM" ;;
        'g')
            if IFS= read -r -s -n 1 -t "$DIGGORY_KEY_SEQ_TIMEOUT" rest 2> /dev/null; then
                if [[ "$rest" == "g" ]]; then
                    echo "TOP"
                else
                    echo "OTHER"
...
create_inline_spinner_control_dir() {
    ...
    local control_root="$DIGGORY_RESOLVED_TMPDIR"

    [[ -d "$control_root" && ! -L "$control_root" ]] || return 1
    INLINE_SPINNER_CONTROL_DIR=$(umask 077 && mktemp -d "$control_root/.diggory-spinner.XXXXXX") || return 1
    [[ -d "$INLINE_SPINNER_CONTROL_DIR" && ! -L "$INLINE_SPINNER_CONTROL_DIR" && -O "$INLINE_SPINNER_CONTROL_DIR" ]] || return 1
    DIGGORY_TEMP_DIRS+=("$INLINE_SPINNER_CONTROL_DIR")
}
    ...
    if [[ -t 1 ]]; then
        ...
        (
            ...
            while [[ ! -f "$stop_file" ]]; do
                ...
                printf "${frame_lead}${DIGGORY_SPINNER_PREFIX:-}${BLUE}%s${NC} %s" "$c" "$current_message" >&2 || break
                i=$((i + 1))
                /bin/sleep 0.05
            done
...
has_full_disk_access() {
    ...
    if [[ -n "${DIGGORY_HAS_FDA:-}" ]]; then
        if [[ "$DIGGORY_HAS_FDA" == "1" ]]; then
            return 0
        elif [[ "$DIGGORY_HAS_FDA" == "unknown" ]]; then
            return 2
        else
            return 1
        fi
