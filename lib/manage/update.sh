#!/bin/bash
# Diggory self-update: version discovery (GitHub + Homebrew), install-channel
...
#
# VERSION lives in `diggory` (install.sh reads it from there); these functions
# read it at call time, so this file must be sourced after it is set.

set -euo pipefail

# The `diggory` dispatcher assigns VERSION before sourcing this file, so the
# linter cannot see the assignment from here; declare it as an inherited value.
: "${VERSION:=}"

if [[ -n "${DIGGORY_MANAGE_UPDATE_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_MANAGE_UPDATE_LOADED=1
...
_update_lock_path() {
    local install_dir="$1"
    printf '%s/.diggory-update.lock/kernel.lock\n' "$install_dir"
}
...
_update_lock_current_shell_pid() {
    ...
    pid_file=$(/usr/bin/mktemp /tmp/diggory-update-pid.XXXXXX) || return 1
...
# keepalive ran, DIGGORY_ASSUME_SUDO_AUTH stayed 0, and the installer then asked
...
update_install_requires_sudo() {
    ...
    # needs directory write, never target-file write, so a read-only diggory
...
_DIGGORY_UPDATE_VERIFIED_VERSION=""
_DIGGORY_UPDATE_VERIFIED_COMMIT=""
...
_update_verify_installed_generation() {
    ...
    local repair_reason=""
    installed_channel=$(DIGGORY_CONFIG_DIR="$config_dir" get_install_channel 2> /dev/null || true)
    installed_receipt=$(DIGGORY_CONFIG_DIR="$config_dir" get_install_receipt 2> /dev/null || true)
    repair_reason=$(DIGGORY_CONFIG_DIR="$config_dir" manual_install_repair_reason 2> /dev/null || true)
    ...
    if [[ "$installed_receipt" != "$expected_receipt" || -n "$repair_reason" ]]; then
        ...
        local probe_rc=0
        installed_commit=$(DIGGORY_CONFIG_DIR="$config_dir" get_install_commit 2> /dev/null || true)
        version_output=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
            "$diggory_path" --version 2> /dev/null) || probe_rc=$?
        ...
        if [[ $verification_status -eq 0 && "$installed_commit" =~ ^[0-9a-f]{7,40}$ ]]; then
            _DIGGORY_UPDATE_VERIFIED_COMMIT="${installed_commit:0:7}"
        fi
    else
        ...
        if [[ "$installed_channel" != "stable" || "$installed_version" != "${update_ref#V}" ]]; then
            ...
            _DIGGORY_UPDATE_VERIFIED_VERSION="$installed_version"
        fi
    fi
...
_update_print_verified_success() {
    ...
    if [[ "$update_ref" == "main" && -n "$_DIGGORY_UPDATE_VERIFIED_COMMIT" ]]; then
        printf '\n%s\n\n' "${GREEN}${ICON_SUCCESS}${NC} Updated to ${success_label}, ${_DIGGORY_UPDATE_VERIFIED_COMMIT}"
    elif [[ "$update_ref" == "main" ]]; then
        printf '\n%s\n\n' "${GREEN}${ICON_SUCCESS}${NC} Updated to ${success_label}"
    else
        printf '\n%s\n\n' "${GREEN}${ICON_SUCCESS}${NC} Updated to ${success_label}, ${_DIGGORY_UPDATE_VERIFIED_VERSION}"
    fi
}
...
_update_self_heal_reinstall() {
    ...
    _update_verify_installed_generation \
        "$update_ref" "$install_dir" "$config_dir" "$diggory_path" \
...
run_brew_detect() {
    run_brew_command "${DIGGORY_HOMEBREW_DETECT_TIMEOUT:-2}" "$@"
}

run_brew_query() {
    run_brew_command "${DIGGORY_HOMEBREW_QUERY_TIMEOUT:-5}" "$@"
}

brew_diggory_formula_installed() {
    local brew_cmd="${1:-brew}"
    run_brew_detect "$brew_cmd" list diggory > /dev/null 2>&1
}
...
get_homebrew_latest_version() {
    ...
    line=$(run_brew_query brew outdated --formula --verbose diggory 2> /dev/null | head -1 || true)
    if [[ "$line" == *"< "* ]]; then
        candidate="${line##*< }"
        candidate="${candidate%% *}"
    fi
    ...
    if [[ -z "$candidate" ]]; then
        line=$(run_brew_query brew info diggory 2> /dev/null | awk 'NR==1 { print; exit }' || true)
        line="${line#==> }"
        line="${line#*: }"
        if [[ "$line" == stable* ]]; then
            candidate=$(printf '%s\n' "$line" | awk '{print $2}')
...
resolve_diggory_source_path() {
    # DIGGORY_ENTRY_SCRIPT is set by the `diggory` entrypoint before this file is
    ...
    # diggory binary the user invoked.
    local diggory_path="${DIGGORY_ENTRY_SCRIPT:-${BASH_SOURCE[0]:-$0}}"
    if [[ "$diggory_path" != /* ]]; then
        if [[ "$diggory_path" == */* ]]; then
            diggory_path="$(cd "$(dirname "$diggory_path")" 2> /dev/null && pwd)/${diggory_path##*/}"
        else
            diggory_path=$(command -v "$diggory_path" 2> /dev/null || true)
        fi
    fi
    [[ -n "$diggory_path" ]] && printf '%s\n' "$diggory_path"
}

manual_install_repair_reason() {
    local config_root="${DIGGORY_CONFIG_DIR:-$SCRIPT_DIR}"
    local reason=""
    local helper
...
is_homebrew_diggory_path() {
    local diggory_path="$1"
    local has_brew="$2"
    local link_target=""
    [[ -n "$diggory_path" ]] || return 1

    if [[ -L "$diggory_path" ]]; then
        link_target=$(readlink "$diggory_path" 2> /dev/null) || true
        if [[ "$link_target" == *"Cellar/diggory"* ]]; then
            if $has_brew; then
                brew_diggory_formula_installed brew && return 0
            fi
            return 1
        fi
        return 1
    ...
    if [[ -f "$diggory_path" ]]; then
        ...
        case "$diggory_path" in
            "/opt/homebrew/bin/diggory" | "/usr/local/bin/diggory")
                if [[ -d "/opt/homebrew/Cellar/diggory" ]] || [[ -d "/usr/local/Cellar/diggory" ]]; then
                    if $has_brew; then
                        brew_diggory_formula_installed brew && return 0
                    else
...
# Always follows the invoked Diggory script, never PATH, so update and remove act
# on the Diggory the user actually ran instead of another copy earlier in PATH.
is_homebrew_install() {
    ...
    local diggory_path
    diggory_path=$(resolve_diggory_source_path || true)
    is_homebrew_diggory_path "$diggory_path" "$has_brew"
}

get_install_channel() {
    # Try user config dir first (matches install.sh behavior), fallback to SCRIPT_DIR
    local channel_file="${DIGGORY_CONFIG_DIR:-$HOME/.config/diggory}/install_channel"
...
get_latest_commit_from_github() {
    ...
    if command -v git > /dev/null 2>&1; then
        ...
        if response=$(run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" \
            /usr/bin/env \
...
diggory_update_message_cache_is_current() {
    local msg_cache="$1"
    [[ -f "$msg_cache" && -s "$msg_cache" ]] || return 1

    local diggory_path
    diggory_path=$(resolve_diggory_source_path || true)
    [[ -n "$diggory_path" && -e "$diggory_path" ]] || return 0

    local cache_mtime diggory_mtime
    cache_mtime=$(get_file_mtime "$msg_cache")
    diggory_mtime=$(get_file_mtime "$diggory_path")

    if [[ "$cache_mtime" =~ ^[0-9]+$ && "$diggory_mtime" =~ ^[0-9]+$ &&
        "$cache_mtime" -gt 0 && "$diggory_mtime" -gt 0 &&
        "$cache_mtime" -lt "$diggory_mtime" ]]; then
        return 1
    fi
...
read_update_message_cache() {
    local msg_cache="$1"
    if diggory_update_message_cache_is_current "$msg_cache"; then
...
check_for_updates() {
    local msg_cache="$HOME/.cache/diggory/update_message"
...
update_diggory() (
    ...
    # Resolve the invoked Diggory up front so the installer targets this manual
    # install, not another diggory earlier in PATH. Fail before any download.
    local diggory_path
    if ! diggory_path=$(resolve_diggory_source_path); then
        log_error "Unable to resolve current Diggory path"
        exit 1
    fi
    local install_dir
    if ! install_dir="$(cd "$(dirname "$diggory_path")" && pwd)"; then
        log_error "Unable to resolve current Diggory install directory"
        exit 1
    fi
    local latest=""
    local latest_commit=""
    ...
    if [[ "$repair_install" == "true" ]]; then
        ...
        install_label="Repairing Diggory installation..."
        log_warning "Diggory installation needs repair: $repair_reason"
    fi
    ...
    if [[ "$requires_sudo" == "true" ]]; then
        if ! request_sudo_access "Diggory update requires admin access"; then
    ...
    process_install_output() {
        ...
        if ! printf '%s\n' "$output" | grep -Eq "Updated to latest version|Already on latest version"; then
            ...
            if [[ -z "$new_version" ]]; then
                new_version=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
                    "$diggory_path" --version 2> /dev/null | awk 'NF {print $NF; exit}' || true)
            fi
            if [[ -z "$new_version" ]]; then
    ...
    local config_dir="${DIGGORY_CONFIG_DIR:-$SCRIPT_DIR}"
    if [[ ! -f "$config_dir/lib/core/common.sh" ]]; then
        config_dir="$HOME/.config/diggory"
    fi
    ...
    _run_staged_installer() {
        ...
        install_output=$(DIGGORY_ASSUME_SUDO_AUTH="$installer_assume_sudo_auth" \
            DIGGORY_VERSION="$update_ref" DIGGORY_INSTALL_COMMIT="$expected_commit" \
            DIGGORY_INSTALL_RECEIPT="$install_receipt" \
    ...
    if [[ "$nightly_update" == "true" ]]; then
        if _run_staged_installer "main" "$latest_commit"; then
            ...
            if [[ -t 1 ]]; then stop_inline_spinner; fi
            if ! _update_self_heal_reinstall "$installer_assume_sudo_auth" "main" "$install_dir" "$config_dir" "$diggory_path" "$final_success_label" "$latest_commit"; then
                rm -f "$tmp_installer"
                _update_cleanup
        ...
        if _run_staged_installer "$update_tag" ""; then
            ...
            if [[ -t 1 ]]; then stop_inline_spinner; fi
            if ! _update_self_heal_reinstall "$installer_assume_sudo_auth" "$update_tag" "$install_dir" "$config_dir" "$diggory_path" "$final_success_label"; then
                rm -f "$tmp_installer"
                _update_cleanup
                log_error "Update failed"
                _print_failed_installer_output
    ...
    rm -f "$tmp_installer"
    rm -f "$HOME/.cache/diggory/update_message"
