...
_DIGGORY_MANAGE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$_DIGGORY_MANAGE_DIR/../core/common.sh"
source "$_DIGGORY_MANAGE_DIR/../ui/menu_simple.sh"
source "$_DIGGORY_MANAGE_DIR/../optimize/catalog.sh"

# Config file paths
readonly WHITELIST_CONFIG_CLEAN="$HOME/.config/diggory/whitelist"
readonly WHITELIST_CONFIG_OPTIMIZE="$HOME/.config/diggory/whitelist_optimize"
readonly WHITELIST_CONFIG_OPTIMIZE_LEGACY="$HOME/.config/diggory/whitelist_checks"
...
get_optimize_whitelist_items() {
    ...
    for ((index = 0; index < ${#DIGGORY_OPTIMIZE_ACTIONS[@]}; index++)); do
        printf '%s|%s|optimize_task\n' \
            "${DIGGORY_OPTIMIZE_WHITELIST_NAMES[$index]}" \
            "${DIGGORY_OPTIMIZE_ACTIONS[$index]}"
    done
}
...
${GRAY}Edit: ${display_config}${NC}"
    ...
    if [[ ${#selected_cache_patterns[@]} -gt 0 ]]; then
        ...
        local IFS=','
        export DIGGORY_PRESELECTED_INDICES="${preselected_indices[*]}"
    else
        unset DIGGORY_PRESELECTED_INDICES
    fi
    ...
    if [[ -n "$DIGGORY_SELECTION_RESULT" ]]; then
        local -a selected_indices
        IFS=',' read -ra selected_indices <<< "$DIGGORY_SELECTION_RESULT"
