...
if [[ -n "${DIGGORY_OPTIMIZE_CATALOG_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_OPTIMIZE_CATALOG_LOADED=1
...
DIGGORY_OPTIMIZE_ACTIONS=()
DIGGORY_OPTIMIZE_HANDLERS=()
DIGGORY_OPTIMIZE_HEALTH_NAMES=()
DIGGORY_OPTIMIZE_WHITELIST_NAMES=()
DIGGORY_OPTIMIZE_DESCRIPTIONS=()
DIGGORY_OPTIMIZE_SAFE_VALUES=()

_optimize_catalog_register() {
    local index=${#DIGGORY_OPTIMIZE_ACTIONS[@]}
    DIGGORY_OPTIMIZE_ACTIONS[index]="$1"
    DIGGORY_OPTIMIZE_HANDLERS[index]="$2"
    DIGGORY_OPTIMIZE_HEALTH_NAMES[index]="$3"
    DIGGORY_OPTIMIZE_WHITELIST_NAMES[index]="$4"
    DIGGORY_OPTIMIZE_DESCRIPTIONS[index]="$5"
    DIGGORY_OPTIMIZE_SAFE_VALUES[index]="$6"
}
...
optimize_catalog_index_for() {
    ...
    local index
    for ((index = 0; index < ${#DIGGORY_OPTIMIZE_ACTIONS[@]}; index++)); do
        if [[ "${DIGGORY_OPTIMIZE_ACTIONS[$index]}" == "$requested_action" ]]; then
            printf '%s\n' "$index"
            return 0
        fi
    done
...
optimize_catalog_handler_for() {
    ...
    printf '%s\n' "${DIGGORY_OPTIMIZE_HANDLERS[$index]}"
}

optimize_catalog_health_name_for() {
    local index
    index=$(optimize_catalog_index_for "$1") || return 1
    printf '%s\n' "${DIGGORY_OPTIMIZE_HEALTH_NAMES[$index]}"
}

optimize_catalog_validate() {
    local count=${#DIGGORY_OPTIMIZE_ACTIONS[@]}
    if [[ $count -eq 0 ]]; then
        echo "Optimize task catalog is empty" >&2
        return 1
    fi
    if [[ ${#DIGGORY_OPTIMIZE_HANDLERS[@]} -ne $count ||
        ${#DIGGORY_OPTIMIZE_HEALTH_NAMES[@]} -ne $count ||
        ${#DIGGORY_OPTIMIZE_WHITELIST_NAMES[@]} -ne $count ||
        ${#DIGGORY_OPTIMIZE_DESCRIPTIONS[@]} -ne $count ||
        ${#DIGGORY_OPTIMIZE_SAFE_VALUES[@]} -ne $count ]]; then
    ...
    for ((index = 0; index < count; index++)); do
        action=${DIGGORY_OPTIMIZE_ACTIONS[$index]}
        handler=${DIGGORY_OPTIMIZE_HANDLERS[$index]}
        if [[ ! "$action" =~ ^[a-z0-9_]+$ || ! "$handler" =~ ^opt_[a-z0-9_]+$ ]]; then
            echo "Invalid optimize task identity: $action|$handler" >&2
            return 1
        fi
        if [[ -z "${DIGGORY_OPTIMIZE_HEALTH_NAMES[$index]}" ||
            -z "${DIGGORY_OPTIMIZE_WHITELIST_NAMES[$index]}" ||
            -z "${DIGGORY_OPTIMIZE_DESCRIPTIONS[$index]}" ]]; then
            echo "Optimize task metadata is incomplete: $action" >&2
            return 1
        fi
        if [[ "${DIGGORY_OPTIMIZE_SAFE_VALUES[$index]}" != "true" ]]; then
...
optimize_catalog_validate
readonly -a DIGGORY_OPTIMIZE_ACTIONS
readonly -a DIGGORY_OPTIMIZE_HANDLERS
readonly -a DIGGORY_OPTIMIZE_HEALTH_NAMES
readonly -a DIGGORY_OPTIMIZE_WHITELIST_NAMES
readonly -a DIGGORY_OPTIMIZE_DESCRIPTIONS
readonly -a DIGGORY_OPTIMIZE_SAFE_VALUES
