...
readonly DIGGORY_OPTIMIZE_DIAG_CPU_THRESHOLD_DEFAULT=25
readonly DIGGORY_OPTIMIZE_DIAG_SAMPLE_DELAY_DEFAULT=1

opt_diag_cpu_threshold() {
    local threshold="${DIGGORY_OPTIMIZE_DIAG_CPU_THRESHOLD:-$DIGGORY_OPTIMIZE_DIAG_CPU_THRESHOLD_DEFAULT}"
    if ! [[ "$threshold" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
        threshold="$DIGGORY_OPTIMIZE_DIAG_CPU_THRESHOLD_DEFAULT"
    fi
    printf '%s\n' "$threshold"
}

opt_diag_sample_delay() {
    local delay="${DIGGORY_OPTIMIZE_DIAG_SAMPLE_DELAY:-$DIGGORY_OPTIMIZE_DIAG_SAMPLE_DELAY_DEFAULT}"
    if ! [[ "$delay" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
        delay="$DIGGORY_OPTIMIZE_DIAG_SAMPLE_DELAY_DEFAULT"
    fi
    printf '%s\n' "$delay"
}
...
opt_diag_get_ps_sample() {
    ...
    case "$index" in
        1) override="${DIGGORY_OPTIMIZE_PS_SAMPLE_1:-}" ;;
        2) override="${DIGGORY_OPTIMIZE_PS_SAMPLE_2:-}" ;;
    esac
...
opt_diag_get_spctl_status() {
    if [[ -n "${DIGGORY_OPTIMIZE_SPCTL_STATUS:-}" ]]; then
        printf '%s\n' "$DIGGORY_OPTIMIZE_SPCTL_STATUS"
        return 0
    fi
...
opt_diag_get_hdiutil_info() {
    if [[ -n "${DIGGORY_OPTIMIZE_HDIUTIL_INFO:-}" ]]; then
...
opt_diag_family_note() {
    case "$1" in
        cloudshell)
            printf '%s\n' "External enterprise agent pressure detected. Diggory will not terminate enterprise security processes; restart or policy checks must happen outside Diggory."
            ;;
        syspolicyd)
...
opt_diag_detach_candidates() {
    ...
    while IFS=$'\t' read -r image_path mount_path; do
        ...
        safe_mount_path=$(diggory_terminal_safe_text "$mount_path")
...
opt_diag_offer_detach_candidates() {
    ...
    while IFS=$'\t' read -r image_path mount_path; do
        ...
        safe_image_path=$(diggory_terminal_safe_text "${image_path##*/}")
        safe_mount_path=$(diggory_terminal_safe_text "$mount_path")
...
run_optimize_diagnostics() {
    ...
    if [[ -z "${DIGGORY_OPTIMIZE_PS_SAMPLE_1:-}" || -z "${DIGGORY_OPTIMIZE_PS_SAMPLE_2:-}" ]]; then
        sleep "$delay"
    fi
