...
if [[ -n "${DIGGORY_OPTIMIZE_TASKS_LOADED:-}" ]]; then
    return 0
fi
readonly DIGGORY_OPTIMIZE_TASKS_LOADED=1

_DIGGORY_OPTIMIZE_TASKS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
readonly _DIGGORY_OPTIMIZE_TASKS_DIR
source "$_DIGGORY_OPTIMIZE_TASKS_DIR/catalog.sh"
source "$_DIGGORY_OPTIMIZE_TASKS_DIR/outcomes.sh"

# Config constants (override via env).
readonly DIGGORY_TM_THIN_TIMEOUT=180
readonly DIGGORY_TM_THIN_VALUE=9999999999
readonly DIGGORY_SQLITE_MAX_SIZE=104857600 # 100MB
...
# Test-mode env vars hard-deny so ad-hoc task calls under DIGGORY_TEST_NO_AUTH=1
...
optimize_sudo_available() {
    ...
    [[ "${DIGGORY_OPTIMIZE_SUDO_AVAILABLE:-true}" == "true" ]]
}
...
run_launchctl_unload() {
    ...
    if [[ "$need_sudo" == "true" ]]; then
        ...
        local unload_rc=0
        run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" sudo launchctl \
...
has_active_vpn_interface() {
    case "${DIGGORY_ASSUME_VPN_ACTIVE:-}" in
        1 | true | TRUE | yes | YES)
            return 0
            ;;
        0 | false | FALSE | no | NO)
    ...
    scutil_output=$(LC_ALL=C run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" scutil --nc list 2> /dev/null) || scutil_status=$?
    ...
    route_output=$(LC_ALL=C run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" route -n get default 2> /dev/null) || route_status=$?
...
opt_system_maintenance() {
    if [[ "${DIGGORY_DRY_RUN:-0}" != "1" ]] && ! optimize_sudo_available; then
    ...
    if ! spotlight_status=$(run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" mdutil -s / 2> /dev/null); then
...
opt_saved_state_cleanup() {
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        ...
        debug_operation_detail "Method" "Find and remove .savedState folders older than $DIGGORY_SAVED_STATE_AGE_DAYS days"
    ...
    if [[ -d "$state_dir" ]]; then
        ...
        local scan_rc=0
        run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" find "$state_dir" \
            -type d -name "*.savedState" \
            -mtime "+$DIGGORY_SAVED_STATE_AGE_DAYS" -print0 \
...
opt_fix_broken_configs() {
    ...
    if [[ -t 1 ]]; then
        DIGGORY_SPINNER_PREFIX="  " start_inline_spinner "Checking preferences..."
...
opt_network_optimization() {
    ...
    if [[ "${DIGGORY_DNS_FLUSHED:-0}" == "1" ]]; then
...
opt_quarantine_cleanup() {
    ...
    row_count=$(run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" sqlite3 "$quarantine_db" "SELECT COUNT(*) FROM LSQuarantineEvent;" 2> /dev/null) || count_status=$?
    ...
    if [[ "${DIGGORY_DRY_RUN:-0}" != "1" ]]; then
        local exit_code=0
        run_with_timeout "$DIGGORY_TIMEOUT_PKG_LIST_SEC" sqlite3 "$quarantine_db" "DELETE FROM LSQuarantineEvent; VACUUM;" 2> /dev/null || exit_code=$?
...
opt_sqlite_vacuum() {
    ...
    local spinner_started="false"
    if [[ "${DIGGORY_DRY_RUN:-0}" != "1" && -t 1 ]]; then
        DIGGORY_SPINNER_PREFIX="  " start_inline_spinner "Optimizing databases..."
        spinner_started="true"
    fi
    ...
    for pattern in "${db_paths[@]}"; do
        while IFS= read -r db_file; do
            ...
            if [[ "$file_size" -gt "$DIGGORY_SQLITE_MAX_SIZE" ]]; then
            ...
            local page_info=""
            local page_status=0
            page_info=$(run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" sqlite3 "$db_file" "PRAGMA page_count; PRAGMA freelist_count;" 2> /dev/null) || page_status=$?
            ...
            if [[ "${DIGGORY_DRY_RUN:-0}" != "1" ]]; then
                ...
                integrity_check=$(run_with_timeout "$DIGGORY_TIMEOUT_PKG_LIST_SEC" sqlite3 "$db_file" "PRAGMA integrity_check;" 2> /dev/null) || integrity_status=$?
            ...
            local exit_code=0
            if [[ "${DIGGORY_DRY_RUN:-0}" != "1" ]]; then
                run_with_timeout "$DIGGORY_TIMEOUT_PKG_CLEANUP_SEC" sqlite3 "$db_file" "VACUUM;" 2> /dev/null || exit_code=$?
...
opt_launch_services_rebuild() {
    ...
    if [[ -t 1 ]]; then
        DIGGORY_SPINNER_PREFIX="  " start_inline_spinner "Repairing LaunchServices..."
    fi
...
opt_network_stack_optimize() {
    ...
    if run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" route -n get default > /dev/null 2>&1; then
        route_status=0
    else
        route_status=$?
    fi
    if run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" dscacheutil -q host -a name "example.com" > /dev/null 2>&1; then
        dns_status=0
    else
        dns_status=$?
    fi
...
opt_spotlight_index_optimize() {
    ...
    spotlight_status=$(run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" mdutil -s / 2> /dev/null) || spotlight_status_code=$?
    ...
    if echo "$spotlight_status" | grep -qi "Indexing enabled" && ! echo "$spotlight_status" | grep -qi "Indexing and searching disabled"; then
        ...
        local slow_threshold="${DIGGORY_OPTIMIZE_SPOTLIGHT_SLOW_SEC:-3}"
        ...
        local spinner_started="false"
        if [[ -t 1 ]]; then
            DIGGORY_SPINNER_PREFIX="  " start_inline_spinner "Checking Spotlight speed..."
            spinner_started="true"
        fi
        ...
        for probe in 1 2; do
            ...
            probe_status=0
            run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" mdfind "kMDItemFSName == 'Applications'" > /dev/null 2>&1 || probe_status=$?
    ...
    while entry=$(/usr/libexec/PlistBuddy -c "Print :EnabledPreferenceRules:$i" "$plist" 2> /dev/null); do
        case "$entry" in
            ...
            *)
                ...
                if ! diggory_is_reverse_dns_bundle_id "$entry"; then
                    keep+=("$entry")
                else
                    local resolver_rc=0
...
opt_launch_agents_cleanup() {
    ...
    for plist in "$agents_dir"/*.plist; do
        ...
        local binary=""
        local plist_rc=0
        binary=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
...
# Log path is configurable via DIGGORY_PERIODIC_LOG for testing; defaults to /var/log/daily.out.
...
opt_periodic_maintenance() {
    ...
    local daily_log="${DIGGORY_PERIODIC_LOG:-/var/log/daily.out}"
    local stale_days=7
    ...
    if [[ "${DIGGORY_DRY_RUN:-0}" != "1" ]]; then
        if [[ "${DIGGORY_TEST_MODE:-0}" == "1" || "${DIGGORY_TEST_NO_AUTH:-0}" == "1" ]] || ! optimize_sudo_available; then
...
opt_shared_file_list_repair() {
    ...
    local scan_rc=0
    run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" find "$sfl_dir" \
...
# causing the system to freeze. Set DIGGORY_ENABLE_DISK_VERIFY=1 to opt in.
opt_disk_verify() {
    if [[ "${DIGGORY_ENABLE_DISK_VERIFY:-0}" != "1" ]]; then
        opt_msg "Disk verify skipped (set DIGGORY_ENABLE_DISK_VERIFY=1 to enable)"
        optimize_task_result "$DIGGORY_OPTIMIZE_OUTCOME_SKIPPED"
        return 0
    fi
    ...
    if [[ -t 1 ]]; then
        DIGGORY_SPINNER_PREFIX="  " start_inline_spinner "Verifying disk filesystem..."
    fi
    local output=""
    local verify_status=0
    output=$(run_with_timeout "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" diskutil verifyVolume / 2>&1) || verify_status=$?
    if [[ -t 1 ]]; then
        stop_inline_spinner
    fi
...
opt_coreduet_cleanup() {
    ...
    if [[ ${#knowledge_files[@]} -gt 0 ]]; then
        local size_status=0
        total_size=$(run_with_timeout "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" du -skcP "${knowledge_files[@]}" 2> /dev/null | awk 'END {print $1 + 0}') || size_status=$?
...
_login_item_app_exists() {
    ...
    local btm_path=""
    if [[ "${DIGGORY_TEST_MODE:-0}" != "1" && "${DIGGORY_TEST_NO_AUTH:-0}" != "1" ]] && sudo -n true 2> /dev/null; then
