...
select_apps_for_uninstall() {
    ...
    # - DIGGORY_MENU_META_EPOCHS: numeric last_used_epoch per item
    # - DIGGORY_MENU_META_SIZEKB: numeric size in KB per item
    # The menu will gracefully fallback if these are unset or malformed.
    if [[ $has_epoch_metadata == true ]]; then
        export DIGGORY_MENU_META_EPOCHS="$epochs_csv"
    else
        unset DIGGORY_MENU_META_EPOCHS
    fi
    if [[ $has_size_metadata == true ]]; then
        export DIGGORY_MENU_META_SIZEKB="$sizekb_csv"
    else
        unset DIGGORY_MENU_META_SIZEKB
    fi
    export DIGGORY_MENU_FILTER_NAMES="$names_newline"
    export DIGGORY_MENU_IGNORE_INITIAL_ENTER=1

    # Use paginated menu - result will be stored in DIGGORY_SELECTION_RESULT
    ...
    unset DIGGORY_MENU_META_EPOCHS DIGGORY_MENU_META_SIZEKB DIGGORY_MENU_FILTER_NAMES DIGGORY_MENU_IGNORE_INITIAL_ENTER
    # leave DIGGORY_MENU_SORT_DEFAULT untouched if user set it globally

    if [[ $exit_code -ne 0 ]]; then
        return 1
    fi

    if [[ -z "$DIGGORY_SELECTION_RESULT" ]]; then
    ...
    IFS=',' read -r -a indices_array <<< "$DIGGORY_SELECTION_RESULT"
