...
paginated_multi_select() {
    ...
    _DIGGORY_MENU_USER_QUIT=0
    local external_alt_screen=false
    if [[ "${DIGGORY_MANAGED_ALT_SCREEN:-}" == "1" || "${DIGGORY_MANAGED_ALT_SCREEN:-}" == "true" ]]; then
    ...
    local cursor_pos=0
    local top_index=0
    local sort_mode="${DIGGORY_MENU_SORT_MODE:-${DIGGORY_MENU_SORT_DEFAULT:-date}}" # date|name|size
    local sort_reverse="${DIGGORY_MENU_SORT_REVERSE:-false}"
    local filter_text="" # Filter keyword
    local filter_text_lower=""
    local ignore_initial_enter="${DIGGORY_MENU_IGNORE_INITIAL_ENTER:-false}"
    ...
    if [[ -n "${DIGGORY_MENU_META_EPOCHS:-}" ]]; then
        while IFS= read -r v; do
            epochs+=("${v:-0}")
            [[ "${v:-0}" =~ ^[0-9]+$ && "${v:-0}" -gt 0 ]] && has_epoch_metadata="true"
        done < <(_pm_parse_csv_to_array "$DIGGORY_MENU_META_EPOCHS")
    fi
    if [[ -n "${DIGGORY_MENU_META_SIZEKB:-}" ]]; then
        while IFS= read -r v; do
            sizekb+=("${v:-0}")
            [[ "${v:-0}" =~ ^[0-9]+$ && "${v:-0}" -gt 0 ]] && has_size_metadata="true"
        done < <(_pm_parse_csv_to_array "$DIGGORY_MENU_META_SIZEKB")
    fi
    if [[ "$has_epoch_metadata" == "true" || "$has_size_metadata" == "true" ]]; then
        has_metadata="true"
    fi
    if [[ -n "${DIGGORY_MENU_FILTER_NAMES:-}" ]]; then
        while IFS= read -r v; do filter_names+=("$v"); done <<< "$DIGGORY_MENU_FILTER_NAMES"
    ...
    if [[ -n "${DIGGORY_PRESELECTED_INDICES:-}" ]]; then
        local cleaned_preselect="${DIGGORY_PRESELECTED_INDICES//[[:space:]]/}"
    ...
    _pm_cleanup() {
        ...
        export DIGGORY_MENU_SORT_MODE="${sort_mode:-name}"
        export DIGGORY_MENU_SORT_REVERSE="${sort_reverse:-false}"
        restore_terminal
    }
    ...
    draw_header() {
        ...
        elif [[ -n "${DIGGORY_READ_KEY_FORCE_CHAR:-}" ]]; then
    ...
    handle_filter_char() {
        local char="$1"
        if [[ -z "${DIGGORY_READ_KEY_FORCE_CHAR:-}" ]]; then
            return 1
        fi
        if [[ "$char" =~ ^[[:print:]]$ ]]; then
            local char_lower
    ...
    while true; do
        ...
        case "$key" in
            "QUIT")
                if [[ -n "$filter_text" || -n "${DIGGORY_READ_KEY_FORCE_CHAR:-}" ]]; then
                    filter_text=""
                    filter_text_lower=""
                    unset DIGGORY_READ_KEY_FORCE_CHAR
                    rebuild_view
            ...
            "CHAR:o" | "CHAR:O")
                ...
                ;;
            "CHAR:/" | "CHAR:?")
                if [[ -n "${DIGGORY_READ_KEY_FORCE_CHAR:-}" ]]; then
                    unset DIGGORY_READ_KEY_FORCE_CHAR
                else
            ...
            "ENTER")
                ...
                _menu_restore_traps
                DIGGORY_SELECTION_RESULT="$final_result"
                unset DIGGORY_READ_KEY_FORCE_CHAR
                export DIGGORY_MENU_SORT_MODE="${sort_mode:-name}"
                export DIGGORY_MENU_SORT_REVERSE="${sort_reverse:-false}"
                restore_terminal
