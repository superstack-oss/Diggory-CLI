...
[[ -z "${DIGGORY_COMMON_LOADED:-}" ]] && source "$SCRIPT_DIR/lib/core/common.sh"
...
# stays in bootout_login_item_helpers via diggory_is_reverse_dns_bundle_id.
...
# tests exercise the real branch with DIGGORY_TEST_MODE=0 and a launchctl mock.
_uninstall_background_job_loaded() {
    ...
    while IFS= read -r label; do
        [[ -n "$label" ]] || continue
        diggory_is_reverse_dns_bundle_id "$label" || continue
...
discover_login_item_helper_bundle_ids() {
    ...
    local _DIGGORY_UNINSTALL_DISCOVERY_DEADLINE="${_DIGGORY_UNINSTALL_DISCOVERY_DEADLINE:-$((SECONDS + DIGGORY_TIMEOUT_DISK_VERIFY_SEC))}"
    [[ -d "$login_items_root" ]] || return 0

    local scan_file=""
    scan_file=$(create_temp_file) || return 1
    local scan_rc=0
    _diggory_uninstall_materialize_find0 "$scan_file" \
    ...
    while IFS= read -r -d '' helper; do
        ...
        local plist_rc=0
        local plist_timeout=""
        plist_timeout=$(_diggory_timeout_with_deadline \
            "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
            "$_DIGGORY_UNINSTALL_DISCOVERY_DEADLINE") || plist_rc=$?
        ...
        if diggory_is_reverse_dns_bundle_id "$bundle_id"; then
            printf '%s\n' "$bundle_id"
        fi
    done < "$scan_file"
    rm -f -- "$scan_file" 2> /dev/null || true # SAFE: exact tracked temp file created above
...
bootout_login_item_helpers() {
    ...
    if is_uninstall_dry_run || [[ "${DIGGORY_TEST_MODE:-0}" == "1" || "${DIGGORY_TEST_NO_AUTH:-0}" == "1" ]]; then
    ...
    while IFS= read -r helper_id; do
        ...
        diggory_is_reverse_dns_bundle_id "$helper_id" || continue
        ...
        local bootout_rc=0
        run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" launchctl \
...
unload_launch_plist() {
    ...
    local unload_timeout="$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC"
    if [[ -n "$deadline" ]]; then
        unload_timeout=$(_diggory_timeout_with_deadline "$unload_timeout" \
            "$deadline") || return $?
    fi
...
_uninstall_unload_launch_plists() {
    ...
    local scan_rc=0
    if [[ -n "$bundle_id" ]]; then
        _diggory_uninstall_materialize_find0 "$scan_file" "$root" \
    ...
    while IFS= read -r -d '' plist; do
        if [[ -n "$app_path" ]]; then
            local grep_rc=0
            local grep_timeout=""
            grep_timeout=$(_diggory_timeout_with_deadline \
                "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
                "$_DIGGORY_UNINSTALL_DISCOVERY_DEADLINE") || grep_rc=$?
        ...
        unload_launch_plist "$plist" "$needs_sudo" \
            "$_DIGGORY_UNINSTALL_DISCOVERY_DEADLINE" || unload_rc=$?
...
stop_launch_services() {
    ...
    elif ! diggory_is_reverse_dns_bundle_id "$bundle_id"; then
    ...
    if [[ "$bundle_id_usable" == "true" && "$has_system_files" == "true" && "${DIGGORY_TEST_MODE:-0}" != "1" && "${DIGGORY_TEST_NO_AUTH:-0}" != "1" ]]; then
    ...
    if [[ -n "$app_path" ]]; then
        ...
        if [[ "$has_system_files" == "true" && "${DIGGORY_TEST_MODE:-0}" != "1" && "${DIGGORY_TEST_NO_AUTH:-0}" != "1" ]]; then
            if [[ -d /Library/LaunchAgents ]]; then
                _uninstall_unload_launch_plists \
...
unregister_app_bundle() {
    ...
    [[ "${DIGGORY_DRY_RUN:-0}" == "1" ]] && return 0

    local unregister_rc=0
    run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" "$lsregister" \
...
refresh_launch_services_after_uninstall() {
    ...
    run_with_timeout "$DIGGORY_TIMEOUT_PKG_LIST_SEC" "$lsregister" -gc > /dev/null 2>&1 || true
    ...
    if [[ $success -eq 124 ]]; then
        ...
        run_with_timeout "$DIGGORY_TIMEOUT_PKG_LIST_SEC" "$lsregister" -r -f -domain local -domain user > /dev/null 2>&1
        success=$?
...
remove_login_item() {
    ...
    if [[ -n "$clean_name" ]]; then
        ...
        if [[ "${DIGGORY_TEST_MODE:-0}" != "1" && "${DIGGORY_TEST_NO_AUTH:-0}" != "1" ]]; then
            ...
            run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" osascript \
...
# Performance: when DIGGORY_DELETE_MODE=trash and the batch is sudo-free and
...
remove_file_list() {
    ...
    while IFS= read -r file; do
        ...
        # stay on the per-file diggory_delete path. The latter targets bypass
        # third-party Trash tools and Finder inside _diggory_move_to_trash.
        if [[ "$mode" == "trash" && "$use_sudo" != "true" && ! -L "$file" ]] &&
            ! _diggory_path_requires_direct_trash "$file" &&
            ! is_uninstall_dry_run; then
            if _diggory_snapshot_path_identity "$file"; then
                trash_batch+=("$file")
                _DIGGORY_TRASH_BATCH_SNAPSHOT_PATHS+=("$file")
                _DIGGORY_TRASH_BATCH_SNAPSHOT_PARENTS+=("$_DIGGORY_PATH_SNAPSHOT_PARENT")
                _DIGGORY_TRASH_BATCH_SNAPSHOT_PARENT_IDS+=("$_DIGGORY_PATH_SNAPSHOT_PARENT_ID")
                _DIGGORY_TRASH_BATCH_SNAPSHOT_TARGET_IDS+=("$_DIGGORY_PATH_SNAPSHOT_TARGET_ID")
            else
                debug_log "Skipped Trash batch path with unstable identity: $file"
                log_operation "${DIGGORY_CURRENT_COMMAND:-uninstall}" "SKIPPED" "$file" "path identity unavailable"
            fi
        else
            fallback_paths+=("$file")
        fi
    ...
    if [[ ${#trash_batch[@]} -gt 0 ]]; then
        local batch_rc=0
        _diggory_move_to_trash_batch "${trash_batch[@]}" || batch_rc=$?
        if [[ $batch_rc -eq 0 && ${#_DIGGORY_TRASH_BATCH_MOVED_PATHS[@]} -eq 0 ]]; then
            # Test doubles and compatible older helpers report all-or-nothing
            # success without populating the optional moved-path ledger.
            _DIGGORY_TRASH_BATCH_MOVED_PATHS=("${trash_batch[@]}")
        fi
        local _bp _bsize
        if [[ ${#_DIGGORY_TRASH_BATCH_MOVED_PATHS[@]} -gt 0 ]]; then
            for _bp in "${_DIGGORY_TRASH_BATCH_MOVED_PATHS[@]}"; do
                _bsize="unknown"
                _diggory_delete_log "trash" "$_bsize" "ok" "$_bp"
                log_operation "${DIGGORY_CURRENT_COMMAND:-uninstall}" "TRASHED" "$_bp" "batch"
                count=$((count + 1))
            done
        fi
...
# absence is trusted.
_DIGGORY_UNINSTALL_LIVE_APP_ROOTS=(
    "/Applications"
    "$HOME/Applications"
    "/System/Applications"
...
_DIGGORY_UNINSTALL_LIVE_VOLUMES_ROOT="/Volumes"
...
readonly DIGGORY_UNINSTALL_SCAN_PARTIAL=3
...
_uninstall_materialize_complete_find0() {
    ...
    if [[ $scan_rc -eq 0 ]]; then
        ...
        if [[ $scan_rc -eq 1 && -s "$scan_errors" ]]; then
            ...
            scan_rc="$DIGGORY_UNINSTALL_SCAN_PARTIAL"
        fi
        rm -f -- "$scan_errors" 2> /dev/null || true # SAFE: exact tracked temp file created above
    fi
    if [[ $scan_rc -ne 0 && $scan_rc -ne $DIGGORY_UNINSTALL_SCAN_PARTIAL ]]; then
        : > "$output_file" || true
        return "$scan_rc"
    fi
    return "$scan_rc"
...
_DIGGORY_UNINSTALL_LIVE_SIBLING_FINGERPRINT=""
_DIGGORY_UNINSTALL_LIVE_SIBLING_PATHS=()
...
_uninstall_live_sibling_record() {
    ...
    local identity_rc=0
    identity_timeout=$(_diggory_timeout_with_deadline \
        "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" "$deadline_seconds") || identity_rc=$?
...
_uninstall_materialize_complete_pkg_apps() {
    ...
    remaining=$(_diggory_timeout_with_deadline \
        "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" "$deadline_seconds") || remaining_rc=$?
    ...
    # timeout/incomplete paths degrade to DIGGORY_UNINSTALL_SCAN_PARTIAL and
    # narrow the plan rather than deleting shared leftovers.
    local producer_rc=0
    DIGGORY_PKG_RECEIPT_LIST_TIMEOUT="$remaining" \
        DIGGORY_PKG_RECEIPT_SCAN_TIMEOUT="$remaining" \
...
_uninstall_collect_live_sibling_candidate() {
    ...
    local plist_rc=0
    plist_timeout=$(_diggory_timeout_with_deadline \
        "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" "$deadline_seconds") || plist_rc=$?
    local app_bundle=""
    if [[ $plist_rc -eq 0 ]]; then
    ...
    if [[ $plist_rc -ne 0 || -z "$app_bundle" ]]; then
        ...
        local lint_rc=0
        local lint_timeout=""
        if lint_timeout=$(_diggory_timeout_with_deadline \
            "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" "$deadline_seconds"); then
...
# A deadline timeout degrades to DIGGORY_UNINSTALL_SCAN_PARTIAL: out of budget
...
uninstall_live_bundle_has_other_install() {
    ...
    local deadline_seconds=$((SECONDS + (2 * DIGGORY_TIMEOUT_DISK_VERIFY_SEC)))
    ...
    for configured_root in "${_DIGGORY_UNINSTALL_LIVE_APP_ROOTS[@]+"${_DIGGORY_UNINSTALL_LIVE_APP_ROOTS[@]}"}"; do
        live_roots+=("$configured_root")
    done
    if [[ -d "$_DIGGORY_UNINSTALL_LIVE_VOLUMES_ROOT" ]]; then
        ...
        _uninstall_materialize_complete_find0 "$volume_roots_file" \
            "$deadline_seconds" "$_DIGGORY_UNINSTALL_LIVE_VOLUMES_ROOT" \
            -mindepth 2 -maxdepth 2 \
            \( \
            \( -type d -name Applications \) -o \
            \( \( -type d -o -type l \) -name '*.app' \) \
            \) || volume_scan_rc=$?
        if [[ $volume_scan_rc -eq $DIGGORY_UNINSTALL_SCAN_PARTIAL || $volume_scan_rc -eq 124 ]]; then
    ...
    for root in "${live_roots[@]+"${live_roots[@]}"}"; do
        ...
        if [[ $scan_rc -eq $DIGGORY_UNINSTALL_SCAN_PARTIAL ]]; then
    ...
    if [[ $result -eq 0 ]]; then
        local IFS=$'\n'
        _DIGGORY_UNINSTALL_LIVE_SIBLING_FINGERPRINT="${live_records[*]}"
        _DIGGORY_UNINSTALL_LIVE_SIBLING_PATHS=("${live_paths[@]}")
    fi
    ...
    if [[ "$scan_indeterminate" == true && "$result" -eq 1 ]]; then
        result="$DIGGORY_UNINSTALL_SCAN_PARTIAL"
    fi
...
_batch_selected_app_identity() {
    ...
    local identity=""
    local identity_rc=0
    identity=$(run_with_timeout "$DIGGORY_TIMEOUT_QUICK_DETECT_SEC" \
...
_batch_scan_app_details() {
    ...
    local _DIGGORY_UNINSTALL_DISCOVERY_DEADLINE=$((SECONDS + (2 * DIGGORY_TIMEOUT_DISK_VERIFY_SEC)))
    ...
    for selected_app in "${selected_apps[@]}"; do
        ...
        local preview_live_sibling_fingerprint="$_DIGGORY_UNINSTALL_LIVE_SIBLING_FINGERPRINT"
        ...
        local remnant_floor=$((SECONDS + DIGGORY_TIMEOUT_HINT_SCAN_SEC))
        if ((_DIGGORY_UNINSTALL_DISCOVERY_DEADLINE < remnant_floor)); then
            debug_log "Extending uninstall discovery deadline by ${DIGGORY_TIMEOUT_HINT_SCAN_SEC}s for remnant scan of $app_name"
            _DIGGORY_UNINSTALL_DISCOVERY_DEADLINE=$remnant_floor
        fi
        ...
        local delete_mode="${DIGGORY_DELETE_MODE:-permanent}"
        ...
        if [[ "$needs_sudo" == true && "$is_brew_cask" != "true" ]] &&
            _diggory_privileged_path_has_mutable_ancestor "$app_path"; then
            manual_removal_apps+=("$app_name")
            continue
        fi
        ...
        if [[ -n "$discovery_app_name" ]]; then
            ...
            related_files=$(DIGGORY_UNINSTALL_SIBLING_SURVIVES="$sibling_survives" \
    ...
    if [[ ${#manual_removal_apps[@]} -gt 0 ]]; then
        ...
        for manual_name in "${manual_removal_apps[@]}"; do
            log_warning "$manual_name cannot be removed safely by Diggory from this location"
            log_info "Move it to Trash in Finder; Diggory left protected containers and app data untouched"
        done
    fi
}
...
_batch_preview_and_confirm() {
    ...
    # does not work reliably under Diggory's timed non-interactive execution path.
    if [[ "${DIGGORY_DRY_RUN:-0}" != "1" ]] &&
...
_batch_execute_removals() {
    ...
    for detail in "${app_details[@]}"; do
        ...
        if [[ -z "$reason" ]] && diggory_is_reverse_dns_bundle_id "$original_bundle_id"; then
            ...
            if [[ $live_sibling_rc -eq 0 || $live_sibling_rc -eq 1 ]]; then
                if [[ "$preview_live_sibling_fingerprint" != "$_DIGGORY_UNINSTALL_LIVE_SIBLING_FINGERPRINT" ]]; then
                    reason="the app installation set changed after preview"
                    suggestion="Select the app again and review the new removal plan"
                fi
            elif [[ $live_sibling_rc -eq $DIGGORY_UNINSTALL_SCAN_PARTIAL &&
                "$sibling_guard" == "guard_login" &&
        ...
        if [[ -z "$reason" ]]; then
            if [[ "$is_brew_cask" == "true" && -n "$cask_name" ]]; then
                ...
                if [[ $brew_uninstall_rc -eq 0 ]]; then
                    ...
                    # where brew still reports the app as installed after Diggory
        ...
        if [[ -z "$reason" ]]; then
            ...
            if ! is_uninstall_dry_run; then
                ...
                if [[ ${#leftover_paths[@]} -gt 0 ]]; then
                    local _du_total=""
                    local _du_rc=0
                    _du_total=$(run_with_timeout "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" \
            ...
            if diggory_is_reverse_dns_bundle_id "$bundle_id"; then
                ...
                # User-owned plists, so route through user-mode diggory_delete to
                ...
                if [[ -d "$HOME/Library/Preferences/ByHost" ]]; then
                    ...
                    run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" find \
                    ...
                    while IFS= read -r -d '' plist_file; do
                        ...
                        diggory_delete "$plist_file" "false" || plist_delete_rc=$?
            ...
            if ! is_uninstall_dry_run; then
                ...
                while IFS= read -r _review_path; do
                    ...
                    _review_key=$(diggory_normalize_path "$_review_path")
                    if [[ ${#review_only_system_leftover_keys[@]} -eq 0 ]] ||
                        ! diggory_identity_in_list "$_review_key" "${review_only_system_leftover_keys[@]}"; then
            ...
            if diggory_is_reverse_dns_bundle_id "$bundle_id" && [[ -d /Library/SystemExtensions ]]; then
                local system_extension_path=""
                local has_bundle_system_extension=false
                while IFS= read -r -d '' system_extension_path; do
                    if diggory_name_starts_with_bundle_id_boundary "$system_extension_path" "$bundle_id"; then
...
_batch_render_summary() {
    ...
    if [[ ${#review_only_system_leftovers[@]} -gt 0 ]]; then
        ...
        summary_details+=("${ICON_REVIEW} Kept ${kept_count} system-level ${kept_label}, which Diggory never removes")
    fi
...
batch_uninstall_applications() {
    ...
    trap 'stop_inline_spinner 2>/dev/null; _cleanup_sudo_keepalive; unset DIGGORY_UNINSTALL_MODE; echo ""; _restore_uninstall_traps; _batch_interrupted=1; return 130' INT TERM
    ...
    if [[ $brew_apps_removed -gt 0 && "${DIGGORY_DRY_RUN:-0}" != "1" ]]; then
        ...
        (
            HOMEBREW_NO_ENV_HINTS=1 HOMEBREW_NO_AUTO_UPDATE=1 NONINTERACTIVE=1 \
                run_with_timeout "$DIGGORY_TIMEOUT_DISK_VERIFY_SEC" brew autoremove > /dev/null 2>&1 || true
