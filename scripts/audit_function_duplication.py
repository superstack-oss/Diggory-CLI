...
`_user_safe_clean_process_guarded` differed only by a `_DIGGORY_DEV_` vs
`_DIGGORY_USER_` prefix, and nine delete guards each re-implemented the same
...
def normalize(body: list[str]) -> str:
    ...
    text = "\n".join(kept)
    text = re.sub(r"_DIGGORY_[A-Z0-9_]+", "VAR", text)
