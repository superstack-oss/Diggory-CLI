#!/bin/bash
# Create Raycast script commands and Alfred keywords for Diggory (clean + uninstall).
...
LAUNCHER_COMMAND_SPECS=(
    "clean|Diggory Clean|Deep system cleanup with Diggory|Run Diggory clean"
    "uninstall|Diggory Uninstall|Uninstall applications with Diggory|Uninstall apps via Diggory"
    "optimize|Diggory Optimize|System health checks and optimization|System health and optimization"
    "analyze|Diggory Analyze|Disk space analysis with Diggory|Disk space analysis"
    "status|Diggory Status|Live system status dashboard|Live system dashboard"
)
...
detect_digg() {
    if command -v digg > /dev/null 2>&1; then
        ...
        log_error "Diggory not found. Install it first via Homebrew or ./install.sh."
        exit 1
    fi
}
...
create_raycast_commands() {
    ...
    for entry in "${LAUNCHER_COMMAND_SPECS[@]}"; do
        ...
        write_raycast_script "$dir/diggory-${subcommand}.sh" "$title" "$description" "$mo_bin" "$subcommand"
    done
