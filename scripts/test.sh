...
TEST_SYSTEM_STUB_DIR="$(mktemp -d "${TMPDIR:-/tmp}/diggory-test-stubs.XXXXXX")"
TEST_GO_HELPER_DIR=""
...
printf 'diggory test blocked sudo: %s\n' "$*" >&2
exit 1
EOF

cat > "$TEST_SYSTEM_STUB_DIR/osascript" << 'EOF'
#!/bin/bash
printf 'diggory test blocked osascript: %s\n' "$*" >&2
exit 1
EOF

cat > "$TEST_SYSTEM_STUB_DIR/launchctl" << 'EOF'
#!/bin/bash
printf 'diggory test blocked launchctl: %s\n' "$*" >&2
exit 0
EOF
...
report_slowest_test_files() {
    local report="${DIGGORY_TEST_REPORT_DIR:-}/report.xml"
    [[ -n "${DIGGORY_TEST_REPORT_DIR:-}" && -f "$report" ]] || return 0
...
GO_TEST_CACHE="${DIGGORY_GO_TEST_CACHE:-/tmp/diggory-go-build-cache}"
export DIGGORY_GO_TEST_CACHE="$GO_TEST_CACHE"
...
prepare_go_test_helpers() {
    ...
    TEST_GO_HELPER_DIR="$(mktemp -d "${TMPDIR:-/tmp}/diggory-go-helpers.XXXXXX")"
    ...
    if GOCACHE="$GO_TEST_CACHE" go build -o "$TEST_GO_HELPER_DIR/analyze-go" ./cmd/analyze > /dev/null 2>&1 &&
        ...
        export DIGGORY_TEST_ANALYZE_BIN="$TEST_GO_HELPER_DIR/analyze-go"
        export DIGGORY_TEST_STATUS_BIN="$TEST_GO_HELPER_DIR/status-go"
    else
        rm -rf "$TEST_GO_HELPER_DIR"
        TEST_GO_HELPER_DIR=""
    fi
...
if command -v bats > /dev/null 2>&1 && [ -d "tests" ]; then
    ...
    if $bats_has_jobs && { command -v parallel > /dev/null 2>&1 || command -v rush > /dev/null 2>&1; }; then
        ...
        if [[ "${DIGGORY_TEST_JOBS:-}" =~ ^[0-9]+$ && "${DIGGORY_TEST_JOBS:-0}" -gt 0 ]]; then
            _jobs="$DIGGORY_TEST_JOBS"
        else
    ...
    if [[ "${DIGGORY_TEST_TIMING:-0}" == "1" ]]; then
        bats_opts+=("--timing")
    fi
    ...
    if [[ -n "${DIGGORY_TEST_REPORT_DIR:-}" ]] && $bats_has_formatter; then
        mkdir -p "$DIGGORY_TEST_REPORT_DIR"
        bats_opts+=("--report-formatter" "junit" "--output" "$DIGGORY_TEST_REPORT_DIR")
    fi
...
if bash -n diggory && bash -n bin/clean.sh && bash -n bin/optimize.sh; then
...
if [[ "$(uname -s)" != "Darwin" ]]; then
    ...
    # Skip if Homebrew diggory is installed (install.sh will refuse to overwrite)
    install_test_home=""
    install_test_prefix=""
    if command -v brew > /dev/null 2>&1 && brew list diggory &> /dev/null; then
    ...
    if [[ -z "$install_test_home" ]]; then
        ...
        install_test_prefix="$install_test_home/diggory-bin"
    fi
    if [[ -z "$install_test_home" ]]; then
        :
        ...
        if [[ -f "$install_test_prefix/diggory" ]]; then
