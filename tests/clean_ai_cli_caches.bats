...
@test "clean_codex_desktop_caches removes only measured fixed cache leaves through the real sink" {
    run env HOME="$HOME/codex-cache-positive" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
...
@test "clean_codex_desktop_caches stops when Codex starts after the initial probe" {
    run env HOME="$HOME/codex-cache-start-race" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
target="$HOME/Library/Caches/Codex/Default/Cache/owned.bin"
mkdir -p "$(dirname "$target")"
touch "$target"
...
@test "clean_codex_desktop_caches skips while ChatGPT owns the Codex app cache" {
    run env HOME="$HOME/codex-cache-running" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
target="$HOME/Library/Caches/Codex/Default/Cache/owned.bin"
mkdir -p "$(dirname "$target")"
touch "$target"
...
@test "clean_codex_desktop_caches skips when its process probe fails" {
    run env HOME="$HOME/codex-cache-probe-error" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
target="$HOME/Library/Caches/Codex/Default/Cache/owned.bin"
mkdir -p "$(dirname "$target")"
touch "$target"
...
@test "clean_codex_desktop_caches skips when pgrep is unavailable" {
    run env HOME="$HOME/codex-cache-probe-missing" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
target="$HOME/Library/Caches/Codex/Default/Cache/owned.bin"
mkdir -p "$(dirname "$target")"
touch "$target"
...
@test "clean_codex_desktop_caches rejects symlinked Library and Caches ancestors" {
    run env HOME="$HOME/codex-cache-ancestor-links" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
...
@test "clean_codex_desktop_caches rejects a symlinked Codex root" {
    run env HOME="$HOME/codex-cache-root-link" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
...
@test "clean_codex_desktop_caches rejects a symlinked fixed profile" {
    run env HOME="$HOME/codex-cache-profile-link" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
cache_root="$HOME/Library/Caches/Codex"
...
@test "clean_codex_desktop_caches rejects a symlinked cache leaf" {
    run env HOME="$HOME/codex-cache-leaf-link" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
...
[[ ${#_DIGGORY_VERSIONED_AGENT_RETENTION_TARGETS[@]} -eq 2 ]] || exit 1
found_old=false
found_newline=false
...
_DIGGORY_VERSIONED_AGENT_GUARD_ROOT="$root"
_DIGGORY_VERSIONED_AGENT_GUARD_ACTIVE_SYMLINK="$HOME/bin/agent"
...
EOF
    ...
    run env HOME="$isolated_home" PROJECT_ROOT="$PROJECT_ROOT" \
        PATH="$fake_bin:/usr/bin:/bin" DIGGORY_TIMEOUT_DISK_VERIFY_SEC=1 \
        /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
...
@test "versioned agent cleanup rechecks the active symlink after sizing" {
    ...
    run env HOME="$isolated_home" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/bin/clean.sh"
...
diggory_defer_cleanup_family() { echo "DEFER:$1"; }
note_activity() { :; }
clean_codex_marketplace_staging
INNER
