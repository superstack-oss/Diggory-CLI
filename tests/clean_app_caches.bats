...
@test "clean_final_cut_pro_generated_caches rechecks activity after sizing" {
    run env HOME="$HOME/fcp-size-race" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
...
@test "safe_remove refuses a live reverse-DNS user cache (#1390)" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_DRY_RUN=0 /bin/bash --noprofile --norc << 'INNER'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
