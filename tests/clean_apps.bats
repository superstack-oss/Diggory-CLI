...
printf '%s\n%s\n' "com.example.App" "$INSTALLED_APPS_CACHE_COMPLETE_MARKER" > "$HOME/.cache/diggory/installed_apps_cache"
...
EOF
    ...
    [[ "$output" != *"diggory-installed-apps-cache"* ]]
}

@test "scan_installed_apps fails closed when a complete cache cannot reach scan output" {
    run env HOME="$HOME/cache-output-failure" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"

mkdir -p "$HOME/.cache/diggory" "$HOME/installed-output"
printf '%s\n%s\n' "com.example.App" "$INSTALLED_APPS_CACHE_COMPLETE_MARKER" > "$HOME/.cache/diggory/installed_apps_cache"
get_file_mtime() { date +%s; }
debug_log() { :; }
scan_status=0
...
@test "scan_installed_apps rejects the previous complete-cache schema and finds the installed app" {
    run env HOME="$HOME/unmarked-cache" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"

mkdir -p "$HOME/.cache/diggory" "$HOME/Applications/Present.app/Contents"
printf '%s\n%s\n' "com.example.Missing" "# diggory-installed-apps-cache:v2:complete" > "$HOME/.cache/diggory/installed_apps_cache"
...
@test "scan_installed_apps rejects a cache timestamp from the future" {
    run env HOME="$HOME/future-cache" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"

mkdir -p "$HOME/.cache/diggory" "$HOME/Applications/FuturePresent.app/Contents"
printf '%s\n%s\n' "com.example.FutureStale" "$INSTALLED_APPS_CACHE_COMPLETE_MARKER" > "$HOME/.cache/diggory/installed_apps_cache"
...
@test "scan_installed_apps ignores same-directory staging files" {
    run env HOME="$HOME/staged-cache" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"

mkdir -p "$HOME/.cache/diggory" "$HOME/Applications/StagePresent.app/Contents"
printf '%s\n%s\n' "com.example.PartialStage" "$INSTALLED_APPS_CACHE_COMPLETE_MARKER" > "$HOME/.cache/diggory/installed_apps_cache.tmp.interrupted"
...
@test "scan_installed_apps keeps the previous complete cache when publish fails" {
    run env HOME="$HOME/publish-failure" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"

cache_file="$HOME/.cache/diggory/installed_apps_cache"
...
rm -f "$HOME/.cache/diggory/installed_apps_cache"
...
[[ ! -e "$HOME/.cache/diggory/installed_apps_cache" ]] || exit 1
...
@test "scan_installed_apps fails closed when every running-app probe fails" {
    ...
    run env HOME="$scan_home" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"
...
@test "scan_installed_apps aggregates LaunchAgent bundle names without scratch paths" {
    run env HOME="$HOME/agent-scan" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"
...
@test "scan_installed_apps leaves tracked scratch cleanup to the temp registry (#1313)" {
    ...
    run env HOME="$scan_home" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail

mkdir -p "$HOME/diggory-tmp"
export TMPDIR="$HOME/diggory-tmp"
...
[[ -s "$DIGGORY_TEMP_REGISTRY_FILE" ]] || exit 1
scan_tmp_dir=$(head -n 1 "$DIGGORY_TEMP_REGISTRY_FILE")
...
@test "clean_orphaned_app_data renders hostile unreadable bundle names as inert text" {
    ...
    run env HOME="$scan_home" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=1 NO_COLOR=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"
...
@test "clean_orphaned_system_services respects dry-run" {
    # Without DIGGORY_TEST_MODE=0 the sweep early-returns under setup_file's
    # DIGGORY_TEST_MODE=1, leaving $output empty and both negative assertions true.
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 DRY_RUN=true DIGGORY_DRY_RUN=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"
...
@test "clean_orphaned_system_services reports an authorization timeout" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 \
        DRY_RUN=false DIGGORY_DRY_RUN=0 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"
_diggory_bounded_sudo() { return 124; }
...
note_activity() { printf 'ACTIVITY\n'; }
debug_log() { :; }
_diggory_bounded_sudo() { return 0; }
...
@test "clean_orphaned_system_services does not count protected skips as cleaned" {
    # setup_file exports DIGGORY_TEST_MODE=1, under which clean_orphaned_system_services
    # returns immediately and leaves $output empty. Override it as the sibling cases do.
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 DRY_RUN=false DIGGORY_DRY_RUN=0 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"
...
# DIGGORY_UNINSTALL_MODE=1, which deliberately stops consulting DATA_PROTECTED_BUNDLES
...
@test "clean_orphaned_system_services dry-run skips protected paths (#886)" {
    # DIGGORY_TEST_NO_AUTH=0 overrides the CI default (=1) so the function actually
    # runs past the auth-skip guard in apps.sh; the sudo() mock satisfies the
    # `sudo -n true` probe.
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 DRY_RUN=true /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"
...
@test "clean_orphaned_system_services dry-run reports unprotected orphans (#886)" {
    # DIGGORY_TEST_NO_AUTH=0 overrides CI default so the function executes.
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 DRY_RUN=true /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/apps.sh"
...
stub="/Users/diggory-clean-apps-fixture-$$/Library/Containers/com.macpaw.CleanMyMac-mas"
