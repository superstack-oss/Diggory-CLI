...
setup_file() {
    ...
    # Two tests below run the real pipeline (DIGGORY_TEST_MODE=0), which otherwise
    ...
    # $HOME between tests.
    DIGGORY_XCODE_SIM_RUNTIME_VOLUMES_ROOT="$HOME/absent-sim-runtime-volumes"
    DIGGORY_XCODE_SIM_RUNTIME_CRYPTEX_ROOT="$HOME/absent-sim-runtime-cryptex"
    DIGGORY_LSREGISTER_PATH=""
    export DIGGORY_XCODE_SIM_RUNTIME_VOLUMES_ROOT
    export DIGGORY_XCODE_SIM_RUNTIME_CRYPTEX_ROOT
    export DIGGORY_LSREGISTER_PATH
...
setup() {
    ...
    mkdir -p "$HOME/Library/Caches" "$HOME/.config/diggory"
...
run_clean_dry_run() {
    ...
    run env HOME="$HOME" DIGGORY_TEST_MODE=1 PATH="$test_path" \
        "$PROJECT_ROOT/diggory" clean --dry-run
}
...
@test "safe_clean item count reflects cleaned items, not raw target count" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=1 /bin/bash --noprofile --norc << EOF
set -euo pipefail
...
@test "safe_clean treats a real directory size timeout as size-unknown and keeps cleaning" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=1 \
        /bin/bash --noprofile --norc <<EOF
set -euo pipefail
source "\$PROJECT_ROOT/bin/clean.sh"
DRY_RUN=false
DIGGORY_CURRENT_COMMAND=clean
unset DIGGORY_CLEAN_SIZING_TIMEOUTS
run_with_timeout() { return 124; }
safe_remove() { echo "REMOVE:\$1"; /bin/rm -rf "\$1"; }

rc=0
safe_clean "$base/candidate" "Timed out directory" || rc=\$?
printf 'RC=%s CANCEL=%s TIMEOUTS=%s\n' "\$rc" "\${DIGGORY_CLEAN_CANCEL_STATUS:-0}" "\${DIGGORY_CLEAN_SIZING_TIMEOUTS:-0}"
[[ ! -e "$base/candidate" ]]
EOF
...
total_size_cleaned=0
total_items=0
unset DIGGORY_CLEAN_SIZING_TIMEOUTS DIGGORY_CLEAN_REMOVAL_TIMEOUTS
start_section_spinner() { :; }
stop_section_spinner() { :; }
start_inline_spinner() { :; }
stop_inline_spinner() { :; }
...
printf 'RC=%s CANCEL=%s REMOVAL=%s\n' "\$rc" "\${DIGGORY_CLEAN_CANCEL_STATUS:-0}" "\${DIGGORY_CLEAN_REMOVAL_TIMEOUTS:-0}"
[[ -d "$base/a" && -d "$base/b" ]]
EOF
...
@test "DIGGORY_DRY_RUN enables the complete clean preview without deleting Trash" {
    mkdir -p "$HOME/.Trash"
    printf 'keep\n' > "$HOME/.Trash/env-dry-run-sentinel"

    run env HOME="$HOME" DIGGORY_TEST_MODE=1 DIGGORY_DRY_RUN=1 \
        "$PROJECT_ROOT/diggory" clean
...
printf 'DIGGORY_SUDO_ESTABLISHED=%s\n' "$DIGGORY_SUDO_ESTABLISHED"
printf 'DIGGORY_SUDO_KEEPALIVE_PID=%s\n' "$DIGGORY_SUDO_KEEPALIVE_PID"
SCRIPT
    ...
    [[ "$output" == *"DIGGORY_SUDO_ESTABLISHED=true"* ]] || return 1
    [[ "$output" == *"DIGGORY_SUDO_KEEPALIVE_PID=keepalive-pid"* ]]
}
...
MOCK
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PATH="$mock_bin:$PATH" DIGGORY_DF_COUNT="$HOME/df.count" DIGGORY_TEST_MODE=0 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/bin/clean.sh"
...
@test "digg clean --dry-run survives an unwritable TMPDIR" {
    ...
    run env HOME="$HOME" TMPDIR="$blocked_tmp" DIGGORY_TEST_MODE=1 PATH="$test_path" \
        "$PROJECT_ROOT/diggory" clean --dry-run
...
@test "digg clean --dry-run reports user cache without deleting it" {
    ...
    run env HOME="$HOME" DIGGORY_TEST_MODE=1 "$PROJECT_ROOT/diggory" clean --dry-run
    [ "$status" -eq 0 ]
...
PLIST
    ...
    # DIGGORY_TEST_MODE=1 short-circuits clean into a stub that never reaches
    # the App leftovers section, so the report assertion needs the real
    # sections to run. Dry-run keeps this side-effect free.
    set_mock_host_toolchains
    run env HOME="$HOME" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=1 \
        PATH="$MOCK_TOOLCHAIN_BIN:$PATH" "$PROJECT_ROOT/diggory" clean --dry-run
    [ "$status" -eq 0 ]
...
@test "digg clean --dry-run does not export duplicate targets across sections" {
    ...
    set_mock_host_toolchains
    run env HOME="$HOME" DIGGORY_TEST_MODE=0 \
        PATH="$MOCK_TOOLCHAIN_BIN:$PATH" "$PROJECT_ROOT/diggory" clean --dry-run
    [ "$status" -eq 0 ]

    run grep -c "Application Support/Code/CachedData" "$HOME/.config/diggory/clean-list.txt"
    [ "$status" -eq 0 ]
    [ "$output" -eq 1 ]
}
...
@test "digg clean --dry-run keeps container totals and preview paths consistent (#1282)" {
    ...
    printf '%s\n' "$whitelisted_cache/keep.bin" > "$test_home/.config/diggory/whitelist"
    set_mock_sudo_uncached "$test_home"
    set_mock_host_toolchains "$test_home"
    run env HOME="$test_home" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=1 \
        PATH="$TEST_MOCK_BIN:$MOCK_TOOLCHAIN_BIN:$PATH" \
        "$PROJECT_ROOT/diggory" clean --dry-run

    [ "$status" -eq 0 ] || return 1
    local preview="$test_home/.config/diggory/clean-list.txt"
    [[ -f "$preview" ]] || return 1
...
@test "digg clean --dry-run never previews a live SQLite database family (#1390)" {
    ...
    run env HOME="$test_home" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=1 \
        PATH="$MOCK_TOOLCHAIN_BIN:$PATH" "$PROJECT_ROOT/diggory" clean --dry-run
    [ "$status" -eq 0 ] || return 1
    [[ "$(grep -cF "Cache.db" "$test_home/.config/diggory/clean-list.txt")" -eq 0 ]] || return 1
...
@test "digg clean honors whitelist entries" {
    ...
    cat > "$HOME/.config/diggory/whitelist" << EOF
...
@test "custom whitelist without FINDER_METADATA still protects .DS_Store via safety merge (#1396)" {
    mkdir -p "$HOME/Documents" "$HOME/.config/diggory"
    touch "$HOME/Documents/.DS_Store"
    # Pre-FINDER_METADATA user file: custom path only, no sentinel.
    printf '%s\n' "$HOME/.cache/custom-keep/*" > "$HOME/.config/diggory/whitelist"
...
done < "$HOME/.config/diggory/whitelist"
...
@test "_clean_mail_downloads uses dry-run wording and keeps attachments" {
    ...
    # DIGGORY_MAIL_DOWNLOADS_MIN_KB is readonly in base.sh, so an env override is
...
@test "start_section recycles an idle section header in place on a TTY" {
    ...
    env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
        DIGGORY_TEST_NO_AUTH=1 TERM=xterm-256color \
...
@test "log rows do not trigger purge's export-only note_activity override" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" EXPORT_LIST_FILE="$export_file" \
        DIGGORY_SKIP_MAIN=1 DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc -c '
...
@test "cleanup libs share one engine-absent shim instead of forking their own" {
    ...
    # diggory_defer_cleanup_family. Pin the shape so a fourth cannot appear: a
...
@test "diggory_clean_process_guard denies on an unknown process state" {
    ...
    # would turn "Diggory could not tell" into "safe to delete" across the whole
...
diggory_clean_process_guard not_running "App started" || exit 1
[[ -z "$_DIGGORY_CLEAN_GUARD_REASON" ]] || exit 1

rc=0
diggory_clean_process_guard running "App started" || rc=$?
[[ $rc -eq 1 ]] || exit 1
[[ "$_DIGGORY_CLEAN_GUARD_REASON" == "App started" ]] || exit 1

rc=0
diggory_clean_process_guard unknown "App started" || rc=$?
[[ $rc -eq 1 ]] || exit 1
[[ "$_DIGGORY_CLEAN_GUARD_REASON" == "process state unknown" ]] || exit 1

rc=0
diggory_clean_process_guard unknown "Updater started" "updater state unknown" || rc=$?
[[ $rc -eq 1 ]] || exit 1
[[ "$_DIGGORY_CLEAN_GUARD_REASON" == "updater state unknown" ]] || exit 1
EOF
    [ "$status" -eq 0 ] || {
        echo "$output"
        return 1
...
@test "cleanup delete guards do not re-implement the process-state translation" {
    ...
    [ -z "$open_coded" ] || {
        echo "these guards translate the process state themselves instead of calling diggory_clean_process_guard:"
        echo "$open_coded"
        return 1
    }
}
