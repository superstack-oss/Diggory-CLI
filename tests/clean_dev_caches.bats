...
npm() {
    if [[ "$1" == "config" && "$2" == "get" && "$3" == "cache" ]]; then
        echo "/tmp/diggory-custom-npm-cache"
        return 0
    fi
    return 0
}
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"npm cache directory|$HOME/.npm/_cacache/*"* ]] || return 1
    [[ "$output" == *"npm cache directory (custom path)|/tmp/diggory-custom-npm-cache/_cacache/*"* ]] || return 1
    [[ "$output" == *"npm npx cache (custom path)|/tmp/diggory-custom-npm-cache/_npx/*"* ]] || return 1
    [[ "$output" == *"npm logs (custom path)|/tmp/diggory-custom-npm-cache/_logs/*"* ]] || return 1
    [[ "$output" == *"npm prebuilds (custom path)|/tmp/diggory-custom-npm-cache/_prebuilds/*"* ]]
}
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"Bun cache|/tmp/diggory-bun-cache/*"* ]] || return 1
    [[ "$output" == *"Orphaned bun cache|$HOME/.bun/install/cache/*"* ]]
}
...
@test "clean_codex_desktop_staging rechecks physical containment after sizing" {
    ...
    run env HOME="$case_home" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 DRY_RUN=false /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/bin/clean.sh"
pgrep() { return 1; }
codex_sparkle_staging_has_open_files() { return 1; }
...
source "$PROJECT_ROOT/lib/clean/dev.sh"
safe_clean() { echo "$2|$1"; }
diggory_cleanup_targets_exist() { return 0; }
...
diggory_defer_cleanup_family() { printf 'DEFER=%s\n' "$1"; }
safe_clean() { printf 'DELETE=%s|%s\n' "$2" "$1"; }
clean_dev_rust
EOF
...
safe_clean_guarded() {
    local guard="$1"
    shift
    if [[ "$_DIGGORY_RUST_CACHE_ROOT" == "$HOME/.cargo/registry/src" ]]; then
...
_DIGGORY_CODEX_STAGING_ENTRY="$staging_root/entry"
_DIGGORY_CODEX_STAGING_MODE="superseded"
_DIGGORY_CODEX_INSTALLED_BUILD="5848"
