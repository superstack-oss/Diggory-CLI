...
setup() {
    ...
    rm -f "$HOME/.cache/diggory/permissions_granted"
}
...
@test "check_tcc_permissions skips in non-interactive mode" {
    ...
    [ "$status" -eq 0 ]
    [[ ! -f "$HOME/.cache/diggory/permissions_granted" ]]
}

@test "check_tcc_permissions skips when permissions already granted" {
    mkdir -p "$HOME/.cache/diggory"
    touch "$HOME/.cache/diggory/permissions_granted"
...
set -euo pipefail
printf '%s\n' "$HOME/CustomProjects" > "$HOME/.config/diggory/purge_paths"
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/caches.sh"
run_with_timeout() { shift; "$@"; }
safe_clean() { echo "$2|$1"; }
...
EOF
    ...
    rm -rf "$HOME/CustomProjects" "$HOME/.config/diggory" "$fake_bin" "$find_log"
}
...
@test "discover_project_cache_roots dedupes aliased roots by filesystem identity" {
    ...
    printf '%s\n' "$HOME/Code" > "$HOME/.config/diggory/purge_paths"
...
@test "clean_project_caches skips stalled root scans" {
    ...
    printf '%s\n' "$HOME/SlowProjects" > "$HOME/.config/diggory/purge_paths"
...
export DIGGORY_PROJECT_CACHE_DISCOVERY_TIMEOUT=0.5
export DIGGORY_PROJECT_CACHE_SCAN_TIMEOUT=0.5
SECONDS=0
clean_project_caches
...
EOF
    ...
    rm -rf "$HOME/.config/diggory" "$HOME/SlowProjects" "$fake_bin"
}
