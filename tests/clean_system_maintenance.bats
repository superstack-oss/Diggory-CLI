...
safe_sudo_find_delete() {
    echo "safe_sudo_find_delete:$1:$2" >> "$CALL_LOG"
    if [[ "$1" == "$IDLE_DIR" ]]; then
        DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT=1
    else
        DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT=0
    fi
    return 0
...
date +%s > "$HOME/.cache/diggory/brew_last_cleanup"
...
mkdir -p "$HOME/.cache/diggory"
rm -f "$HOME/.cache/diggory/brew_last_cleanup"
...
mkdir -p "$HOME/.cache/diggory" "$HOME/Library/Caches/Homebrew"
rm -f "$HOME/.cache/diggory/brew_last_cleanup"
...
sudo() {
    [[ "${1:-}" == "-n" ]] && shift
    if [[ "${1:-}" == "$STAT_BSD" && "${2:-}" == "-f%z" && "${3:-}" == "$DIGGORY_ACTIVE_POWERLOG_DB_PATH" ]]; then
...
sudo() {
    [[ "${1:-}" == "-n" ]] && shift
    if [[ "$probe_succeeds" == "1" && "${1:-}" == "$STAT_BSD" && "${2:-}" == "-f%z" && "${3:-}" == "$DIGGORY_ACTIVE_POWERLOG_DB_PATH" ]]; then
        printf '%s\n' "$mock_size"
        return 0
    fi
    return 1
...
_diggory_bounded_sudo() { return 1; }
safe_sudo_remove() {
