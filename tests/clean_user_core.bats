...
diggory_truncate_log_file() { echo "WRONG:truncate"; return 99; }
clean_user_essentials
EOF
...
@test "clean_user_essentials keeps Diggory runtime logs while cleaning other user logs" {
    mkdir -p "$HOME/Library/Logs/diggory"
    mkdir -p "$HOME/Library/Logs/OtherApp"
    touch "$HOME/Library/Logs/diggory/operations.log"
...
[[ -f "$HOME/Library/Logs/diggory/operations.log" ]]
...
export DIGGORY_EXTERNAL_VOLUMES_ROOT="$link_root"
...
@test "clean_app_caches caps precise sandbox size scans when many containers exist" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DRY_RUN=true DIGGORY_CONTAINER_CACHE_PRECISE_SIZE_LIMIT=2 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/user.sh"
start_section_spinner() { :; }
