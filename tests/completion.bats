...
@test "completion --help shows usage" {
	...
	[ "$status" -ne 0 ]
	[[ "$output" == *"Usage: diggory completion"* ]] || return 1
	[[ "$output" == *"Auto-install"* ]]
}
...
@test "completion bash can be loaded in bash" {
	run /bin/bash -c "eval \"\$(\"$PROJECT_ROOT/bin/completion.sh\" bash)\" && complete -p diggory"
	[ "$status" -eq 0 ]
	[[ "$output" == *"_diggory_completions"* ]]
}
...
@test "completion auto-install detects zsh" {
	...
	run grep -E "diggory[[:space:]]+completion" "$HOME/.zshrc"
	[ "$status" -eq 0 ]
}
...
@test "completion auto-install detects already installed" {
	...
	echo 'eval "$(diggory completion zsh)"' >"$HOME/.zshrc"
