...
small_kb=1
large_kb=$(((DIGGORY_ONE_GB_BYTES * 2) / 1024))
...
@test "diggory_is_reverse_dns_bundle_id rejects defaults domains and glob-like ids" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"

for valid in "com.example.App" "andriiliakh.Artpaper" "dev.zed.Zed-Nightly" "org.keepassxc.KeePassXC"; do
    diggory_is_reverse_dns_bundle_id "$valid" || {
...
for invalid in "-g" "NSGlobalDomain" "com-example" "com.foo.*" "com.foo.[abc]" "unknown" ""; do
    if diggory_is_reverse_dns_bundle_id "$invalid"; then
...
@test "diggory_name_has_bundle_id_boundary rejects sibling bundle prefixes" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
for valid in \
    ...
    diggory_name_starts_with_bundle_id_boundary "$valid" "$bundle_id" || {
...
for invalid in \
    ...
    if diggory_name_starts_with_bundle_id_boundary "$invalid" "$bundle_id"; then
...
for valid in \
    ...
    "TEAM.com.example.TestApp.FileProvider" \
    "/tmp/com.example.TestApp.pkg.bom"; do
    diggory_name_has_bundle_id_boundary "$valid" "$bundle_id" || {
...
for invalid in \
    ...
    if diggory_name_has_bundle_id_boundary "$invalid" "$bundle_id"; then
...
@test "log_info prints message and appends to log file" {
    ...
    local log_file="$HOME/Library/Logs/diggory/diggory.log"
    [[ -f "$log_file" ]] || return 1
...
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
rm -rf "$HOME/Library/Logs/diggory"
log_operation "clean" "REMOVED" "/tmp/example" "1KB"
EOF
    [ "$status" -eq 0 ]

    local oplog="$HOME/Library/Logs/diggory/operations.log"
    [[ -f "$oplog" ]] || return 1
    grep -Fq "[clean] REMOVED /tmp/example (1KB)" "$oplog"
}

@test "should_protect_path protects Diggory runtime logs" {
    result="$(
        HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" /bin/bash --noprofile --norc -c \
            'source "$PROJECT_ROOT/lib/core/common.sh"; should_protect_path "$HOME/Library/Logs/diggory/operations.log" && echo protected || echo not-protected'
    )"
    [ "$result" = "protected" ]
}
...
@test "rotate_log_once only checks log size once per session" {
    ...
    # log.sh calls rotate_log_once at source time and exports DIGGORY_LOG_ROTATED.
    ...
    env -u DIGGORY_LOG_ROTATED HOME="$HOME" /bin/bash --noprofile --norc -c "source '$PROJECT_ROOT/lib/core/common.sh'"
    [[ -f "${log_file}.old" ]] || return 1

    result=$(HOME="$HOME" DIGGORY_LOG_ROTATED=1 /bin/bash --noprofile --norc -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$DIGGORY_LOG_ROTATED")
    [[ "$result" == "1" ]]
}
...
@test "diggory_pgrep_any distinguishes active, inactive, and unknown probes" {
    run env PROJECT_ROOT="$PROJECT_ROOT" /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
diggory_pgrep_any -x missing -f active
set +e
diggory_pgrep_any -x missing -f absent
inactive_rc=$?
diggory_pgrep_any -x missing -f broken
unknown_rc=$?
set -e
...
source "$PROJECT_ROOT/lib/core/common.sh"
DIGGORY_SPINNER_PREFIX="  " start_inline_spinner "Testing..."
sleep 0.1
stop_inline_spinner
echo "done"
EOF
...
@test "update_inline_spinner_message swaps a live TTY spinner's text in place" {
    ...
    PROJECT_ROOT="$PROJECT_ROOT" HOME="$HOME" TERM=xterm-256color \
        /usr/bin/script -q "$raw" /bin/bash --noprofile --norc -c '
            source "$PROJECT_ROOT/lib/core/common.sh"
            DIGGORY_SPINNER_PREFIX="  " start_inline_spinner "Phase one..."
...
@test "safe_clear_lines emits the same erase sequence per line to the target device" {
    ...
    run /bin/bash --noprofile --norc -c \
        "export DIGGORY_ANSI_SUPPORTED_CACHE=0; source '$PROJECT_ROOT/lib/core/common.sh'; safe_clear_lines 2 '$out'"
    [ "$status" -eq 0 ]
...
@test "read_key maps j/k/h/l to navigation" {
    run /bin/bash -c "export DIGGORY_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'j' | read_key"
    [ "$output" = "DOWN" ]

    run /bin/bash -c "export DIGGORY_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'k' | read_key"
    [ "$output" = "UP" ]

    run /bin/bash -c "export DIGGORY_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'h' | read_key"
    [ "$output" = "LEFT" ]

    run /bin/bash -c "export DIGGORY_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'l' | read_key"
    [ "$output" = "RIGHT" ]
}
...
@test "read_key maps gg to TOP and a lone g to OTHER" {
    run /bin/bash -c "export DIGGORY_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; printf 'gg' | read_key"
    [ "$output" = "TOP" ]

    run /bin/bash -c "export DIGGORY_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; printf 'g' | read_key"
    [ "$output" = "OTHER" ]
}
...
@test "read_key respects DIGGORY_READ_KEY_FORCE_CHAR" {
    run /bin/bash -c "export DIGGORY_BASE_LOADED=1; export DIGGORY_READ_KEY_FORCE_CHAR=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'j' | read_key"
    [ "$output" = "CHAR:j" ]
}

@test "read_key keeps Ctrl-C as quit when forcing printable characters" {
    run /bin/bash -c "export DIGGORY_BASE_LOADED=1; export DIGGORY_READ_KEY_FORCE_CHAR=1; source '$PROJECT_ROOT/lib/core/ui.sh'; printf '\\003' | read_key"
    [ "$output" = "QUIT" ]
}

@test "ensure_sudo_session returns 1 and sets DIGGORY_SUDO_ESTABLISHED=false in test mode" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc <<'SCRIPT'
source "$PROJECT_ROOT/lib/core/base.sh"
source "$PROJECT_ROOT/lib/core/sudo.sh"
DIGGORY_SUDO_ESTABLISHED=""
ensure_sudo_session "Test prompt" && rc=0 || rc=$?
echo "EXIT=$rc"
echo "FLAG=$DIGGORY_SUDO_ESTABLISHED"
SCRIPT
...
echo "EXIT=$?"
echo "FLAG=$DIGGORY_SUDO_ESTABLISHED"
echo "PID=$DIGGORY_SUDO_KEEPALIVE_PID"
SCRIPT
