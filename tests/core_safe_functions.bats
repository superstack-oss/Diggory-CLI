...
@test "validate_path_for_deletion rejects temp roots while allowing their children" {
    ...
    run /bin/bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; validate_path_for_deletion '/private/tmp/diggory-old-artifact'"
    [ "$status" -eq 0 ]

    run /bin/bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; validate_path_for_deletion '/private/var/tmp/diggory-old-artifact'"
    [ "$status" -eq 0 ]
...
MOCK
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_FILE="$target_file" \
        PATH="$mock_bin:$PATH" DIGGORY_RM_TRACE="$trace" DIGGORY_TIMEOUT_DISK_VERIFY_SEC=1 \
...
@test "safe_sudo_remove never opens an interactive sudo prompt" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 /bin/bash --noprofile --norc <<'SCRIPT'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
MOCK
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" \
        PATH="$mock_bin:$PATH" DIGGORY_SUDO_RM_TRACE="$trace" \
        DIGGORY_TIMEOUT_DISK_VERIFY_SEC=1 MO_NO_OPLOG=1 \
        DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 /bin/bash --noprofile --norc <<'SCRIPT'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
_diggory_privileged_path_has_mutable_ancestor() { return 1; }
started=$(date +%s)
...
@test "safe_sudo_remove dry-run refuses an existing compiled model cache" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" \
        DIGGORY_TEST_MODE=1 DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc <<'SCRIPT'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
DIGGORY_DRY_RUN=1
_diggory_privileged_path_has_mutable_ancestor() { return 1; }
...
MOCK
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" \
        PATH="$mock_bin:$PATH" DIGGORY_SUDO_FIND_TRACE="$trace" \
        DIGGORY_TIMEOUT_DISK_VERIFY_SEC=1 DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 \
        /bin/bash --noprofile --norc <<'SCRIPT'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
started=$(date +%s)
...
MOCK
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" \
        PATH="$mock_bin:$PATH" DIGGORY_SUDO_BATCH_TRACE="$trace" DIGGORY_TIMEOUT_DISK_VERIFY_SEC=1 \
        DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 /bin/bash --noprofile --norc <<'SCRIPT'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
_diggory_privileged_path_has_mutable_ancestor() { return 1; }
...
printf 'RC=%s\nCOUNT=%s\n' "$rc" "${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-unset}"
SCRIPT
...
@test "_diggory_timeout_with_deadline clamps fractional timeouts to remaining whole seconds" {
...
SECONDS=100
printf 'CLAMPED=%s\n' "$(_diggory_timeout_with_deadline 30.5 101)"
SECONDS=100
printf 'SHORT=%s\n' "$(_diggory_timeout_with_deadline 0.5 101)"
SECONDS=100
printf 'ZERO=%s\n' "$(_diggory_timeout_with_deadline 0 101)"
SECONDS=100
printf 'LEADING=%s\n' "$(_diggory_timeout_with_deadline 08.5 101)"
SCRIPT
...
MOCK
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" APP_DIR="$app_dir" \
        PATH="$mock_bin:$PATH" DIGGORY_MDLS_TRACE="$trace" \
        /bin/bash --noprofile --norc <<'SCRIPT'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
started=$(date +%s)
...
cat "$HOME/Library/Logs/diggory/operations.log" 2> /dev/null || true
SCRIPT
...
safe_sudo_find_delete "$TARGET_DIR" "*.log" "0" "f" "1" || rc=$?
printf 'RC=%s\nCOUNT=%s\nCONTENT=%s\n' "$rc" \
    "${DIGGORY_SAFE_SUDO_FIND_DELETE_COUNT:-unset}" "$(< "$TARGET_FILE")"
SCRIPT
...
MOCK
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" \
        PATH="$mock_bin:$PATH" DIGGORY_PARTIAL_ACK_TRACE="$trace" \
        DIGGORY_TIMEOUT_DISK_VERIFY_SEC=1 DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 \
        /bin/bash --noprofile --norc <<'SCRIPT'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
@test "safe_sudo_find_delete rejects an oversized privileged batch before deletion" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" \
        MO_NO_OPLOG=1 DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 \
        /bin/bash --noprofile --norc <<'SCRIPT'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
_diggory_privileged_path_has_mutable_ancestor() { return 1; }
_diggory_privileged_batch_max_items() { printf '4\n'; }
sudo() {
    [[ "${1:-}" == "-n" ]] && shift
    case "${1:-}" in
        true) return 0 ;;
...
SCRIPT
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" \
        MO_NO_OPLOG=1 DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 /bin/bash --noprofile --norc "$script"
...
@test "safe_sudo_remove honours the cleanup whitelist before sudo" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET="$target" \
        DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 /bin/bash --noprofile --norc -c '
            set -euo pipefail
            source "$PROJECT_ROOT/lib/core/common.sh"
...
MOCK
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" \
        TARGET_FILE="$target_file" DIGGORY_FIND_TRACE="$trace" PATH="$mock_bin:$PATH" \
        DIGGORY_TIMEOUT_DISK_VERIFY_SEC=1 /bin/bash --noprofile --norc <<'SCRIPT'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
safe_remove() {
    printf 'UNEXPECTED_DELETE:%s\n' "$1"
...
@test "DIGGORY_* constants are defined" {
    run /bin/bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$DIGGORY_TEMP_FILE_AGE_DAYS"
    [ "$status" -eq 0 ]
    [ "$output" = "7" ]

    run /bin/bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$DIGGORY_MAX_PARALLEL_JOBS"
    [ "$status" -eq 0 ]
    [ "$output" = "15" ]

    run /bin/bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$DIGGORY_TM_BACKUP_SAFE_HOURS"
    [ "$status" -eq 0 ]
    [ "$output" = "48" ]
}
...
# running ShipIt made Diggory treat VS Code's idle cache as live. Measured on a
...
state=0
_diggory_user_cache_owner_process_state "com.microsoft.VSCode.ShipIt" || state=$?
printf 'SHIPIT=%s\n' "$state"
state=0
_diggory_user_cache_owner_process_state "com.plausiblelabs.crashreporter.data" || state=$?
printf 'DATA=%s\n' "$state"
state=0
_diggory_user_cache_owner_process_state "com.anthropic.claudefordesktop.ShipIt" || state=$?
printf 'CLAUDE=%s\n' "$state"
EOF
...
state=0
_diggory_user_cache_owner_process_state "com.autodesk.AcCoreConsole" || state=$?
printf 'HELPER=%s\n' "$state"
EOF
...
# Diggory's own size probe runs `du` over the very directory it is judging, so the
# cache id appears in the table because Diggory is looking at it. Counting that as
# ownership would hide every cache that takes long enough to measure.
@test "cache owner probe ignores Diggory's own measurement processes" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/file_ops.sh"
ps() {
  ...
  702     1 /bin/sh sh -c diggory clean
TABLE
}
state=0
_diggory_user_cache_owner_process_state "com.example.SampleApp" || state=$?
printf 'SELF=%s\n' "$state"
EOF
