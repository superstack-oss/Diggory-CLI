...
@test "empty Xcode roots do not register active cleanup families" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
        DIGGORY_XCODE_DOCUMENTATION_CACHE_DIR="$doc_root" \
        DIGGORY_XCODE_SYSTEM_CORESIMULATOR_CACHE_DIR="$simulator_root" \
...
@test "clean_xcode_device_support preserves every version when metadata is unreadable" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_XCODE_DEVICE_SUPPORT_KEEP=2 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
note_activity() { :; }
...
@test "clean_xcode_device_support reports completed removals before a tooling race stops it" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_XCODE_DEVICE_SUPPORT_KEEP=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
note_activity() { :; }
...
@test "clean_xcode_device_support dry run does not double-count stale inner caches" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_XCODE_DEVICE_SUPPORT_KEEP=2 DRY_RUN=true /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
note_activity() { :; }
...
@test "clean_xcode_documentation_cache keeps newest DeveloperDocumentation index" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_XCODE_DOCUMENTATION_CACHE_DIR="$doc_root" /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
note_activity() { :; }
...
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
target="$DIGGORY_XCODE_DOCUMENTATION_CACHE_DIR/DeveloperDocumentation-16.0.index"
should_protect_path() { return 1; }
is_path_whitelisted() { [[ "$1" == "$target" ]]; }
_xcode_xctest_devices_process_running() { return 0; }
defer_cleanup_family() { echo "UNEXPECTED_DEFER:$1"; }
...
clean_xcode_documentation_cache
[[ -e "$DIGGORY_XCODE_DOCUMENTATION_CACHE_DIR/DeveloperDocumentation.index" ]] || exit 1
[[ ! -e "$DIGGORY_XCODE_DOCUMENTATION_CACHE_DIR/DeveloperDocumentation-15.0.index" ]] || exit 1
[[ ! -e "$DIGGORY_XCODE_DOCUMENTATION_CACHE_DIR/$newline_index" ]] || exit 1
EOF
...
clean_xcode_documentation_cache
[[ -e "$DIGGORY_XCODE_DOCUMENTATION_CACHE_DIR/DeveloperDocumentation-16.0.index" ]] || exit 1
EOF
...
rm -f "$HOME/xcode-started"
clean_xcode_documentation_cache
remaining=$(command find "$DIGGORY_XCODE_DOCUMENTATION_CACHE_DIR" -mindepth 1 -maxdepth 1 | wc -l | tr -d ' ')
...
@test "clean_xcode_system_coresimulator_caches removes only direct cache children" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_XCODE_SYSTEM_CORESIMULATOR_CACHE_DIR="$cache_root" /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
note_activity() { :; }
...
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
target="$DIGGORY_XCODE_SYSTEM_CORESIMULATOR_CACHE_DIR/dyld"
should_protect_path() { return 1; }
is_path_whitelisted() { [[ "$1" == "$target" ]]; }
...
while IFS='|' read -r EXPECTED_MODE EXPECTED_PATTERN expected_label; do
    ...
    [[ "$_DIGGORY_XCODE_PROCESS_MATCH" == "$expected_label" ]] || exit 2
...
clean_xcode_system_coresimulator_caches
remaining=$(command find "$DIGGORY_XCODE_SYSTEM_CORESIMULATOR_CACHE_DIR" -mindepth 1 -maxdepth 1 | wc -l | tr -d ' ')
...
clean_xcode_system_coresimulator_caches
[[ -d "$DIGGORY_XCODE_SYSTEM_CORESIMULATOR_CACHE_DIR/entry" ]] || { echo "WRONG: cache removed"; exit 1; }
EOF
...
clean_xcode_system_coresimulator_caches
[[ -d "$DIGGORY_XCODE_SYSTEM_CORESIMULATOR_CACHE_DIR/entry" ]] || exit 1
EOF
...
@test "clean_xcode_xctest_devices rechecks after safe_clean sizing" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
        DIGGORY_XCODE_XCTEST_DEVICES_DIR="$xctest_root" DIGGORY_TEST_NO_AUTH=1 \
        /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
...
clean_xcode_xctest_devices
[[ -d "$DIGGORY_XCODE_XCTEST_DEVICES_DIR/device" ]] || { echo "WRONG: XCTestDevices removed"; exit 1; }
EOF
...
@test "clean_xcode_xctest_devices does not defer a whitelisted root" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_XCODE_XCTEST_DEVICES_DIR="$xctest_root" /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
should_protect_path() { return 1; }
is_path_whitelisted() { [[ "$1" == "$DIGGORY_XCODE_XCTEST_DEVICES_DIR" ]]; }
...
DIGGORY_JETBRAINS_TOOLBOX_KEEP=1
...
@test "clean_dev_ai_agents respects DIGGORY_AI_AGENTS_KEEP and skips missing roots" {
    local claude_root="$HOME/.local/share/claude/versions"
...
note_activity() { :; }
safe_clean() { echo "$1"; }
DIGGORY_AI_AGENTS_KEEP=2 clean_dev_ai_agents
EOF
...
@test "clean_xcode_simulator_runtime_volumes shows scan progress and skips sizing in-use volumes" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" MO_DEBUG=1 DIGGORY_XCODE_SIM_RUNTIME_VOLUMES_ROOT="$volumes_root" DIGGORY_XCODE_SIM_RUNTIME_CRYPTEX_ROOT="$cryptex_root" /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
...
_sim_runtime_mount_points() {
    printf '%s\n' "$DIGGORY_XCODE_SIM_RUNTIME_VOLUMES_ROOT/in-use-runtime"
}
...
@test "clean_xcode_simulator_runtime_volumes deletes nothing when mount enumeration fails" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_XCODE_SIM_RUNTIME_VOLUMES_ROOT="$volumes_root" DIGGORY_XCODE_SIM_RUNTIME_CRYPTEX_ROOT="$HOME/none" /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
...
_sim_runtime_mount_points() {
    ...
    if [[ $round -eq 1 ]]; then
        ...
        printf '%s\n' "$DIGGORY_XCODE_SIM_RUNTIME_VOLUMES_ROOT/runtime-a"
    fi
}
_sim_runtime_size_kb() { echo 1; }
safe_sudo_remove() { echo "UNEXPECTED_REMOVE:$1"; return 0; }
...
clean_xcode_simulator_runtime_volumes
[[ -d "$DIGGORY_XCODE_SIM_RUNTIME_VOLUMES_ROOT/runtime-a" ]] || exit 1
EOF
...
_sim_runtime_mount_points() {
    ...
    if [[ $round -le 2 ]]; then
        ...
        printf '%s\n' "$DIGGORY_XCODE_SIM_RUNTIME_VOLUMES_ROOT/runtime-b"
    fi
}
_sim_runtime_size_kb() { echo 1; }
safe_sudo_remove() { return 1; }
...
clean_xcode_simulator_runtime_volumes
[[ -d "$DIGGORY_XCODE_SIM_RUNTIME_VOLUMES_ROOT/runtime-a" ]] || exit 1
[[ -d "$DIGGORY_XCODE_SIM_RUNTIME_VOLUMES_ROOT/runtime-b" ]] || exit 1
EOF
...
@test "clean_dev_mobile leaves an idle section when no unavailable simulator exists" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 DRY_RUN=true \
        /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/bin/clean.sh"
check_android_ndk() { :; }
...
_resolve_simctl_developer_dir() {
    _DIGGORY_SIMCTL_RESOLUTION_STATUS="ready"
    _DIGGORY_SIMCTL_DEVELOPER_DIR="$HOME/Xcode.app/Contents/Developer"
}
...
_DIGGORY_SIMCTL_XCODE_APP_ROOTS=("$HOME/EmptyApplications")
mkdir -p "${_DIGGORY_SIMCTL_XCODE_APP_ROOTS[0]}"
check_android_ndk() { :; }
clean_xcode_documentation_cache() { :; }
clean_xcode_system_coresimulator_caches() { :; }
clean_xcode_simulator_runtime_volumes() { :; }
...
_DIGGORY_SIMCTL_XCODE_APP_ROOTS=("$HOME/Applications")
check_android_ndk() { :; }
clean_xcode_documentation_cache() { :; }
clean_xcode_system_coresimulator_caches() { :; }
clean_xcode_simulator_runtime_volumes() { :; }
...
@test "clean_dev_mobile stops before simctl delete when unavailable-device sizing times out" {
    ...
    run env HOME="$case_home" PROJECT_ROOT="$PROJECT_ROOT" DRY_RUN=false \
        DIGGORY_CURRENT_COMMAND=clean DIGGORY_CLEAN_CANCEL_STATUS=0 \
