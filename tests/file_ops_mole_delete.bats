...
# Tests for diggory_delete in lib/core/file_ops.sh.
# Exercises permanent mode (default), trash mode (via DIGGORY_TEST_TRASH_DIR
...
@test "diggory_delete defaults to permanent mode and removes the target" {
...
$(prelude)
diggory_delete "$victim"
EOF

    [ "$status" -eq 0 ]
    [[ ! -e "$victim" ]] || return 1
    # Trash dir must remain empty in permanent mode.
    [[ -z "$(ls -A "$DIGGORY_TEST_TRASH_DIR" 2> /dev/null || true)" ]]
}

@test "diggory_delete trash mode moves the target instead of rm -rf" {
...
EOF
    ...
    [[ -n "$(ls -A "$DIGGORY_TEST_TRASH_DIR" 2> /dev/null || true)" ]]
}

@test "diggory_delete moves sudo-required paths to invoking user Trash" {
...
unset DIGGORY_TEST_NO_AUTH
export DIGGORY_DELETE_MODE=trash
export PATH="$fake_bin:\$PATH"
export HOME="$fake_home"
export DIGGORY_TEST_TRACE="$trace"
_diggory_privileged_path_has_mutable_ancestor() { return 1; }
_diggory_create_privileged_trash_stage() {
    local stage="$SANDBOX/stage-sudo-trash"
    mkdir -p "\$stage"
    printf '%s\n' "\$stage"
}
diggory_delete "$victim" true
EOF
    ...
    [[ "$(grep -c 'rmdir /Library/DiggoryTrashStaging' "$trace" 2> /dev/null || true)" -eq 0 ]] || return 1
    ...
    local status_col
    status_col=$(awk -F'\t' 'END { print $4 }' "$DIGGORY_DELETE_LOG")
    [ "$status_col" = "ok" ]
}

@test "diggory_delete refuses symlinked invoking user Trash for sudo-required paths" {
...
@test "diggory_delete uses unique Trash name for sudo-required path conflicts" {
...
@test "privileged Trash staging never uses world-writable Library Caches" {
    run grep -nF '/Library/Caches/.diggory-trash' "$PROJECT_ROOT/lib/core/file_ops.sh"
    [ "$status" -ne 0 ]

    run grep -nF 'stage_root="/Library/DiggoryTrashStaging"' "$PROJECT_ROOT/lib/core/file_ops.sh"
    [ "$status" -eq 0 ]
}
...
_diggory_path_requires_direct_trash "/Applications/Microsoft Word.app"
! _diggory_path_requires_direct_trash "/Applications/Utilities/Microsoft Word.app"
! _diggory_path_requires_direct_trash "/Applications/Microsoft Word.app/Contents"
_diggory_move_to_trash "/Applications/Microsoft Word.app" false
EOF
...
_diggory_move_to_trash "/Applications/Developer.app" false
EOF
...
unset DIGGORY_TEST_NO_AUTH
export DIGGORY_DELETE_MODE=trash
_diggory_privileged_path_has_mutable_ancestor() { return 0; }
_diggory_move_path_to_user_trash() {
    printf 'DIRECT:%s:%s\n' "\$1" "\$2" >> "$trace"
}
osascript() {
...
set -e
printf 'RC=%s\n' "\$rc"
[[ \$rc -eq \$DIGGORY_ERR_MUTABLE_PARENT ]] || exit 1
EOF
    ...
    [ "$(awk -F'\t' 'END { print $4 }' "$DIGGORY_DELETE_LOG")" = "mutable-parent" ]
}
...
$(prelude)
unset DIGGORY_TEST_NO_AUTH
export DIGGORY_DELETE_MODE=permanent
_diggory_privileged_path_has_mutable_ancestor() { return 0; }
...
set -e
printf 'RC=%s\n' "\$rc"
[[ \$rc -eq \$DIGGORY_ERR_PRIVACY_DENIED ]] || exit 1
EOF
    ...
    [[ ! -s "$trace" ]] || return 1
    [ "$(awk -F'\t' 'END { print $4 }' "$DIGGORY_DELETE_LOG")" = "privacy-denied" ]
}
...
set +e
_diggory_move_path_to_user_trash "$victim" false
rc=\$?
set -e
printf 'RC=%s\n' "\$rc"
[[ \$rc -eq \$DIGGORY_ERR_PRIVACY_DENIED ]] || exit 1
...
@test "privacy denial diagnosis recommends terminal privacy access, not Touch ID" {
    run /bin/bash --noprofile --norc <<EOF
$(prelude)
diagnose_removal_failure "\$DIGGORY_ERR_PRIVACY_DENIED" "Microsoft Word"
EOF
...
@test "mutable-parent diagnosis recommends manual Trash without promising container cleanup" {
    run /bin/bash --noprofile --norc <<EOF
$(prelude)
diagnose_removal_failure "\$DIGGORY_ERR_MUTABLE_PARENT" "Microsoft Word"
EOF
...
diagnose_removal_failure "\$DIGGORY_ERR_SIP_PROTECTED" "Microsoft Word"
diagnose_removal_failure "\$DIGGORY_ERR_READONLY_FS" "Microsoft Word"
diagnose_removal_failure "\$DIGGORY_ERR_PROTECTED_PATH" "Microsoft Word"
diagnose_removal_failure "\$DIGGORY_ERR_PRIVACY_DENIED" "Microsoft Word"
printf 'checks=%s\n' "\$touchid_checks"
EOF
...
diagnose_removal_failure "\$DIGGORY_ERR_AUTH_FAILED" "Microsoft Word"
...
if check_touchid_support; then
    ...
    expected="authentication failed|Try 'diggory touchid' to enable fingerprint auth"
fi
actual=\$(diagnose_removal_failure "\$DIGGORY_ERR_AUTH_FAILED" "Microsoft Word")
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"authentication failed|Try 'diggory touchid' to enable fingerprint auth"* ]] || return 1
    [[ "$output" == *"permission denied|Try 'diggory touchid' or check with 'ls -l'"* ]]
}
...
@test "diggory_delete writes a tab-separated log line per call" {
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ -s "$DIGGORY_DELETE_LOG" ]] || return 1

    # Expect 5 tab-separated fields: timestamp, mode, size_kb, status, path.
    local fields
    fields=$(awk -F'\t' 'END { print NF }' "$DIGGORY_DELETE_LOG")
    [ "$fields" -eq 5 ]
...
@test "diggory_delete rejects symlinks to protected system paths" {
...
@test "diggory_delete dry-run does not touch the filesystem but still logs" {
    local victim="$SANDBOX/dry"
    : > "$victim"

    run /bin/bash --noprofile --norc <<EOF
$(prelude)
export DIGGORY_DRY_RUN=1
diggory_delete "$victim"
EOF
...
@test "diggory_delete records a forensic log entry for rejected paths" {
    run /bin/bash --noprofile --norc <<EOF
$(prelude)
diggory_delete "/tmp/../etc/hosts"
EOF
    ...
    status_col=$(awk -F'\t' 'END { print $4 }' "$DIGGORY_DELETE_LOG")
    size_col=$(awk -F'\t' 'END { print $3 }' "$DIGGORY_DELETE_LOG")
    [ "$status_col" = "rejected" ]
    [ "$size_col" = "0" ]
}

@test "diggory_delete is a no-op on a non-existent path" {
    run /bin/bash --noprofile --norc <<EOF
$(prelude)
diggory_delete "$SANDBOX/does-not-exist"
EOF

    [ "$status" -eq 0 ]
    [[ ! -s "$DIGGORY_DELETE_LOG" ]]
}

@test "diggory_delete rejects unknown delete mode without touching target" {
    local victim="$SANDBOX/invalid_mode_target"
    : > "$victim"

    run /bin/bash --noprofile --norc <<EOF
$(prelude)
export DIGGORY_DELETE_MODE=surprise
diggory_delete "$victim"
EOF
    ...
    mode_col=$(awk -F'\t' 'END { print $2 }' "$DIGGORY_DELETE_LOG")
    status_col=$(awk -F'\t' 'END { print $4 }' "$DIGGORY_DELETE_LOG")
    [ "$mode_col" = "surprise" ]
...
@test "diggory_delete warns once for repeated invalid delete mode" {
...
export DIGGORY_DELETE_MODE=surprise
set +e
diggory_delete "$first"
first_rc=\$?
diggory_delete "$second"
second_rc=\$?
set -e
...
EOF
    ...
    [[ -e "$first" ]] || return 1
    [[ -e "$second" ]] || return 1
    [[ "$(grep -c 'invalid DIGGORY_DELETE_MODE' <<< "$output")" -eq 1 ]]
}

@test "diggory_delete trash failure leaves target in place" {
    local victim="$SANDBOX/fallback_target"
    : > "$victim"

    # Pointing DIGGORY_TEST_TRASH_DIR at a non-writable parent forces the stub
...
$(prelude)
export DIGGORY_DELETE_MODE=trash
export DIGGORY_TEST_TRASH_DIR="$blocked"
diggory_delete "$victim"
EOF
...
@test "diggory_delete warns once for repeated Trash failures" {
...
@test "diggory_delete records 'unknown' (not 0) when size measurement fails" {
...
@test "diggory_delete stops before Trash or permanent removal when sizing is interrupted" {
...
export DIGGORY_DELETE_MODE=trash
trash_rc=0
diggory_delete "$trash_victim" || trash_rc=\$?
printf 'TRASH_RC=%s\n' "\$trash_rc"
...
sudo_rc=0
diggory_delete "$sudo_victim" true || sudo_rc=\$?
printf 'SUDO_RC=%s\n' "\$sudo_rc"
EOF
...
@test "diggory_delete preserves an interrupted Trash action in its return code and forensic log" {
...
$(prelude)
export DIGGORY_DELETE_MODE=trash
_diggory_move_to_trash() { return 130; }
safe_remove() {
    printf 'UNEXPECTED_REMOVE:%s\n' "\$1"
    return 99
}
rc=0
diggory_delete "$victim" || rc=\$?
printf 'RC=%s\n' "\$rc"
EOF
    ...
    [[ -d "$victim" ]] || return 1
    [ "$(awk -F'\t' 'END { print $4 }' "$DIGGORY_DELETE_LOG")" = "interrupted" ]
}
...
@test "safe_remove refuses a replacement reached through a swapped parent" {
    ...
    run /bin/bash --noprofile --norc <<EOF
$(prelude)
_diggory_snapshot_path_identity "$victim"
...
@test "safe_remove records a clean interruption and blocks later deletion sinks" {
    ...
    run /bin/bash --noprofile --norc <<EOF
$(prelude)
export DIGGORY_CURRENT_COMMAND=clean
export DIGGORY_CLEAN_CANCEL_STATUS=0
rm() { return 130; }
rc=0
safe_remove "$first" true 1 || rc=\$?
[[ \$rc -eq 130 ]] || exit 1
[[ \$DIGGORY_CLEAN_CANCEL_STATUS -eq 130 ]] || exit 1
...
@test "diggory_delete never binds a replacement installed during identity snapshot" {
...
_diggory_snapshot_path_identity() {
    _real_diggory_snapshot_path_identity "\$1" || return \$?
    mv "\$1" "\$1.original"
    mkdir -p "\$1"
    printf 'replacement\n' > "\$1/data"
}
...
rc=0
diggory_delete "$victim" false "\$expected_identity" || rc=\$?
[[ \$rc -ne 0 ]] || exit 1
...
_diggory_path_is_application_bundle() { return 0; }
_diggory_move_path_to_user_trash() {
    mv "\$1" "\$1.original"
    mkdir -p "\$1"
    printf 'replacement\n' > "\$1/data"
...
@test "diggory_delete dry-run propagates an interrupted preview registration" {
...
@test "diggory_delete warns once per session when audit log is unwritable" {
    ...
    run /bin/bash --noprofile --norc <<EOF
set -euo pipefail
export DIGGORY_DELETE_LOG="$broken_log_dir/deletions.log"
export DIGGORY_TEST_TRASH_DIR="$DIGGORY_TEST_TRASH_DIR"
export DIGGORY_TEST_NO_AUTH=1
source "$PROJECT_ROOT/lib/core/common.sh"
diggory_delete "$victim"
...
diggory_delete "$SANDBOX/second_victim"
EOF
...
export DIGGORY_TIMEOUT_DISK_VERIFY_SEC=1
export DIGGORY_TEST_NO_AUTH=1
source "$PROJECT_ROOT/lib/core/common.sh"
...
SH
    ...
    run /bin/bash --noprofile --norc <<EOF
$(prelude)
unset DIGGORY_TEST_NO_AUTH DIGGORY_TEST_MODE
export PATH="$fake_bin:\$PATH"
export DIGGORY_TEST_TRACE="$trace"
_diggory_privileged_path_has_mutable_ancestor() { return 1; }
...
# two Diggory processes can both observe a missing root, and a plain mkdir would
...
export PATH="$fake_bin:\$PATH"
export DIGGORY_TEST_TRACE="$trace"
_diggory_prepare_privileged_trash_stage_root "$stage_root"
EOF
