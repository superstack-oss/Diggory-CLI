...
setup() {
	...
	cat > "$HOME/source/diggory" << 'DIGGORY'
VERSION="1.2.3"
DIGGORY
}
...
@test "install_files fails closed when sudo is unavailable, even under || caller (#update-incident)" {
	# Old diggorys invoke `install_files || {...}`, which disables errexit inside
	...
	# DIGGORY_TEST_NO_AUTH must not leak in: it would take the blocked-in-test-mode
	# branch instead of the real ensure_sudo_ready gate. sudo is a function mock.
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=0 DIGGORY_TEST_MODE=0 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
...
cat > "$INSTALL_DIR/diggory" <<'DIGGORY'
#!/bin/bash
VERSION="9.9.9"
sleep 3
DIGGORY
...
grep -qF -- "-k|1|5|$INSTALL_DIR/diggory|--version" "$trace"
grep -qF -- "-k|1|5|$INSTALL_DIR/diggory|--help" "$trace"
EOF
...
INSTALL_SOURCE_TMP=$(mktemp -d "${TMPDIR}diggory-source.XXXXXX")
...
@test "source download mktemp template derives from TMPDIR" {
	run grep -qE 'mktemp -d "\$\{TMPDIR:-/tmp\}/diggory\.XXXXXX"' "$PROJECT_ROOT/install.sh"
	[ "$status" -eq 0 ] || {
		echo "$output"
		return 1
	}
...
lock_path="$INSTALL_DIR/.diggory-update.lock/kernel.lock"
[[ "$INSTALL_LOCK_PATH" == "$lock_path" ]] || exit 1
if /bin/ls -lde "$INSTALL_DIR/.diggory-update.lock" | /usr/bin/grep -Eq '^[[:space:]]+[0-9]+:'; then
...
acquire_install_lock
release_install_lock
! compgen -G "$INSTALL_DIR/.diggory-update.lock/control.*" > /dev/null
...
! grep -qF 'Another Diggory installation or update is already writing' "$PROJECT_ROOT/install.sh"
...
! grep -qF 'Could not acquire the Diggory installation lock for' "$PROJECT_ROOT/install.sh"
...
stubdir="$(mktemp -d "${TMPDIR:-/tmp}/diggory-gh-stub.XXXXXX")"
cat > "$stubdir/gh" <<'STUB'
#!/bin/bash
case "$1 $2" in
...
STUB
chmod +x "$stubdir/gh"
target="$(mktemp "${TMPDIR:-/tmp}/diggory-att-file.XXXXXX")"
...
file="$(mktemp "${TMPDIR:-/tmp}/diggory-asset.XXXXXX")"
...
# cannot verify (status 2) + DIGGORY_REQUIRE_ATTESTATION=1 -> fatal
verify_release_attestation() { return 2; }
out="$(DIGGORY_REQUIRE_ATTESTATION=1 verify_release_asset_checksum V1.0.0 "$asset" "$file")" && rc=0 || rc=$?
[ "$rc" -eq 1 ] || { echo "WRONG: require-gate rc=$rc want 1"; exit 1; }
[[ "$out" == *"ERROR:DIGGORY_REQUIRE_ATTESTATION=1 set but gh"* ]] || { echo "WRONG: require-gate error missing: $out"; exit 1; }
...
mutex="$INSTALL_DIR/.diggory-update.lock/holder"
...
@test "the update path never runs brew inside the pre-authed window" {
	...
	if command grep -vE '^[[:space:]]*#' "$PROJECT_ROOT/install.sh" |
		command grep -q 'brew list diggory'; then
		echo "brew list diggory is back in install.sh"
		return 1
	fi
	command grep -q 'homebrew_owns_diggory()' "$PROJECT_ROOT/install.sh" || {
	...
	eval "$(sed -n '/^homebrew_owns_diggory()/,/^}/p' "$PROJECT_ROOT/install.sh")"
	local fake_prefix="$BATS_TEST_TMPDIR/fakebrew"
	mkdir -p "$fake_prefix/Cellar/diggory"
	HOMEBREW_PREFIX="$fake_prefix" homebrew_owns_diggory || {
		echo "cellar dir not detected"
		return 1
	}
	if HOMEBREW_PREFIX="$BATS_TEST_TMPDIR/empty" homebrew_owns_diggory 2> /dev/null &&
		[[ ! -d /opt/homebrew/Cellar/diggory && ! -d /usr/local/Cellar/diggory ]]; then
