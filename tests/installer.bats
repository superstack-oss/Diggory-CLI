...
@test "scan_installers_in_path (fallback find): honors DIGGORY_INSTALLER_SCAN_MAX_DEPTH" {
	mkdir -p "$HOME/Downloads/level1"
	touch "$HOME/Downloads/top.dmg"
	touch "$HOME/Downloads/level1/nested.dmg"

	run env PATH="/usr/bin:/bin" DIGGORY_INSTALLER_SCAN_MAX_DEPTH=1 /bin/bash -euo pipefail -c "
        export DIGGORY_TEST_MODE=1
        source \"\$1\"
        scan_installers_in_path \"\$2\"
    " bash "$PROJECT_ROOT/bin/installer.sh" "$HOME/Downloads"
...
@test "delete_selected_installers removes selected files and records successes" {
	...
	run env HOME="$HOME" TERM="$TERM" /bin/bash -euo pipefail -c '
        ...
        export DIGGORY_DELETE_LOG="$HOME/deletions.log"
        source "$1"

        INSTALLER_PATHS=("$2" "$3")
        INSTALLER_SIZES=(3 3)
        DIGGORY_SELECTION_RESULT="0,1"
        ...
        [[ ! -e "$2" ]] || return 1
        [[ ! -e "$3" ]] || return 1
        grep -F "[installer] REMOVED $2" "$HOME/Library/Logs/diggory/operations.log" > /dev/null
