...
@test "check_touchid_support falls back to legacy sudo when sudo_local is present" {
    ...
    export DIGGORY_PAM_SUDO_FILE="$pam_sudo"
    export DIGGORY_PAM_SUDO_LOCAL_FILE="$pam_sudo_local"
...
@test "stop_sudo_session cleans up keepalive process" {
    export DIGGORY_SUDO_KEEPALIVE_PID="99999"

    run /bin/bash -c "export DIGGORY_SUDO_KEEPALIVE_PID=99999; source '$PROJECT_ROOT/lib/core/common.sh'; source '$PROJECT_ROOT/lib/core/sudo.sh'; stop_sudo_session"
    [ "$status" -eq 0 ]
}

@test "sudo manager initializes global state correctly" {
    result=$(/bin/bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; source '$PROJECT_ROOT/lib/core/sudo.sh'; echo \$DIGGORY_SUDO_ESTABLISHED")
