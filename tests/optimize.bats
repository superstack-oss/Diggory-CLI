...
@test "dry-run keeps healthy conditional system tasks unchanged" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_DRY_RUN=1 DIGGORY_ASSUME_VPN_ACTIVE=0 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
...
@test "opt_system_maintenance reports DNS and Spotlight" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_DRY_RUN=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
...
@test "opt_fix_broken_configs reports partial results when scan hits its time budget" {
	...
	run env HOME="$test_home" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TIMEOUT_HINT_SCAN_SEC=0 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/maintenance.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
...
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
opt_quarantine_cleanup() { echo "quarantine"; optimize_task_result "$DIGGORY_OPTIMIZE_OUTCOME_APPLIED"; }
...
pgrep() { return 1; }
file() { echo "SQLite 3.x database"; }
get_file_size() { echo $((DIGGORY_SQLITE_MAX_SIZE + 1)); }
...
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
opt_cache_refresh() { echo "cache"; optimize_task_result "$DIGGORY_OPTIMIZE_OUTCOME_APPLIED"; }
optimize_outcomes_reset
execute_optimization cache_refresh
[[ "$(optimize_outcome_count applied)" == "1" ]] || exit 1
EOF
...
@test "opt_spotlight_index_optimize dry-run reports rebuild when probes are slow" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_DRY_RUN=1 DIGGORY_OPTIMIZE_SPOTLIGHT_SLOW_SEC=-1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
...
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
opt_prune_spotlight_orphan_rules() { echo "pruned"; optimize_task_result "$DIGGORY_OPTIMIZE_OUTCOME_APPLIED"; }
optimize_outcomes_reset
execute_optimization spotlight_orphan_rules_cleanup
[[ "$(optimize_outcome_count applied)" == "1" ]] || exit 1
EOF
...
<dict>
    ...
    <key>ProgramArguments</key>
    <array>
        <string>/Volumes/DiggoryNonexistentDisk/tool</string>
    </array>
</dict>
</plist>
PLIST
...
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
opt_launch_agents_cleanup() { echo "launch_agents"; optimize_task_result "$DIGGORY_OPTIMIZE_OUTCOME_APPLIED"; }
optimize_outcomes_reset
execute_optimization launch_agents_cleanup
[[ "$(optimize_outcome_count applied)" == "1" ]] || exit 1
EOF
...
periodic() { true; }
export -f periodic
tmplog="$(mktemp /tmp/diggory-test-daily.XXXXXX)"
touch "$tmplog"
DIGGORY_PERIODIC_LOG="$tmplog" execute_optimization periodic_maintenance
...
periodic() { true; }
export -f periodic
tmpdir="$(mktemp -d /tmp/diggory-test-stat-path.XXXXXX)"
mkdir -p "$tmpdir/bin"
...
touch "$tmplog"
PATH="$tmpdir/bin:$PATH" DIGGORY_PERIODIC_LOG="$tmplog" execute_optimization periodic_maintenance
[[ "$(optimize_outcome_count unchanged)" == "1" ]] || exit 1
rm -rf "$tmpdir"
EOF
...
periodic() { true; }
export -f periodic
DIGGORY_PERIODIC_LOG="/tmp/diggory-test-nonexistent-daily.out" execute_optimization periodic_maintenance
[[ "$(optimize_outcome_count applied)" == "1" ]] || exit 1
EOF
...
@test "opt_periodic_maintenance reports skipped without admin access" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
periodic() { true; }
export -f periodic
DIGGORY_PERIODIC_LOG="$HOME/missing-daily.out" execute_optimization periodic_maintenance
[[ "$(optimize_outcome_count skipped)" == "1" ]] || exit 1
EOF
...
@test "opt_periodic_maintenance reports command failure" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=0 DIGGORY_TEST_MODE=0 DIGGORY_OPTIMIZE_SUDO_AVAILABLE=true /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
periodic() { true; }
...
@test "opt_disk_verify reports a timed out probe as failed" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_ENABLE_DISK_VERIFY=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
...
@test "opt_network_stack_optimize reports a partial flush failure" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_ASSUME_VPN_ACTIVE=0 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
mkdir -p "$HOME/bin"
...
@test "run_optimize_diagnostics flags sustained CloudShell as primary bottleneck" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		DIGGORY_OPTIMIZE_PS_SAMPLE_1=$'120 /Applications/AliEntSafe.app/Contents/Services/CloudShell.app/Contents/MacOS/CloudShell --type=event-capture\n35 /usr/libexec/syspolicyd\n20 /System/Library/PrivateFrameworks/SkyLight.framework/Resources/WindowServer' \
		DIGGORY_OPTIMIZE_PS_SAMPLE_2=$'140 /Applications/AliEntSafe.app/Contents/Services/CloudShell.app/Contents/MacOS/CloudShell --type=event-processor\n30 /usr/libexec/syspolicyd\n18 /System/Library/PrivateFrameworks/SkyLight.framework/Resources/WindowServer' \
...
@test "run_optimize_diagnostics suppresses one-off CPU spikes" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		DIGGORY_OPTIMIZE_PS_SAMPLE_1=$'180 /Applications/AliEntSafe.app/Contents/Services/CloudShell.app/Contents/MacOS/CloudShell --type=event-capture' \
		DIGGORY_OPTIMIZE_PS_SAMPLE_2=$'5 /Applications/AliEntSafe.app/Contents/Services/CloudShell.app/Contents/MacOS/CloudShell --type=event-capture' \
		/bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/diagnostics.sh"
...
@test "run_optimize_diagnostics offers user-mounted images under syspolicyd pressure in dry-run" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_DRY_RUN=1 \
		DIGGORY_OPTIMIZE_PS_SAMPLE_1=$'55 /usr/libexec/syspolicyd' \
		DIGGORY_OPTIMIZE_PS_SAMPLE_2=$'60 /usr/libexec/syspolicyd' \
		DIGGORY_OPTIMIZE_SPCTL_STATUS="assessments enabled" \
...
@test "run_optimize_diagnostics keeps healthy runs quiet even with user-mounted images" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_DRY_RUN=1 \
		DIGGORY_OPTIMIZE_PS_SAMPLE_1=$'1 /usr/sbin/distnoted' \
		DIGGORY_OPTIMIZE_PS_SAMPLE_2=$'1 /usr/sbin/distnoted' \
		DIGGORY_OPTIMIZE_HDIUTIL_INFO=$'================================================\nimage-path      : /Users/test/Downloads/TestInstaller.dmg\n/dev/disk14s1\t/Volumes/Test Installer\n' \
		/bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
@test "run_optimize_diagnostics honors optimize whitelist paths for mounted images (#977)" {
	...
	cat > "$HOME/.config/diggory/whitelist_optimize" <<'EOF'
system_maintenance
...
@test "run_optimize_diagnostics stays quiet when nothing matches" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		DIGGORY_OPTIMIZE_PS_SAMPLE_1=$'4 /usr/sbin/distnoted\n3 /usr/libexec/coreaudiod' \
		DIGGORY_OPTIMIZE_PS_SAMPLE_2=$'5 /usr/sbin/distnoted\n2 /usr/libexec/coreaudiod' \
		/bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/diagnostics.sh"
...
DIGGORY_DRY_RUN=1 opt_diag_offer_detach_candidates "${image_path}"$'\t'"${mount_path}"
EOF
...
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
opt_periodic_maintenance() { echo "periodic"; optimize_task_result "$DIGGORY_OPTIMIZE_OUTCOME_APPLIED"; }
optimize_outcomes_reset
execute_optimization periodic_maintenance
[[ "$(optimize_outcome_count applied)" == "1" ]] || exit 1
EOF
...
@test "optimize_sudo_available returns false when sudo session was denied" {
	run env PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_OPTIMIZE_SUDO_AVAILABLE="false" /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
...
@test "optimize_sudo_available returns false in test mode regardless of optimize entrypoint" {
	# Ad-hoc task invocation under DIGGORY_TEST_NO_AUTH must hard-deny sudo
	# even when DIGGORY_OPTIMIZE_SUDO_AVAILABLE was never set by bin/optimize.sh.
	run env PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
unset DIGGORY_OPTIMIZE_SUDO_AVAILABLE
if optimize_sudo_available; then
...
@test "flush_dns_cache does not invoke sudo under DIGGORY_TEST_NO_AUTH" {
...
@test "sudo-required optimize tasks short-circuit without invoking sudo when access denied" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		DIGGORY_OPTIMIZE_SUDO_AVAILABLE="false" \
		DIGGORY_DRY_RUN="0" \
		/bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
@test "has_active_vpn_interface respects DIGGORY_ASSUME_VPN_ACTIVE override" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_ASSUME_VPN_ACTIVE=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
...
@test "has_active_vpn_interface returns false when DIGGORY_ASSUME_VPN_ACTIVE=0" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_ASSUME_VPN_ACTIVE=0 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
