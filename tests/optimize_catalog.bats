...
for action in "${DIGGORY_OPTIMIZE_ACTIONS[@]}"; do
...
for ((index = 0; index < ${#DIGGORY_OPTIMIZE_ACTIONS[@]}; index++)); do
    printf -v row '%s|%s|%s|%s|%s|%s' \
        ...
        "${DIGGORY_OPTIMIZE_DESCRIPTIONS[$index]}" \
        "${DIGGORY_OPTIMIZE_SAFE_VALUES[$index]}"
    actual+="${actual:+$'\n'}$row"
done
...
[[ ${#DIGGORY_OPTIMIZE_ACTIONS[@]} -eq 21 ]] || exit 1
for handler in "${DIGGORY_OPTIMIZE_HANDLERS[@]}"; do
