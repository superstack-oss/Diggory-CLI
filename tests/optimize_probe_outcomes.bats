...
@test "sudo-dependent maintenance is skipped when admin access is denied" {
	run env HOME="$TEST_HOME/admin" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_OPTIMIZE_SUDO_AVAILABLE=false /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
...
[[ "$system_body" == *'run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" mdutil -s /'* ]] || exit 1
[[ "$saved_body" == *'run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" find'* ]] || exit 1
[[ "$network_body" == *'run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" route -n get default'* ]] || exit 1
[[ "$network_body" == *'run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" dscacheutil -q host'* ]] || exit 1
[[ "$vpn_body" == *'run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" scutil --nc list'* ]] || exit 1
[[ "$vpn_body" == *'run_with_timeout "$DIGGORY_TIMEOUT_SHORT_QUERY_SEC" route -n get default'* ]] || exit 1
[[ "$shared_body" == *'run_with_timeout "$DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC" find'* ]] || exit 1
EOF
...
@test "network probe timeout never authorizes maintenance" {
	run env HOME="$TEST_HOME/network-timeout" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_ASSUME_VPN_ACTIVE=0 DIGGORY_DRY_RUN=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
