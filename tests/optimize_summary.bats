...
@test "optimize dry-run summary reports outcomes instead of catalog size" {
	run env HOME="$TEST_HOME" DIGGORY_TEST_NO_AUTH=1 DIGGORY_ASSUME_VPN_ACTIVE=0 NO_COLOR=1 "$PROJECT_ROOT/diggory" optimize --dry-run
	...
	run env HOME="$TEST_HOME" "$PROJECT_ROOT/diggory" history --json
	[[ "$status" -eq 0 ]] || { echo "$output"; return 1; }
...
@test "optimize failure reaches terminal exit and history contracts" {
	local config_dir="$TEST_HOME/.config/diggory"
...
EOF
	...
	run env HOME="$TEST_HOME" DIGGORY_TEST_NO_AUTH=1 NO_COLOR=1 PATH="$stub_dir:$PATH" \
		"$PROJECT_ROOT/diggory" optimize
