...
@test "diggory_purge_is_cloud_synced_path matches only exact cloud roots and descendants" {
...
diggory_purge_is_cloud_synced_path "$HOME/Library/CloudStorage"
diggory_purge_is_cloud_synced_path "$HOME/Library/CloudStorage/Provider/project/target"
diggory_purge_is_cloud_synced_path "$HOME/Library/Mobile Documents"
diggory_purge_is_cloud_synced_path "$HOME/Library/Mobile Documents/com~apple~CloudDocs/project/node_modules"
diggory_purge_is_cloud_synced_path "$physical_home/Library/CloudStorage/Provider/project/target"
diggory_purge_is_cloud_synced_path "$physical_home/Library/Mobile Documents/com~apple~CloudDocs/project/node_modules"

if diggory_purge_is_cloud_synced_path "$HOME/Library/CloudStorageBackup/project/target"; then
	exit 1
fi
if diggory_purge_is_cloud_synced_path "$HOME/Library/Mobile Documents-old/project/target"; then
	exit 1
fi
if diggory_purge_is_cloud_synced_path "$physical_home/Library/CloudStorageBackup/project/target"; then
	exit 1
fi
if diggory_purge_is_cloud_synced_path "$physical_home/Library/Mobile Documents-old/project/target"; then
	exit 1
fi
EOF
...
@test "compact_purge_scan_path keeps the tail of long purge paths visible" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_SKIP_MAIN=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
...
grep -q "^~/" "$HOME/.config/diggory/purge_paths"
EOF
...
@test "scan_purge_targets: trusts empty fd result without falling back to find" {
	mkdir -p "$HOME/.config/diggory" "$HOME/www/empty-project"
	printf '%s\n' "$HOME/www" > "$HOME/.config/diggory/purge_paths"
...
mkdir -p "$HOME/.cache/diggory"
echo "0" > "$HOME/.cache/diggory/purge_stats"
...
export DIGGORY_PURGE_INCLUDE_EMPTY=1
export DIGGORY_DRY_RUN=1
clean_project_artifacts </dev/null

stats_dir="${XDG_CACHE_HOME:-$HOME/.cache}/diggory"
...
echo "0" > "$HOME/.cache/diggory/purge_count"
...
mkdir -p "$artifact" "$HOME/.cache/diggory"
...
mkdir -p "$cloud_artifact" "$HOME/.cache/diggory"
...
mkdir -p "$cloud_artifact" "$local_artifact" "$HOME/.cache/diggory"
...
@test "digg purge: accepts --debug flag" {
	...
	run /bin/bash -c "
        export HOME='$HOME'
        $timeout_cmd 10 '$PROJECT_ROOT/diggory' purge --debug < /dev/null 2>&1 || true
    "
	true
}
...
@test "digg purge: accepts --dry-run flag" {
	...
	run /bin/bash -c "
        export HOME='$HOME'
        $timeout_cmd 10 '$PROJECT_ROOT/diggory' purge --dry-run < /dev/null 2>&1 || true
    "
...
@test "digg purge: accepts --include-empty flag" {
	...
	run /bin/bash -c "
        export HOME='$HOME'
        $timeout_cmd 10 '$PROJECT_ROOT/diggory' purge --include-empty --dry-run < /dev/null 2>&1
    "
...
@test "digg purge: creates cache directory for stats" {
	...
	/bin/bash -c "
        export HOME='$HOME'
        $timeout_cmd 10 '$PROJECT_ROOT/diggory' purge < /dev/null 2>&1 || true
    "

	[ -d "$HOME/.cache/diggory" ] || [ -d "${XDG_CACHE_HOME:-$HOME/.cache}/diggory" ]
}
