...
EOF
    ...
    [[ "$output" != *"cannot be removed safely by Diggory"* ]]
}
...
diggory_delete() { echo "UNEXPECTED_DELETE"; return 1; }
...
EOF
    ...
    [ "$status" -eq 0 ] || return 1
    [[ "$output" == *"cannot be removed safely by Diggory from this location"* ]] || return 1
...
diggory_delete() { echo "UNEXPECTED_DELETE:$*"; return 0; }
...
_DIGGORY_UNINSTALL_LIVE_APP_ROOTS=("$APP_ROOT")
_DIGGORY_UNINSTALL_LIVE_VOLUMES_ROOT="$HOME/no-volumes"
uninstall_live_bundle_has_other_install \
    "com.example.live-shared" "$APP_ROOT/Selected.app"
first_fingerprint="$_DIGGORY_UNINSTALL_LIVE_SIBLING_FINGERPRINT"
[[ -n "$first_fingerprint" && ${#_DIGGORY_UNINSTALL_LIVE_SIBLING_PATHS[@]} -eq 1 ]]
...
uninstall_live_bundle_has_other_install \
    "com.example.live-shared" "$APP_ROOT/Selected.app"
[[ ${#_DIGGORY_UNINSTALL_LIVE_SIBLING_PATHS[@]} -eq 2 ]]
[[ "$first_fingerprint" != "$_DIGGORY_UNINSTALL_LIVE_SIBLING_FINGERPRINT" ]]
...
uninstall_live_bundle_has_other_install \
    "com.example.live-shared" "$APP_ROOT/Selected.app"
[[ ${#_DIGGORY_UNINSTALL_LIVE_SIBLING_PATHS[@]} -eq 3 ]]
EOF
...
[[ ${#_DIGGORY_UNINSTALL_LIVE_SIBLING_PATHS[@]} -eq 1 ]]
EOF
...
_DIGGORY_UNINSTALL_LIVE_VOLUMES_ROOT="$VOLUMES_ROOT"
live_rc=0
uninstall_live_bundle_has_other_install \
    "com.example.volume-root" "$HOME/Selected.app" || live_rc=$?
printf 'LIVE_RC=%s PATHS=%s\n' "$live_rc" "${#_DIGGORY_UNINSTALL_LIVE_SIBLING_PATHS[@]}"
[[ $live_rc -eq 0 ]]
[[ ${#_DIGGORY_UNINSTALL_LIVE_SIBLING_PATHS[@]} -eq 1 ]]
[[ "${_DIGGORY_UNINSTALL_LIVE_SIBLING_PATHS[0]}" == "$SURVIVOR" ]]
EOF
...
rc=0
output=$(DIGGORY_PKG_RECEIPT_CACHE_DISABLE=1 \
...
MOCK
	...
	run env HOME="$HOME/pkg-bound" PROJECT_ROOT="$PROJECT_ROOT" \
		...
		DIGGORY_PKG_RECEIPT_CACHE_DISABLE=1 \
		DIGGORY_PKG_RECEIPT_SCAN_TIMEOUT=1 \
		DIGGORY_PKG_RECEIPT_LIST_TIMEOUT=1 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
uninstall_live_bundle_has_other_install() {
    _DIGGORY_UNINSTALL_LIVE_SIBLING_FINGERPRINT="new-sibling-set"
    _DIGGORY_UNINSTALL_LIVE_SIBLING_PATHS=("$HOME/Applications/New.app")
    return 0
}
...
_DIGGORY_UNINSTALL_LIVE_APP_ROOTS=("$HOME/Applications")
_DIGGORY_UNINSTALL_LIVE_VOLUMES_ROOT="$HOME/no-volumes"
apps_data=(
...
start_inline_spinner() { :; }
stop_inline_spinner() { :; }
diggory_delete() { echo "DIGGORY_DELETE:$1"; return 0; }
...
grep -q "Kept 1 system-level path, which Diggory never removes" "$HOME/output.log"
...
unset DIGGORY_TEST_MODE DIGGORY_TEST_NO_AUTH
...
<plist version="1.0"><dict>
  <key>CFBundleExecutable</key><string>TestApp</string>
  <key>CFBundleIdentifier</key><string>com.example.TestApp&quot; to display dialog &quot;diggory</string>
</dict></plist>
PLIST
...
@test "batch_uninstall_applications proceeds with deletion when force_kill_app fails" {
    # Reproduces the issue where uninstalling a still-running app (e.g. Diggory.app
...
@test "bootout_login_item_helpers never touches the com.apple namespace" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/uninstall/batch.sh"
...
DIGGORY_UNINSTALL_USER_LC_ALL=""
DIGGORY_UNINSTALL_USER_LANG=""
...
EOF
    ...
    [ -f "$HOME/.Trash/diggory-config/whitelist" ] || return 1
}
...
printf '\n' | "$PROJECT_ROOT/diggory" remove --dry-run
EOF
...
EOF
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PATH="$fake_global_bin:/usr/bin:/bin" DIGGORY_TEST_MODE=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
start_inline_spinner() { :; }
stop_inline_spinner() { :; }
export -f start_inline_spinner stop_inline_spinner
...
start_uninstall_interactive_screen() {
    export DIGGORY_ALT_SCREEN_ACTIVE=1
    export DIGGORY_MANAGED_ALT_SCREEN=1
    printf 'start\n' >> "$trace_file"
}
stop_uninstall_interactive_screen() {
    printf 'stop\n' >> "$trace_file"
    unset DIGGORY_ALT_SCREEN_ACTIVE DIGGORY_MANAGED_ALT_SCREEN
}
scan_applications() {
    printf 'scan\n' >> "$trace_file"
    printf '%s\n' "$app_cache_file"
...
select_apps_for_uninstall() {
    [[ "${DIGGORY_ALT_SCREEN_ACTIVE:-}" == "1" ]]
    [[ "${DIGGORY_MANAGED_ALT_SCREEN:-}" == "1" ]]
    printf 'select\n' >> "$trace_file"
    _DIGGORY_MENU_USER_QUIT=1
    return 1
}
...
@test "scan_applications starts feedback before discovery and cleans no-app state" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_FORCE_SCAN_SPINNER=1 /bin/bash --noprofile --norc << 'INNER'
set -euo pipefail

trace_file="$HOME/scan-feedback-trace.log"
scan_temp="$HOME/scan-feedback-temp"

DIGGORY_UNINSTALL_META_CACHE_DIR="$HOME/.cache/diggory"
DIGGORY_UNINSTALL_META_CACHE_FILE="$DIGGORY_UNINSTALL_META_CACHE_DIR/uninstall_app_metadata_v2"
DIGGORY_UNINSTALL_META_CACHE_LOCK="${DIGGORY_UNINSTALL_META_CACHE_FILE}.lock"
...
paginated_multi_select() {
    printf 'guard:%s\n' "${DIGGORY_MENU_IGNORE_INITIAL_ENTER:-unset}" >> "$trace_file"
    printf 'paginated\n' >> "$trace_file"
    DIGGORY_SELECTION_RESULT="0"
    return 0
}

select_apps_for_uninstall
[[ ${#selected_apps[@]} -eq 1 ]]
[[ -z "${DIGGORY_MENU_IGNORE_INITIAL_ENTER:-}" ]]
...
DIGGORY_SELECTION_RESULT=""
set +e
DIGGORY_MENU_IGNORE_INITIAL_ENTER=1 paginated_multi_select "Test Menu" "First App" > "$HOME/menu.out" 2> "$HOME/menu.err"
rc=$?
set -e

echo "rc=$rc"
echo "result=${DIGGORY_SELECTION_RESULT:-}"
INNER
...
DIGGORY_SELECTION_RESULT=""
unset DIGGORY_MENU_SORT_MODE DIGGORY_MENU_SORT_REVERSE DIGGORY_MENU_META_SIZEKB
set +e
DIGGORY_MENU_META_EPOCHS="100,200" paginated_multi_select "Test Menu" "Alpha" "Beta" > "$HOME/menu.out" 2> "$HOME/menu.err" < /dev/null
rc=$?
set -e
echo "rc=$rc"
echo "mode=${DIGGORY_MENU_SORT_MODE:-}"
echo "result=${DIGGORY_SELECTION_RESULT:-}"
[[ $rc -eq 0 ]] || exit 1
INNER
...
DIGGORY_SELECTION_RESULT=""
set +e
DIGGORY_MENU_META_SIZEKB="1,100" DIGGORY_MENU_SORT_MODE=size DIGGORY_MENU_SORT_REVERSE=false paginated_multi_select "Test Menu" "Small" "Large" > "$HOME/menu-default.out" 2> "$HOME/menu-default.err" < /dev/null
default_rc=$?
set -e
echo "default=${DIGGORY_SELECTION_RESULT:-}"

: > "$key_state"
DIGGORY_SELECTION_RESULT=""
set +e
NEXT_KEY="CHAR:O" DIGGORY_MENU_META_SIZEKB="1,100" DIGGORY_MENU_SORT_MODE=size DIGGORY_MENU_SORT_REVERSE=false paginated_multi_select "Test Menu" "Small" "Large" > "$HOME/menu-reverse.out" 2> "$HOME/menu-reverse.err" < /dev/null
reverse_rc=$?
set -e
echo "default_rc=$default_rc"
echo "reverse_rc=$reverse_rc"
echo "reverse=${DIGGORY_SELECTION_RESULT:-}"
...
diggory_delete() { echo "UNEXPECTED_DELETE"; }
...
uninstall_live_bundle_has_other_install() {
    ...
    _DIGGORY_UNINSTALL_DISCOVERY_DEADLINE=$SECONDS
    _DIGGORY_UNINSTALL_LIVE_SIBLING_FINGERPRINT=""
    _DIGGORY_UNINSTALL_LIVE_SIBLING_PATHS=()
    return 1
}
...
@test "uninstall main sets DIGGORY_DELETE_MODE=trash by default" {
    local apps_cache
    apps_cache="$(mktemp "${BATS_TEST_TMPDIR:-$BATS_RUN_TMPDIR:-$HOME}/tmp-723-trash.XXXXXX")"

    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_NO_AUTH=1 \
...
@test "uninstall main sets DIGGORY_DELETE_MODE=permanent with --permanent flag" {
    local apps_cache
...
_bg_items_runner() {
    HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
        ...
        DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 \
        /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/uninstall/batch.sh"
...
@test "_uninstall_match_loaded_background_items checks helper ids under the sibling guard" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEST_MODE=0 DIGGORY_TEST_NO_AUTH=0 \
        /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/uninstall/batch.sh"
