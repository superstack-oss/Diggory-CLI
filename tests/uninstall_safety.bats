...
@test "ByHost cleanup routes through user-mode diggory_delete (no sudo prompt)" {
	mkdir -p "$HOME/Library/Preferences/ByHost"
	touch "$HOME/Library/Preferences/ByHost/com.example.TestApp.ABC123.plist"
...
if grep -q "ByHost.*com.example.TestApp.*plist|true" "$trace"; then
	echo "ByHost plist routed through sudo diggory_delete"
	cat "$trace" >&2
	exit 1
fi
...
[[ "$rc" -eq "$DIGGORY_UNINSTALL_SCAN_PARTIAL" ]] || { echo "RC:$rc want $DIGGORY_UNINSTALL_SCAN_PARTIAL"; exit 1; }
...
mkdir -p "$root"
_DIGGORY_UNINSTALL_LIVE_APP_ROOTS=("$root")
_DIGGORY_UNINSTALL_LIVE_VOLUMES_ROOT="$HOME/no-such-volumes"
...
@test "execution-time partial acceptance is gated on an empty deletion plan" {
	...
	window=$(command grep -A2 'live_sibling_rc -eq \$DIGGORY_UNINSTALL_SCAN_PARTIAL &&' \
