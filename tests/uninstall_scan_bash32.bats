...
@test "scan_applications surfaces inline physical app size before deferred refresh (#1126)" {
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		DIGGORY_TEST_NO_AUTH=1 APPS_ROOT="$apps_root" SRC_PATH="$src" \
		DIGGORY_UNINSTALL_INLINE_MDLS_DISPLAY_TIMEOUT_SEC=0 \
		DIGGORY_UNINSTALL_INLINE_MDLS_SIZE_TIMEOUT_SEC=0 \
...
set -euo pipefail
source "$SRC_PATH"
[[ "$DIGGORY_UNINSTALL_META_CACHE_FILE" == */uninstall_app_metadata_v2 ]]
EOF
...
@test "scan_applications falls back to bounded du when the quick mdls size probe misses" {
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		...
		DIGGORY_UNINSTALL_INLINE_MDLS_DISPLAY_TIMEOUT_SEC=0 \
		DIGGORY_UNINSTALL_INLINE_MDLS_SIZE_TIMEOUT_SEC=0 \
		DIGGORY_UNINSTALL_INLINE_DU_SIZE_TIMEOUT_SEC=0 \
		/bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
...
@test "scan_applications keeps the fast path when cold rows exceed the du fallback cap" {
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		...
		DIGGORY_UNINSTALL_INLINE_MDLS_SIZE_TIMEOUT_SEC=0 \
		DIGGORY_UNINSTALL_INLINE_DU_SIZE_TIMEOUT_SEC=0 \
		DIGGORY_UNINSTALL_INLINE_DU_MAX_COLD_ROWS=0 \
		/bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
...
@test "scan_applications dedupes backup Applications clones by bundle id (#975)" {
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		DIGGORY_TEST_NO_AUTH=1 APPS_ROOT="$apps_root" BACKUP_ROOT="$backup_root" SRC_PATH="$src" \
		/bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
...
@test "scan_applications keeps unique apps from backup Applications roots (#975)" {
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		DIGGORY_TEST_NO_AUTH=1 BACKUP_ROOT="$backup_root" SRC_PATH="$src" \
		/bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
...
MOCK
	...
	sed -e "s|/usr/local/\*.app|$HOME/usr-local/*.app|g" \
		-e 's|DIGGORY_PKG_RECEIPTS_LOADED|DIGGORY_PKG_RECEIPTS_TEST_LOADED|g' \
		"$PROJECT_ROOT/lib/core/pkg_receipts.sh" > "$HOME/pkg_receipts_test.sh"

	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		PATH="$mock_bin:/usr/bin:/bin" \
		DIGGORY_PKG_RECEIPT_CACHE_DISABLE=1 /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
...
MOCK
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PKGS_FILE="$pkgs_file" \
		PATH="$mock_bin:/usr/bin:/bin" \
		DIGGORY_PKG_RECEIPT_CACHE_FILE="$HOME/receipt-cache" \
		/bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
