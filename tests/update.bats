...
make_homebrew_shadow() {
	local bin_dir="$1"
	local cellar_diggory="$2"
	mkdir -p "$bin_dir" "$(dirname "$cellar_diggory")"
	cp "$PROJECT_ROOT/diggory" "$cellar_diggory"
...
#!/usr/bin/env bash
printf '%s\n' "\$*" > "\$INSTALLER_ARGS_LOG"
printf '%s\n' "\${DIGGORY_VERSION:-}" > "\$INSTALLER_VERSION_LOG"
if [[ -n "\${INSTALLER_SUDO_AUTH_LOG:-}" ]]; then
	printf '%s\n' "\${DIGGORY_ASSUME_SUDO_AUTH:-}" > "\$INSTALLER_SUDO_AUTH_LOG"
fi
prefix=""
config=""
while [[ \$# -gt 0 ]]; do
...
printf '#!/bin/bash\necho "Diggory version %s"\n' "\${DIGGORY_VERSION#V}" > "\$prefix/diggory.next"
printf '#!/bin/bash\nexit 0\n' > "\$config/bin/analyze-go"
cp "\$config/bin/analyze-go" "\$config/bin/status-go"
chmod +x "\$prefix/diggory.next" "\$config/bin/analyze-go" "\$config/bin/status-go"
mv "\$prefix/diggory.next" "\$prefix/diggory"
printf 'CHANNEL=stable\nINSTALL_RECEIPT=%s\n' "\${DIGGORY_INSTALL_RECEIPT:-}" > "\$config/install_channel"
rm -f "\$config/.helper_install_incomplete"
echo "Updated to latest version, \${DIGGORY_VERSION#V}"
INSTALLER
	exit 0
fi
...
if [[ -n "\${INSTALLER_COMMIT_LOG:-}" ]]; then
	printf '%s\n' "\${DIGGORY_INSTALL_COMMIT:-}" > "\$INSTALLER_COMMIT_LOG"
fi
if [[ -n "\${INSTALLER_SUDO_AUTH_LOG:-}" ]]; then
	printf '%s\n' "\${DIGGORY_ASSUME_SUDO_AUTH:-}" > "\$INSTALLER_SUDO_AUTH_LOG"
fi
...
mkdir -p "\$prefix" "\$config/bin"
printf '#!/bin/bash\necho "Diggory version nightly"\n' > "\$prefix/diggory.next"
printf '#!/bin/bash\nexit 0\n' > "\$config/bin/analyze-go"
cp "\$config/bin/analyze-go" "\$config/bin/status-go"
chmod +x "\$prefix/diggory.next" "\$config/bin/analyze-go" "\$config/bin/status-go"
mv "\$prefix/diggory.next" "\$prefix/diggory"
printf 'CHANNEL=nightly\nCOMMIT_HASH=%s\nINSTALL_RECEIPT=%s\n' \
	"\${DIGGORY_INSTALL_COMMIT:0:7}" "\${DIGGORY_INSTALL_RECEIPT:-}" > "\$config/install_channel"
rm -f "\$config/.helper_install_incomplete"
echo "Updated to latest version, \${DIGGORY_VERSION#V}"
INSTALLER
	exit 0
...
@test "mo update repairs missing helpers at the current stable version (#1193)" {
	...
	current_version="$(sed -n 's/^VERSION="\([^"]*\)"$/\1/p' "$PROJECT_ROOT/diggory" | head -1)"
	mkdir -p "$fake_bin"
	make_manual_diggory_install "$manual_bin" "$manual_config" "$current_version"
...
@test "mo update retries transient installer download failures" {
	...
	current_version="$(sed -n 's/^VERSION="\([^"]*\)"$/\1/p' "$PROJECT_ROOT/diggory" | head -1)"
	mkdir -p "$fake_bin"
	make_manual_diggory_install "$manual_bin" "$manual_config" "0.0.1"
	make_update_curl_stub "$fake_bin" "$current_version"
	printf '#!/bin/bash\nexit 0\n' > "$fake_bin/sleep"
...
@test "mo update targets the invoked manual install, not another Homebrew diggory in PATH" {
	...
	current_version="$(sed -n 's/^VERSION="\([^"]*\)"$/\1/p' "$PROJECT_ROOT/diggory" | head -1)"
	make_manual_diggory_install "$manual_bin" "$manual_config" "$stale_version"
	make_homebrew_shadow "$fake_brew_bin" "$fake_brew_diggory"
...
@test "mo update --nightly skips reinstall when the installed commit is current" {
	...
	mkdir -p "$fake_bin"
	make_manual_diggory_install "$manual_bin" "$manual_config" "1.41.0"
...
@test "mo update --nightly rejects partial output from a failed or timed-out git probe" {
	...
	for mode in nonzero hang; do
		...
		run env \
			HOME="$HOME" \
			PATH="$fake_bin:/usr/bin:/bin" \
			DIGGORY_TIMEOUT_MEDIUM_PROBE_SEC=0.2 \
			GIT_STUB_MODE="$mode" \
			CURL_URL_LOG="$curl_url_log" \
			GIT_ARGS_LOG="$git_args_log" \
			INSTALLER_ARGS_LOG="$installer_args_log" \
...
DIGGORY_ASSUME_SUDO_AUTH=1 ensure_sudo_ready
grep -qx -- "-n -v" "$SUDO_LOG" || { echo "WRONG: sudo validation was interactive"; cat "$SUDO_LOG"; exit 1; }

: > "$SUDO_LOG"
DIGGORY_ASSUME_SUDO_AUTH=1 maybe_sudo true
...
@test "mo update keeps Homebrew installs on the Homebrew update path" {
	...
	grep -q '^upgrade diggory$' "$brew_log"
}
...
@test "mo update never treats mixed failure output as already installed" {
	...
	local brew_log="$TEST_ROOT/brew.log"
	local brew_upgrade_output
	brew_upgrade_output=$'diggory 1.48.0 already installed\nError: simulated upgrade failure'
...
printf '%s|%s|%s\n' "\${DIGGORY_VERSION:-}" "\$prefix" "\$config" >> "\$HEAL_LOG"
mkdir -p "\$prefix" "\$config/bin"
printf '#!/bin/bash\necho "Diggory version %s"\n' "\${DIGGORY_VERSION#V}" > "\$prefix/diggory.healed"
printf '#!/bin/bash\nexit 0\n' > "\$config/bin/analyze-go"
cp "\$config/bin/analyze-go" "\$config/bin/status-go"
chmod +x "\$prefix/diggory.healed"
chmod +x "\$config/bin/analyze-go" "\$config/bin/status-go"
mv "\$prefix/diggory.healed" "\$prefix/diggory"
printf 'CHANNEL=stable\nINSTALL_RECEIPT=%s\n' "\${DIGGORY_INSTALL_RECEIPT:-}" > "\$config/install_channel"
rm -f "\$config/.helper_install_incomplete"
HEAL
	exit 0
...
@test "mo update prints the manual reinstall command when self-heal fails too" {
	...
	[[ "$output" == *"Retrying with a direct reinstall"* ]] || return 1
	[[ "$output" == *"Update failed"* ]] || return 1
	[[ "$output" == *"DIGGORY_VERSION=V$current_version"* ]] || return 1
...
printf '%s|%s|%s\n' "\${DIGGORY_VERSION:-}" "\$prefix" "\$config" > "\$HEAL_LOG"
[[ "\${DIGGORY_INSTALL_COMMIT:-}" == "$latest_commit" ]] || exit 1
[[ "\${DIGGORY_INSTALL_RECEIPT:-}" == heal-* ]] || exit 1
printf 'CHANNEL=nightly\nCOMMIT_HASH=%s\nINSTALL_RECEIPT=%s\n' \
	"\${DIGGORY_INSTALL_COMMIT:0:7}" "\$DIGGORY_INSTALL_RECEIPT" > "\$config/install_channel"
HEAL
	exit 0
...
! compgen -G "$HOME/install/bin/.diggory-update.lock/control.*" > /dev/null

declare -f _update_acquire_lock | grep -q '/usr/bin/lockf'
declare -f update_diggory | grep -q 'local UPDATE_LOCK_CONTROL=""'
if _update_lock_remove_control "/tmp/not-a-diggory-control" false "$lock_path"; then
...
printf '#!/bin/bash\necho "Diggory version 0.0.1"\n' > "$HOME/bin/diggory"
printf '#!/bin/bash\nexit 0\n' > "$HOME/config/bin/analyze-go"
cp "$HOME/config/bin/analyze-go" "$HOME/config/bin/status-go"
chmod +x "$HOME/bin/diggory" "$HOME/config/bin/analyze-go" "$HOME/config/bin/status-go"
source "$PROJECT_ROOT/lib/core/common.sh"
VERSION="0.0.1"
SCRIPT_DIR="$HOME/config"
source "$PROJECT_ROOT/lib/manage/update.sh"
...
mkdir -p "$config"
[[ "${DIGGORY_INSTALL_COMMIT:-}" == "$expected_commit" ]] || exit 1
[[ "${DIGGORY_INSTALL_RECEIPT:-}" == heal-* ]] || exit 1
printf 'CHANNEL=nightly\nCOMMIT_HASH=def5678\nINSTALL_RECEIPT=%s\n' "$DIGGORY_INSTALL_RECEIPT" > "$config/install_channel"
INSTALLER
}
...
_update_self_heal_reinstall 0 main "$HOME/bin" "$HOME/config" "$HOME/bin/diggory" "nightly build" "$expected_commit"
INNER
...
mkdir -p "$config"
[[ -z "${DIGGORY_INSTALL_COMMIT:-}" ]] || exit 1
[[ "${DIGGORY_INSTALL_RECEIPT:-}" == heal-* ]] || exit 1
printf 'CHANNEL=nightly\nINSTALL_RECEIPT=%s\n' "$DIGGORY_INSTALL_RECEIPT" > "$config/install_channel"
INSTALLER
}
...
_update_self_heal_reinstall 0 main "$HOME/bin" "$HOME/config" "$HOME/bin/diggory" "nightly build" ""
INNER
...
if _update_self_heal_reinstall 0 main "$HOME/bin" "$HOME/config" "$HOME/bin/diggory" "nightly build" ""; then
	exit 1
fi
exit 0
INNER
...
@test "nightly self-heal bounds the installed binary version probe" {
	...
	run "$timeout_cmd" 10 env HOME="$HOME/bounded-version-self-heal" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TIMEOUT_QUICK_DETECT_SEC=0.1 /bin/bash --noprofile --norc << 'INNER'
set -euo pipefail
mkdir -p "$HOME/config" "$HOME/bin"
printf '#!/bin/bash\nsleep 30\n' > "$HOME/bin/diggory"
chmod +x "$HOME/bin/diggory"
...
mkdir -p "$root/readonly-diggory/bin"
touch "$root/readonly-diggory/bin/diggory"
chmod 444 "$root/readonly-diggory/bin/diggory"
if ! update_install_requires_sudo "$root/readonly-diggory/bin"; then
    echo "READONLY_DIGGORY=no-sudo"
fi
EOF
	...
	[[ "$output" == *"READONLY_DIGGORY=no-sudo"* ]] || return 1
}
