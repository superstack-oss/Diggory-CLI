...
@test "prepare_diggory_tmpdir uses writable TMPDIR when available" {
    local writable_tmp="$HOME/custom-tmp"
    mkdir -p "$writable_tmp"

    result=$(env HOME="$HOME" TMPDIR="$writable_tmp" /bin/bash -c "source '$PROJECT_ROOT/lib/core/base.sh'; prepare_diggory_tmpdir")
    [ "$result" = "$writable_tmp" ]
}

@test "prepare_diggory_tmpdir falls back to user cache when TMPDIR is not writable" {
    local blocked_tmp="$HOME/blocked-tmp"
    mkdir -p "$blocked_tmp"
    chmod 500 "$blocked_tmp"

    result=$(env HOME="$HOME" TMPDIR="$blocked_tmp" /bin/bash -c "source '$PROJECT_ROOT/lib/core/base.sh'; prepare_diggory_tmpdir")
    [ "$result" = "$HOME/.cache/diggory/tmp" ]
    [ -d "$HOME/.cache/diggory/tmp" ]
}

@test "ensure_diggory_temp_root caches the first resolved directory" {
    ...
    result=$(env HOME="$HOME" TMPDIR="$first_tmp" /bin/bash -c "
        ...
        first=\$DIGGORY_RESOLVED_TMPDIR
        export TMPDIR='$second_tmp'
        ensure_diggory_temp_root
        second=\$DIGGORY_RESOLVED_TMPDIR
        printf '%s|%s\n' \"\$first\" \"\$second\"
    ")
...
DIGGORY_RESOLVED_TMPDIR="$HOME/.cache/diggory/tmp"
DIGGORY_TEMP_REGISTRY_FILE=""
export DIGGORY_RESOLVED_TMPDIR DIGGORY_TEMP_REGISTRY_FILE
mkdir -p "$DIGGORY_RESOLVED_TMPDIR"
...
# `diggory` resolves the temp root at startup, so the registry path is exported
...
/bin/bash --noprofile --norc -c '
    ...
    ensure_diggory_temp_registry_file || exit 1
    printf "CHILD_OWNS:%s\n" "$DIGGORY_TEMP_REGISTRY_FILE"
    cleanup_temp_files
' || exit 1
...
DIGGORY_RESOLVED_TMPDIR="$HOME/.cache/diggory/tmp"
DIGGORY_TEMP_REGISTRY_FILE="$DIGGORY_RESOLVED_TMPDIR/diggory.registry.$$"
export DIGGORY_RESOLVED_TMPDIR DIGGORY_TEMP_REGISTRY_FILE
mkdir -p "$DIGGORY_RESOLVED_TMPDIR"

persistent_file="$HOME/.cache/diggory/installed_apps_cache"
touch "$persistent_file"
printf '%s\n' "$persistent_file" > "$DIGGORY_TEMP_REGISTRY_FILE"

cleanup_temp_files
[[ -e "$persistent_file" ]] || exit 1

invalid_registry="$HOME/.cache/diggory/not-a-temp-registry"
printf '%s\n' "$persistent_file" > "$invalid_registry"
DIGGORY_TEMP_REGISTRY_FILE="$invalid_registry"
cleanup_temp_files
...
@test "prune_stale_diggory_temp_files leaves persistent cache and fresh temps alone (#1203)" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DIGGORY_TEMP_STALE_MINUTES=60 /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/base.sh"

temp_root="$HOME/.cache/diggory/tmp"
old_temp="$temp_root/old-scan"
fresh_temp="$temp_root/current-scan"
old_spinner="$temp_root/.diggory-spinner.old"
fresh_spinner="$temp_root/.diggory-spinner.fresh"
persistent_cache="$HOME/.cache/diggory/installed_apps_cache"
...
prune_stale_diggory_temp_files "$temp_root"
...
@test "prepare_diggory_tmpdir falls back to /tmp when TMPDIR and invoking home are unavailable" {
...
@test "ensure_user_dir creates nested directory" {
    test_dir="$HOME/.config/diggory/deep/nested/path"
    /bin/bash -c "source '$PROJECT_ROOT/lib/core/base.sh'; ensure_user_dir '$test_dir'"
    [ -d "$test_dir" ]
}
...
@test "ensure_user_file creates file and parent directories" {
    test_file="$HOME/.config/diggory/test.log"
...
@test "multiple ensure_user_file calls in same directory" {
    /bin/bash -c "source '$PROJECT_ROOT/lib/core/base.sh'
        ensure_user_file '$HOME/.config/diggory/file1.txt'
        ensure_user_file '$HOME/.config/diggory/file2.txt'
        ensure_user_file '$HOME/.config/diggory/file3.txt'
    "

    [ -f "$HOME/.config/diggory/file1.txt" ]
    [ -f "$HOME/.config/diggory/file2.txt" ]
    [ -f "$HOME/.config/diggory/file3.txt" ]
}
